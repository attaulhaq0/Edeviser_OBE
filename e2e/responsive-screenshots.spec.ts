...
const PAGES = [
  {
    role: "student",
    email: "student@test.edeviser.com",
    path: "/student/dashboard",
    label: "Student Dashboard",
  },
  {
    role: "teacher",
    email: "teacher@test.edeviser.com",
    path: "/teacher/grading",
    label: "Teacher Grading Queue",
  },
  {
    role: "admin",
    email: "admin@test.edeviser.com",
...
for (const viewport of VIEWPORTS) {
  for (const page of PAGES) {
    test(`${page.label} at ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
      browser,
    }) => {
      ...
      await p
        .waitForURL(new RegExp(`/${page.role}/`), { timeout: 10000 })
        .catch(() => {});
      await p.goto(page.path);
      await p.waitForLoadState("networkidle").catch(() => {});

      await p.screenshot({
        path: `playwright-report/screenshots/${page.label.replace(
          /\s/g,
          "-"
        )}-${viewport.name}.png`,
        fullPage: true,
  ...
  if ((await sidebar.count()) > 0) {
    ...
    if (box) {
      ...
      if (!isHiddenOrCollapsed) {
        console.log(
          `Sidebar visible at ${box.width}px width on mobile — may need responsive fix`
        );
      }
    }
  }

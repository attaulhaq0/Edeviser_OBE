...
// Opt-in guard: this suite requires a SEEDED backend (demo users at the emails
// below). It runs only when explicitly requested — set
// RUN_INTELLIGENCE_CHAIN_E2E=1 (or provide real E2E_*_EMAIL credentials) to enable.
// Without the flag it self-skips, keeping `playwright test --project=legacy-smoke`
// green on environments where the seed data is not guaranteed.
const INTELLIGENCE_CHAIN_ENABLED =
  process.env.RUN_INTELLIGENCE_CHAIN_E2E === "1" ||
  process.env.E2E_ADMIN_EMAIL !== undefined;

test.skip(
  !INTELLIGENCE_CHAIN_ENABLED,
  "Intelligence Chain E2E is opt-in: set RUN_INTELLIGENCE_CHAIN_E2E=1 with a seeded backend."
);
...
/** Authenticated REST GET against Supabase using the browser's live access token. */
async function apiGet<T>(page: Page, path: string): Promise<T[]> {
  ...
  if (!token)
    throw new Error(
      "No access token in session - cannot verify data fixtures."
    );
...
test.describe("Intelligence Layer - OBE hierarchy & mapping direction", () => {
  ...
  test("Admin cannot open a real PLO editor through the ILO edit route (type guard)", async ({
    ...
    const ploCount = await ploLink.count();
    // Fall back to any outcome row whose text marks it as a PLO.
    let editHref: string | null = null;
    if (ploCount > 0) {
      editHref = await ploLink.first().getAttribute("href");
    } else {
      const ploRow = page
  ...
  test("Coordinator curriculum matrix renders canonical parent->child fixtures", async ({
    ...
    const plos = await apiGet<{
      id: string;
      code: string | null;
      title: string | null;
    }>(page, "learning_outcomes?select=id,code,title&type=eq.PLO&limit=10");
    expect(
      plos.length,
    ...
    const labels = plos
      .map((p) =>
        [p.code, p.title].filter(Boolean).join(" ").trim().toLowerCase()
      )
      .filter(Boolean);
    const rendered = labels.some((label) => body.includes(label));
    expect(
      rendered,
      `None of the seeded PLOs (${labels
        .slice(0, 3)
        .join(", ")}...) appear on the matrix`
    ).toBe(true);
  });
});
...
test.describe("Intelligence Layer - agent guardrails", () => {
  ...
  test("Agent executions always reference an approved proposal (no auto-execution)", async ({
    ...
    // UI-level invariants on the approval surface.
    await page.goto("/admin/dashboard", { waitUntil: "networkidle" });
    const body = (await page.locator("body").innerText()).toLowerCase();
    expect(body).not.toContain("auto-executed");
    expect(body).not.toContain("automatically executed");
    const proposals = await apiGet<{ id: string; status: string }>(
      page,
      "agent_action_proposals?select=id,status&status=eq.pending&limit=5"
    );
    if (proposals.length > 0) {
      const approveControl = page
        .getByRole("button", { name: /approve/i })
        .first();
      await expect(approveControl).toBeVisible({ timeout: 10000 });
    }
  });
});

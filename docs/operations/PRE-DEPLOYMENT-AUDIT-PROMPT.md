...
**Client-side (VITE\_ prefix):** `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_SENTRY_DSN` (optional)
...
## PART 13: Backend ↔ Frontend Connectivity & Seamless Integration

This part verifies that every frontend feature that depends on a backend service (Supabase tables, Edge Functions, Realtime channels, Storage buckets, cron jobs) is properly wired end-to-end with no broken links, mismatched contracts, or silent failures.

### 13.1 — Hook → Supabase Table Contract Verification

For every TanStack Query hook in `src/hooks/`, verify:

- The `.from('table_name')` table exists in `src/types/database.ts` `Tables` section
- The `.select('col1, col2, ...')` columns exist on that table in the types
- `.eq()`, `.in()`, `.gte()` filter columns exist and have compatible types
- `.order()` columns exist
- `.insert()` / `.update()` / `.upsert()` payloads match the `Insert` / `Update` types
- No hook references a table or column that was renamed or removed in a recent migration

**Spot-check at minimum these 15 hooks:**
`useUsers`, `useCourses`, `useEnrollments`, `useSubmissions`, `useGradebook`, `useLeaderboard`, `useXPHistory`, `useJournal`, `useChallenges`, `useTeams`, `useAttendance`, `useNotifications`, `useCQIPlans`, `useSurveys`, `useHeatmapData`

For each, record: table name, select columns, filter columns, mutation type — and whether all match the DB types.

### 13.2 — Edge Function Invocation Contract

For every `supabase.functions.invoke('function-name', { body })` call in `src/`:

- The function name matches a folder in `supabase/functions/`
- The `body` shape matches what the Edge Function's `req.json()` destructures
- The response shape the frontend expects (via type assertions or `.data` usage) matches what the Edge Function returns in `JSON.stringify()`
- Auth pattern is consistent: if the Edge Function expects a JWT, the frontend passes `Authorization: Bearer ${session.access_token}`; if it expects service role key, it's only called from server-side (cron)

**List every invocation with:**

| Frontend File | Function Name | Body Fields Sent | Response Fields Expected | Edge Function Exists | Contract Match |
| ------------- | ------------- | ---------------- | ------------------------ | -------------------- | -------------- |
| ...           | ...           | ...              | ...                      | ✅/🚫                | ✅/⚠️/🚫       |

Flag any mismatch as a blocker.

### 13.3 — Realtime Subscription ↔ Table Alignment

For every Realtime subscription (search for `.channel(`, `supabase.channel(`, `useRealtime`, `.on('postgres_changes'`):

- The subscribed table exists in the DB
- The `event` type (`INSERT`, `UPDATE`, `DELETE`, `*`) is appropriate
- The `filter` column exists on the table
- The `schema` is `public` (or explicitly correct)
- RLS policies allow the subscribing role to see the changes (a student subscribing to `xp_transactions` must have a SELECT policy)

### 13.4 — Storage Bucket References

Search `src/` for `supabase.storage.from('bucket-name')`:

- Each referenced bucket exists in Supabase Storage
- Upload policies allow the role performing the upload
- Download/public URL generation matches the bucket's public/private setting
- File size limits in frontend validation match Storage bucket limits

### 13.5 — Cron → Edge Function → DB Write Chain

For each Vercel cron job in `api/cron/`:

- Trace the full chain: Vercel cron → `invokeEdgeFunction('name')` → Edge Function → DB write
- Verify the Edge Function writes to tables that exist
- Verify the columns written match the current schema
- Verify the cron schedule makes sense (e.g., streak-reset runs daily, not hourly)
- Verify `CRON_SECRET` auth is checked at the Vercel handler level

**Document the full chain for each cron:**

| Cron        | Schedule | Vercel Handler          | Edge Function    | Tables Written       | Auth        | Status |
| ----------- | -------- | ----------------------- | ---------------- | -------------------- | ----------- | ------ |
| streak-risk | ...      | api/cron/streak-risk.ts | streak-risk-cron | student_gamification | CRON_SECRET | ?      |
| ...         | ...      | ...                     | ...              | ...                  | ...         | ?      |

### 13.6 — Mutation → Trigger → Side-Effect Chain

For critical user actions, trace the full data flow:

**Grading flow:**

1. Teacher submits grade → `useGradebook` mutation → `submissions` table UPDATE
2. DB trigger fires → calls `calculate-attainment-rollup` Edge Function
3. Edge Function writes to `outcome_attainment` (UPSERT)
4. Realtime notifies student dashboard → CLO progress updates

**XP award flow:**

1. Action triggers → `award-xp` Edge Function called
2. Edge Function inserts into `xp_transactions` (append-only)
3. Edge Function updates `student_gamification.xp_total`
4. `check-badges` called → may insert into `student_badges`
5. Realtime notifies student → XP toast + badge modal

**Verify each step in the chain exists and is connected.** Flag any broken link as a blocker.

### 13.7 — Error Boundary ↔ Backend Failure Handling

Verify the frontend handles backend failures gracefully:

- TanStack Query `onError` callbacks show Sonner toasts (not silent failures)
- Edge Function timeouts (>30s) are handled with user feedback
- Network errors trigger the offline queue or retry logic
- 401 responses redirect to login (session expired)
- 403 responses show "access denied" (not a blank screen)
- 500 responses show error boundary or toast (not unhandled promise rejection)

Search for patterns that swallow errors:
...
Flag each as a warning.

### 13.8 — Query Key Consistency

Read `src/lib/queryKeys.ts` (or wherever query keys are defined):

- Every hook uses keys from the centralized `queryKeys` object (not inline strings)
- Mutation `onSuccess` invalidates the correct query keys
- No two different data sources share the same query key (would cause cache collisions)
- Optimistic updates use the same key structure as the query they're updating

### 13.9 — Dead Backend References (Orphan Detection)

Search for backend references that point to nothing:

- Frontend hooks calling `.from('table_name')` where `table_name` doesn't exist in DB
- Frontend invoking Edge Functions that don't exist in `supabase/functions/`
- Edge Functions querying tables that don't exist
- Cron handlers calling Edge Functions that don't exist
- `queryKeys` entries that no hook uses (stale keys)
- Hooks exported but never imported by any component or page

### 13.10 — API Response Shape Validation

For the 5 most critical data flows, verify the frontend's TypeScript type expectations match what the backend actually returns:

1. **Login** → `supabase.auth.signInWithPassword()` → session shape
2. **Dashboard load** → multiple hooks → verify each returns data matching component props
3. **Grade submission** → mutation → verify response shape matches `onSuccess` handler
4. **XP award** → Edge Function response → verify frontend parses it correctly
5. **Leaderboard** → `leaderboard_weekly` view → verify columns match component rendering

Flag any type mismatch where the frontend assumes a field exists but the backend doesn't return it.

---

## PART 14: Post-Deploy Smoke Test Plan
...
[ ] Grade a submission → attainment rollup fires → student CLO progress updates
[ ] Award XP → xp_transactions row created → leaderboard reflects new total
[ ] Submit assignment → evidence record created → CLO attainment recalculated
[ ] Trigger streak login → streak count increments → streak display updates
[ ] Open student dashboard → Realtime subscription active → live updates work
```
...
- Part 12: Deployment Checklist — [status]
- Part 13: Backend ↔ Frontend Connectivity — [status]
...
| Hooks total                          | XXX                   |
| Hooks with verified DB contract      | XX / XX               |
| Edge Function invocations (frontend) | XX                    |
| Edge Function contract mismatches    | XX                    |
| Realtime subscriptions               | XX                    |
| Orphaned backend references          | XX                    |
| Broken data flow chains              | XX                    |
| Swallowed error patterns             | XX                    |
...
### Part 13: Backend ↔ Frontend Connectivity

[Full details including:

- Hook → Table contract verification results (13.1)
- Edge Function invocation contract table (13.2)
- Realtime subscription alignment (13.3)
- Storage bucket references (13.4)
- Cron → Edge Function → DB chain table (13.5)
- Mutation → Trigger → Side-effect chain traces (13.6)
- Error handling audit (13.7)
- Query key consistency (13.8)
- Dead backend references / orphans (13.9)
- API response shape validation (13.10)]

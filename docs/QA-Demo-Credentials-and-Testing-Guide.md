...
All demo accounts share one password, but it is **not stored in this repo**.
The original QA password was rotated on 2026-07-08 and the current value now
lives only in:

- the gitignored `.env.local` (`VITE_DEMO_PASSWORD`) — drives local demo login, and
- Supabase Auth — the live credential itself.

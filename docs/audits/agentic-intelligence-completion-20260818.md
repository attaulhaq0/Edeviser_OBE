...
- Verification: clean Docker replay through `00008`; rollback-only CQI SQL proof for improved, unchanged, declined, insufficient, and equal-count incompatible populations; focused deterministic TypeScript contract. The negative incompatible-population case is `INSUFFICIENT_EVIDENCE` with no delta.

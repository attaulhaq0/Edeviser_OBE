...
The CodeRabbit review identified five merge-risk areas. The branch now has explicit safeguards and local evidence for each: atomic replacement rollback remains transaction-safe (including failures during the insert after deletion), non-loopback HTTP is rejected, endpoint calls are bounded, embedding metadata is validated, benchmark scoring is fail-closed, and tutor generation is blocked without authorized evidence. These safeguards still require exact-head CI and Preview revalidation on the new PR revision.

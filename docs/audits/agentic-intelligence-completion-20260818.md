...
The normal repository release path did not reconcile this state: `deploy-migrations.yml` is manual-only and protected by the production environment, while `release.yml` performs no Edge Function deployment and the Preview workflow targets only the isolated Preview project. A focused follow-up workflow now declares the Phase 2 dependency closure and deploys all four affected functions behind the protected `production` environment. It must pass reviewed CI and receive the normal human environment approval before production changes occur. After that release, verify that the deployed Edge Functions use the provider-independent registry path and that no active legacy OpenAI embedding URL/key dependency remains. Do not use this audit as authorization to dispatch the workflow.

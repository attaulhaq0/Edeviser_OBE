...
| `send-invitation-email`, `invitation-preview`, `accept-invitation`, `parent-link`, `resend-webhook`, `send-email-notification` | Shared helper for server calls; public functions use token hashing or Svix signature validation as applicable   | Supabase Edge Functions | New invitation/Parent flow; all six were present on the final Git-associated staging branch, which was deleted after evidence capture | Code-ready; production deployment and secret configuration remain blocked pending manual key verification and complete email/Auth E2E |
| Scheduled/analytics/AI/assessment Edge Functions under `supabase/functions/*`                                                  | `SUPABASE_SECRET_KEYS` via `_shared/serverSecret.ts`; bearer + `apikey` headers                                 | Supabase Edge Functions | Active or scheduled in the repository; deployed versions require platform inventory                                                   | Migrated in the repository; staging deployment and runtime verification remain required                                               |
| `api/_utils/auth.ts`                                                                                                           | `SUPABASE_SECRET_KEYS` via `api/_utils/serverSecret.ts`; bearer + `apikey` headers                              | Vercel server route     | Active only when API route is called                                                                                                  | Migrated locally; legacy fallback is explicit and temporary                                                                           |
...
Do not disable the legacy service-role key until staging verifies every Edge
Function and `api/_utils/auth.ts` consumer, including

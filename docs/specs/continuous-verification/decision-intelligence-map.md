...
| Q4 What intervention?                                                  | Deterministic cited draft builder (problemCaseActions, citations ⊆ case evidence, approval-gated at type level) + draft dialog on Unit-Close; AI-drafted CQI (DeepSeek, citation-fail-closed) machinery — 0 intervention rows live     | L3 arch; L0 operational | AI explanation not wired; owner → approval-inbox write path pending                                                        | Wire draft → agent proposal → approval → learning_interventions         | 7.4 · 8.9              |
...
| Q8 Problem class (student/teacher/assessment/prereq/curriculum-design) | classify_problem_cases_v1 (5 classes, live) + contract suite pinning the rules (decisionStackContract.test.ts) + total taxonomy with fail-safe fallback                                                                                | L2 math; L0 operational | Confusion-matrix accuracy on multi-cause fixtures pending (live data exercises student-signal only)                        | Fixture-based confusion matrix (≥90% dominant-cause accuracy)           | 8.9                    |

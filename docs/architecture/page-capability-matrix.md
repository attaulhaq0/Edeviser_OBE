...
> **Canonical home:** relocated here during the D4 review round. This directory is
> the authoritative location the sync guard below validates against; the file was
> moved out of the Kiro spec folder because that directory is do-not-modify
> territory for coding agents. Future matrix updates happen in THIS file only,
> and every registered pattern must appear exactly once in the tables below.

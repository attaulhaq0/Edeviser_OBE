...
- [x] 3.1 Create `supabase/functions/process-onboarding/index.ts` — Profile computation Edge Function
  - [x] 3.1.1 JWT validation and student_id verification
  - [x] 3.1.2 Fetch onboarding_responses and validate against active questions
  - [x] 3.1.3 Calculate Big Five trait scores (weighted average per dimension) — support partial scores for Day 1
  - [x] 3.1.4 Calculate VARK scores (frequency per modality) and determine dominant style
  - [x] 3.1.5 Calculate baseline CLO scores (% correct per CLO)
  - [x] 3.1.6 Calculate self-efficacy scores (mean per domain, normalized 0–100) — support partial scores for Day 1
  - [x] 3.1.7 Calculate study strategy dimension scores (mean per dimension, normalized 0–100)
  - [x] 3.1.8 Calculate profile_completeness percentage
  - [x] 3.1.9 INSERT/UPSERT student_profiles record with personality_traits, learning_style, self_efficacy, study_strategies JSONB, and profile_completeness
  - [x] 3.1.10 INSERT baseline_attainment records (one per student × course × CLO)
  - [x] 3.1.11 UPDATE profiles SET onboarding_completed = true
  - [x] 3.1.12 Generate micro-assessment schedule for Day 1 flow (INSERT into micro_assessment_schedule)
  - [x] 3.1.13 Invoke award-xp for each completed section + completion bonus (once only)
  - [x] 3.1.14 Invoke check-badges for "Self-Aware Scholar" and "Thorough Explorer"
  - [x] 3.1.15 Handle skipped sections (store null for skipped assessment scores)
  - [x] 3.1.16 If Day 1: invoke generate-starter-week Edge Function
- [x] 3.2 Create `supabase/functions/generate-starter-week/index.ts` — AI Starter Week Plan Edge Function
  - [x] 3.2.1 JWT validation and student_id verification
  - [x] 3.2.2 Fetch enrolled courses with schedules and upcoming deadlines (14 days)
  - [x] 3.2.3 Fetch historical cohort study patterns for similar courses
  - [x] 3.2.4 Determine session count and duration based on self-efficacy tier (low: 5×25min, moderate: 4×35min, high: 3×45min)
  - [x] 3.2.5 Distribute sessions across 7-day window, assign session types based on deadline proximity
  - [x] 3.2.6 INSERT sessions into starter_week_sessions table
  - [x] 3.2.7 Handle edge case: no enrolled courses → generate generic study habit sessions
- [x] 3.3 Create `supabase/functions/suggest-goals/index.ts` — AI Goal Suggestion Edge Function
  - [x] 3.3.1 JWT validation and student_id verification
  - [x] 3.3.2 Fetch student's enrolled courses, attainment levels, self-efficacy score
  - [x] 3.3.3 Fetch upcoming deadlines for the week
  - [x] 3.3.4 Fetch historical cohort goal completion rates
  - [x] 3.3.5 Generate 3 goal suggestions (Easy ≥80%, Moderate 50–79%, Ambitious <50% cohort completion)
  - [x] 3.3.6 Format each goal using SMART structure
  - [x] 3.3.7 INSERT suggestions into goal_suggestions table

## 4. TanStack Query Hooks

- [x] 4.1 Create `src/hooks/useOnboardingProgress.ts` — Query progress, mutation to update step
- [x] 4.2 Create `src/hooks/useOnboardingQuestions.ts` — Queries for personality, learning style, self-efficacy, study strategy, and baseline questions
- [x] 4.3 Create `src/hooks/useOnboardingResponses.ts` — Mutation to save batched responses
- [x] 4.4 Create `src/hooks/useStudentProfile.ts` — Query student profile, mutation to call process-onboarding Edge Function
- [x] 4.5 Create `src/hooks/useBaselineTests.ts` — Query baseline test config, results, and course stats (teacher view)
- [x] 4.6 Create `src/hooks/useMicroAssessments.ts` — Query micro-assessment schedule, today's micro, complete/dismiss mutations
- [x] 4.7 Create `src/hooks/useProfileCompleteness.ts` — Query profile completeness percentage
- [x] 4.8 Create `src/hooks/useStarterWeekPlan.ts` — Query starter week sessions, update session status, generate starter week mutation
- [x] 4.9 Create `src/hooks/useGoalSuggestions.ts` — Query goal suggestions, accept/dismiss mutations, generate suggestions mutation

## 5. Onboarding Wizard UI

- [x] 5.1 Create `src/pages/student/onboarding/OnboardingWizard.tsx` — Full-screen overlay with step navigation, progress bar, back/next/skip controls; Day 1 mode (7 questions) vs full mode
- [x] 5.2 Create `src/pages/student/onboarding/WelcomeStep.tsx` — Welcome message, time estimate (under 3 min for Day 1), XP preview
...
- [x] 13.11 `src/__tests__/unit/smartGoalForm.test.tsx` — SMART form rendering, pre-filled fields, goal text composition on submit
- [x] 13.12 `src/__tests__/unit/goalDifficultyBadge.test.tsx` — Badge rendering for Easy (green), Moderate (amber), Ambitious (red)

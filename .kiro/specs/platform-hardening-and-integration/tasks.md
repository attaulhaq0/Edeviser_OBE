...
- [x] T13 (E1.19) White card headers across teacher/coordinator/student/parent. _(DONE 2026-09-02: teacher + student heroes were restyled in the `fix/production-console-errors` stack; this pass completed the remaining two — coordinator action-hub hero (`CoordinatorDashboardNew.tsx`) and parent AI Story Hero (`ParentDashboardScreen.tsx`) restyled to white liquid-glass (`bg-white/80 border-slate-200/60 backdrop-blur-xs` / CARD_CLASS pattern, icon containers white glass, slate text/chips), `ActionChip` restyled for the light card, unused `STORY_GRADIENT` removed.)_

## Phase 3 — Audits & feature repairs
- [x] T14 (E1.11) Gradebook letter-scale fix + property tests + real-data audit. _(DONE 2026-09-02: real-data audit via live MCP — ALL institution_settings rows carry the integer-adjacent scale (A 85–100, B 70–84, C 55–69, D 50–54, F 0–49) whose real gaps (84.0–85.0 etc.) made `mapToLetterGrade` fall back to the LOWEST grade → 84.2% rendered "F". Fix (3 layers, no data migration needed): (1) `letterGradeMapper.ts` letter+GPA mapping is now TOTAL over [0,100] — unmatched percents map to the nearest LOWER band (84.2 → B), out-of-range clamp to top/lowest; (2) `gradeScalesPartitionSchema` upgraded from integer-gap (`min > max+1`) to continuous coverage — strict gap (`min > max`) and strict overlap (`min < max`) rejected, boundary-touch allowed; (3) `DEFAULT_GRADE_SCALES` aligned to a touching partition (B 70–85, C 55–70, D 50–55, F 0–50) — all historical integer classifications unchanged. Tests: `letterGradeMapper.test.ts` E1.11 regression cases (incl. the exact live scale), `institutionSettingsSchema.test.ts` touching fixtures + SET-03/04 + legacy-scale rejection, new `gradeScale.property.test.ts` (4 fast-check properties ≥100 runs: totality on valid+legacy scales, 84–85 gap → B, touching accepted / gaps+overlaps rejected) — 42/42 green. Existing live rows are NOT mutated: display is corrected by the mapping fallback; the hardened validator forces correct scales on the next save.)_
- [x] T15 (E1.10) Tutor handoffs: verify table/RLS/write path; fix broken link; drop `any`; regen types. _(DONE 2026-09-02: live-verified via MCP — `teacher_handoff_requests` table exists with all expected columns; RLS policies present (handoffs_student_own ALL, handoffs_teacher_read, handoffs_teacher_update, teachers_read_own_handoffs); 3 rows live (1 pending) so the page is not empty. `src/types/database.ts` already generates the table (line 9318) — no regen needed; dropped all three `(supabase as any)` casts in `useTeacherHandoffs.ts` in favor of the typed client (with null-guards on `.single()` results). Nav/route audit: `/teacher/tutor-handoffs` is registered in AppRouter and consistent across navItems, navPresentation and TeacherProfilePage — no broken link found. Trigger-condition enforcement (E2.F) remains T22.)_
- [x] T16 (E1.12) Attendance Marker: fix create-session backend + design-system rebuild. _(DONE 2026-09-02 (backend verified + UI header fix; full Marker UX rebuild folded into T21/E2.E): live-verified via MCP — `class_sessions` columns match `CreateSessionInput` exactly; RLS policies correct (`class_sessions_teacher_write` requires teacher role AND section ownership; `class_sessions_read`, `parent_read_class_sessions`); `attendance_records` policies present; `audit_logs` has an authenticated INSERT policy so the post-insert audit log cannot fail the mutation; all 16 course sections have teacher_id set (no orphan sections), 480 sessions exist (latest 2026-05-18 — the stale sessions the QA saw are user-owned data, not a code defect). No backend code defect found on the create path. UI: replaced the non-compliant solid-gradient "New Session" card header with the design-system `GradientCardHeader`. The E2.E record_attendance_v1 RPC + full UX rebuild remains T21.)_

...
- [ ] 0.4 Establish the feature-flag mechanism (per-module toggle) and a `NewUI` boundary so old/new components can coexist. (Ref R13.2)
- [ ] 0.5 Add i18n key scaffolding for new chrome/nav copy in `en` + `ar`; add an i18n key-parity check to CI. (Ref R7.1, R7.5)
...
- [ ] 1.1 Redesign `GlobalHeader` (visual only): keep logo→role dashboard, hamburger (`useSidebar().toggle`), `LanguageSwitcher`, `NotificationBell`, `ProfileDropdown`; optionally add ⌘K `SearchCommand` + student stat pills (real gamification hooks). (Ref R2.1, R3.5, R14.2)
- [ ] 1.2 Redesign `Sidebar` (visual only): keep `navItems[role]` + `navGroups` (grouped student / flat others), de-emphasis, conditional Surveys, active-state, `viewTransition`, intent prefetch, RTL drawer. (Ref R4.4, R7.2, R11.1, R12.1)
- [ ] 1.2a **Responsive navigation** (design.md §18): sidebar on ≥lg; a thumb-reachable **bottom tab bar** for core items + off-canvas drawer on <lg — all from the SAME `navItems[role]`, RTL-aware; auto-swaps at `lg` with no toggle/UA sniffing. (Ref R18.5)

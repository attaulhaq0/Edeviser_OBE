# (Spec annex) Duolingo Feature Gaps — Gamification/UX rationale for the UI build

> **Why this is in the spec:** it carries the gamification/UX reasoning behind the redesigned surfaces (streak, learning path, daily-goal ring, mistakes/review, mastery, companion persona) so whoever builds the new UI has the _intent_, not just the visuals.
>
> **How to use it during the migration (important):**
>
> - Treat this as **UX reference for presentation**, not a feature backlog. The migration is presentation-only (see requirements R1). Build the _look/interaction_ of a mechanic only when the **backend already supports it** (e.g., streaks, XP, CLO attainment, review queue, tutor all exist); do **not** imply mechanics that have no backend.
> - Map each mechanic to the design-system primitives + archetypes in `design.md` (§3, §15): the Path/next-step → student Dashboard + Detail archetypes; Daily-goal ring → `MasteryRing`/hero; Mistakes/Review → Full-screen focus archetype; mastery → `MasteryRing` + attainment; companion → the AI Tutor's persona in the Tutor screen.
> - The **"flow not goal" guardrail** below is a **binding design principle** for the new UI: visually emphasize outcome-linked progress (mastery, corrected mistakes, submissions), never idle activity.
>
> **Source of truth:** `docs/product/duolingo-feature-gaps.md` (this annex is a synced copy for build-time context).
...
Subtle success sounds and haptics on correct answers / XP gains (mobile). Big part of why Duolingo _feels_ good. Respect a mute/reduced-motion setting.
...
## The "flow not goal" guardrail (important — binding design principle)
...
3. **The Mistakes Bank drives mastery** — the fun loop _is_ the learning loop (retrieval + spacing).

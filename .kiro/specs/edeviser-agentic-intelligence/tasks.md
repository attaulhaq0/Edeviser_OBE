...
- [ ] 3.4 Wire AgentApprovalCard to agent_action_proposals (approve/reject flows calling agent-orchestrator decision endpoint); revalidate authorization at execution time. _([~] Wave D3 PARTIAL: `AgentApprovalCard` + `useProposalDecision` + lib `agentProposals.ts` landed — mutation calls the orchestrator decision endpoint which re-runs assertMayDecideProposal + scope revalidation + race-guarded conditional UPDATE server-side; display gates mirrored client-side are hint-only. INBOX LISTING deferred: live DB has RLS enabled with ZERO policies on agent_action_proposals (deny-all) and no listing RPC — the sanctioned read channel (RPC/policies or orchestrator action) ships before AgentTaskInbox so nothing bypasses RLS.)_

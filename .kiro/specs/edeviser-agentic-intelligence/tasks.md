...
- [x] 4.8 Tests: agents produce derived-alignment language only; risk scores never invented; intervention proposals always require approval per PROTECTED*ACTIONS. *(DONE: `src/__tests__/properties/agentCertification.property.test.ts` Properties 3–5 — numeric-score rejection sweep ×100, uncited-habit-signal rejection ×100, `approvalRequired` falsy-variant rejection ×50; derived-alignment labeling covered by outcomeContextBuilder + LLM-security suites)\_
...
- [x] 5.4 Tests: coordinator reads ILOs but cannot edit; teacher explains alignment but cannot edit ILO/PLO; CQI drafts never auto-apply. _(DONE: agentCertification.property.test.ts Property 6 — specialist allow-list matrix, student governance exclusion, admin-only proposal gating; CQI draft-only structural proof: no I/O primitives in cqi-draft.ts, orchestrator attaches `{ cqiDraft }` as data, `create_cqi_action` ∈ PROTECTED_ACTIONS)_

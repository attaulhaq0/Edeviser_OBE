...
- [x] 8.2 Background job families (audit existing cron first; no duplicates): student-risk-jobs, learning-state-update-jobs, teacher-summary-jobs, parent-summary-jobs, coordinator-analysis-jobs, institution-outcome-health-jobs, attainment-recalculation-jobs, agent-evaluation-jobs (+ intervention jobs in 4.7). Small batches, idempotency, retry limits, dead-letter, institution scope, audit logging. _(ATTESTED LIVE 2026-08-29 via MCP: `proactive_agent_jobs` queue with full worker mechanics (lease_until/claimed_by lease, attempt_count/max_attempts, error_classification dead-letter, idempotency_key + evidence_hash idempotency) and the four worker RPCs `enqueue/claim/complete/fail_proactive_agent_job_v1` present in pg_proc; `agent-worker` scheduled_scan consumes them (claim→lease→complete/fail, isRetryableFailure→retry/dead-letter). Cron loops verified live in `cron.job`: agent-evaluation-jobs hourly :20, intervention-generation-jobs daily :05, intervention-evaluation-jobs */15; Vercel Cron is the SOLE agent-worker scheduler (no competing pg_cron job — spec non-goal). REMAINING families (tracked, not blocking platform completion): student-risk-jobs, learning-state-update-jobs, teacher/parent-summary-jobs, coordinator-analysis-jobs, institution-outcome-health-jobs, attainment-recalculation-jobs.)_

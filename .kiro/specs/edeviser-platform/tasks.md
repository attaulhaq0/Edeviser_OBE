...
- [x] 59. Write tests for platform enhancements
  - [x] 59.1 Write property-based tests (Properties 41–50)
    - **Properties 41-42**: Streak Freeze (consumption correctness, purchase constraints)
    - **Properties 43-44**: Notification batching (rate limiting, batching correctness)
    - **Properties 45-46**: Offline queue (flush integrity, draft round-trip)
    - **Property 47**: Student data export completeness
  ...
  - [x] 59.2 Write unit tests for new modules
  ...
  - [x]* 59.3 Write integration tests for new flows
...
- [x] 67. Implement Discussion Forum
  - [x] 67.1 Create discussion TanStack Query hooks (`/src/hooks/useDiscussions.ts`)
    - CRUD for threads and replies
    - _Requirements: 77_

  - [x] 67.2 Create Discussion Thread List page (`/src/pages/student/discussions/DiscussionForum.tsx`)
    - List threads per course, ordered by pinned DESC, created_at DESC
    - Resolved threads visually distinguished
    - _Requirements: 77.6_

  - [x] 67.3 Create Thread Detail page with replies
    - Teacher can mark reply as "answer" (sets is_resolved on thread)
    - _Requirements: 77.2, 77.3_

  - [x] 67.4 Wire XP awards for discussion participation
    - 10 XP for creating a thread (discussion_question)
    - 15 XP for answer marked correct (discussion_answer)
    - _Requirements: 77.4, 77.5_

  - [x] 67.5 Create Discussion Moderation page for Teachers
    - Pin/unpin threads, mark answers, delete inappropriate content
    - _Requirements: 77.3_

- [x] 68. Implement Attendance Tracking
  - [x] 68.1 Create attendance TanStack Query hooks (`/src/hooks/useAttendance.ts`)
    - CRUD for class sessions and attendance records
    - Attendance percentage calculation
    - _Requirements: 78_

  - [x] 68.2 Create Attendance Marker page (`/src/pages/teacher/attendance/AttendanceMarker.tsx`)
  ...
  - [x] 68.3 Create Attendance Report view
    - Per-student attendance percentage per course
    - Flag students below 75% threshold
    - _Requirements: 78.3, 78.4_

  - [x] 68.4 Display attendance on Student Dashboard
    - Attendance percentage per enrolled course
    - _Requirements: 78.5_

  - [x] 68.5 Wire attendance data into AI at-risk prediction
    - Add attendance frequency as contributing signal in `compute-at-risk-signals`
    - _Requirements: 78.6_

  - [x] 68.6 Wire "Perfect Attendance Week" badge
    - Award badge when student is present for all sessions in a 7-day period
    - _Requirements: 78.7_

- [x] 69. Implement Quiz/Exam Module
  - [x] 69.1 Create quiz TanStack Query hooks (`/src/hooks/useQuizzes.ts`)
    - CRUD for quizzes, questions, and attempts
    - _Requirements: 79_

  - [x] 69.2 Create Quiz Builder page (`/src/pages/teacher/quizzes/QuizBuilder.tsx`)
  ...
  - [x] 69.3 Create Quiz Attempt page (`/src/pages/student/quizzes/QuizAttemptPage.tsx`)
    - Display questions with timer (if time limit set)
    - Enforce max attempts
    - Auto-submit on time expiry
    - _Requirements: 79.3, 79.7_

  - [x] 69.4 Create `auto-grade-quiz` Edge Function or client-side grading logic
    - Auto-grade MCQ, true/false, fill-in-blank immediately
    - Flag short answer for manual teacher grading
    - _Requirements: 79.4_

  - [x] 69.5 Wire quiz scores into CLO attainment pipeline
    - Generate evidence records from quiz scores (same as assignment grades)
    - _Requirements: 79.5_

  - [x] 69.6 Wire XP awards for quiz completion
    - 50 XP on-time, 25 XP if late (same schedule as assignments)
    - _Requirements: 79.6_

- [x] 70. Implement Gradebook with Weighted Categories
  - [x] 70.1 Create gradebook TanStack Query hooks (`/src/hooks/useGradebook.ts`)
    - CRUD for grade categories
    - Gradebook matrix query (students × assessments)
    - _Requirements: 80_

  - [x] 70.2 Create Grade Category Manager
  ...
  - [x] 70.3 Create Gradebook View page (`/src/pages/teacher/gradebook/GradebookView.tsx`)
  ...
  - [x] 70.4 Implement letter grade mapping using institution_settings.grade_scales
    - _Requirements: 80.5_

- [x] 71. Implement Calendar View
  - [x] 71.1 Create calendar TanStack Query hooks (`/src/hooks/useCalendar.ts`)
    - Aggregate events from assignments, quizzes, class_sessions, academic_calendar_events
    - _Requirements: 81_

  - [x] 71.2 Create Calendar View page (`/src/pages/shared/CalendarView.tsx`)
  ...
  - [x] 71.3 Integrate calendar with Student Dashboard deadline widget
    - _Requirements: 81.4_

  - [x] 71.4 Add calendar to dashboard sidebar for Students and Teachers
    - _Requirements: 81.5_

- [x] 72. Implement Timetable
  - [x] 72.1 Create timetable TanStack Query hooks (`/src/hooks/useTimetable.ts`)
    - Query timetable slots for student (from enrolled sections) or teacher (from assigned sections)
    - _Requirements: 82_

  - [x] 72.2 Create Timetable View page (`/src/pages/shared/TimetableView.tsx`)
    - Weekly grid: days as columns, time slots as rows
    - Color-coded by course
    - _Requirements: 82.1, 82.4_

  - [x] 72.3 Admin/Coordinator UI for managing timetable slots
    - Assign day, time, room, slot type per section
    - _Requirements: 82.1_

  - [x] 72.4 Add timetable to dashboard sidebar
    - _Requirements: 82.5_

- [x] 73. Implement Academic Calendar Management
  - [x] 73.1 Create academic calendar TanStack Query hooks (`/src/hooks/useAcademicCalendar.ts`)
    - CRUD for academic calendar events
    - _Requirements: 84_

  - [x] 73.2 Create Academic Calendar Manager page (`/src/pages/admin/calendar/AcademicCalendarManager.tsx`)
    - Create events: semester dates, exam periods, holidays, registration deadlines
    - Support recurring events
    - _Requirements: 84.1_

  - [x] 73.3 Display academic calendar events on unified Calendar View
    - _Requirements: 84.2_

  - [x] 73.4 Implement assignment due date holiday validation
    - Warn teachers when due date falls on a holiday
    - _Requirements: 84.3_

  - [x] 73.5 Wire exam period approach notifications
    - "Exam period starts in 5 days" notification to enrolled students
    - _Requirements: 84.4_

- [x] 74. Implement Student Transcript Generation
  - [x] 74.1 Create `generate-transcript` Edge Function (`/supabase/functions/generate-transcript/`)
  ...
  - [x] 74.2 Create transcript TanStack Query hooks (`/src/hooks/useTranscript.ts`)
    - Mutation for triggering generation
    - _Requirements: 85_

  - [x] 74.3 Add "Download Transcript" button to Student Profile page
    - Semester selector for per-semester or cumulative transcript
    - _Requirements: 85.4_

  - [x] 74.4 Add transcript generation to Admin user detail page
    - _Requirements: 85.4_

- [x] 75. Implement Parent/Guardian Portal
  - [x] 75.1 Create parent TanStack Query hooks (`/src/hooks/useParentDashboard.ts`)
    - Query linked students via parent_student_links
    - Query student grades, attendance, CLO progress, habits (read-only)
    - _Requirements: 86_

  - [x] 75.2 Create Parent Dashboard page (`/src/pages/parent/ParentDashboard.tsx`)
  ...
  - [x] 75.3 Create ParentLayout with sidebar navigation
    - _Requirements: 86.7_

  - [x] 75.4 Implement parent email notifications
    - Grade released, attendance alert (below 75%), at-risk warning
    - _Requirements: 86.4_

  - [x] 75.5 Create parent invite flow for Admin
    - Bulk import or individual invite creating parent profile + parent_student_links record
    - _Requirements: 86.5_

  - [x] 75.6 Create RLS policies for parent role
    - Read-only access to linked student data via verified parent_student_links
    - _Requirements: 86.1, 86.6_

- [x] 76. Implement Fee Management
  - [x] 76.1 Create fee management TanStack Query hooks (`/src/hooks/useFees.ts`)
    - CRUD for fee structures and payments
    - Fee collection summary query
    - _Requirements: 87_

  - [x] 76.2 Create Fee Structure Manager page (`/src/pages/admin/fees/FeeManager.tsx`)
  ...
  - [x] 76.3 Display fee status on Student Profile page
    - Outstanding and paid fees
    - _Requirements: 87.3_

  - [x] 76.4 Implement fee receipt PDF generation
    - Downloadable receipt per payment
    - _Requirements: 87.6_

  - [x] 76.5 Implement overdue auto-flagging via pg_cron
    - Daily check: update pending → overdue when past due_date
    - _Requirements: 87.5_

- [x] 77. Implement Report Generator updates for new features
  - [x] 77.1 Update accreditation reports to include survey results as indirect assessment evidence
    - _Requirements: 70.4_

  - [x] 77.2 Update accreditation reports to include CQI action plans as "closing the loop" evidence
    - _Requirements: 71.4_

  - [x] 77.3 Update reports to scope by semester_id
    - _Requirements: 68.4_

  - [x] 77.4 Update reports to show per-section and aggregated attainment
    - _Requirements: 69.5_

  - [x] 77.5 Update report template selector to include QQA and NCAAA
    - _Requirements: 73.2_

- [x] 78. Wire new XP sources and badges
  - [x] 78.1 Update `award-xp` Edge Function to support new sources
    - Add: `discussion_question`, `discussion_answer`, `survey_completion`, `quiz_completion`
    - _Requirements: 77.4, 77.5, 70.5, 79.6_

  - [x] 78.2 Update `check-badges` Edge Function for new badges
    - Perfect Attendance Week, Quiz Master, Discussion Helper, Survey Completer
    - _Requirements: 78.7, 23_

  - [x] 78.3 Update Read habit to include announcements and course materials
    - Wire `useReadHabitTimer` into announcement and material views
    - _Requirements: 75.5, 76.6_

  - [x] 78.4 Update `compute-at-risk-signals` to include attendance and quiz data
    - _Requirements: 78.6, 41_

- [x] 79. Write tests for institutional management & LMS features
  - [x]* 79.1 Write property-based tests (Properties 51–65)
  ...
  - [x]* 79.2 Write unit tests for institutional management modules
  ...
  - [x]* 79.3 Write integration tests for new flows
    - Quiz attempt → auto-grade → evidence → CLO attainment pipeline
    - Attendance marking → at-risk signal → AI prediction pipeline
    - Discussion post → XP award → badge check pipeline
    - Survey response → indirect evidence → accreditation report pipeline
...
- [x] 80. Final verification — All tests pass, production ready
...
- [x] 81. Implement Multi-Language / RTL Support
  - [x] 81.1 Apply database migration: add `language_preference` column to `profiles`
    - `ALTER TABLE profiles ADD COLUMN language_preference text NOT NULL DEFAULT 'en' CHECK (language_preference IN ('en', 'ur', 'ar'))`
    - Regenerate TypeScript types
    - _Requirements: 88.5_

  - [x] 81.2 Create LanguageProvider (`/src/providers/LanguageProvider.tsx`)
  ...
  - [x] 81.3 Create translation file stubs
  ...
  - [x] 81.4 Create RTL-aware CSS utilities (`/src/styles/rtl.css`)
    - Mirror horizontal padding, margins, flexbox directions, border-radius for RTL
    - Ensure sidebar moves to right side in RTL mode
    - _Requirements: 88.3_

  - [x] 81.5 Create LanguageSelector component (`/src/components/shared/LanguageSelector.tsx`)
    - Dropdown: English, اردو (Urdu), العربية (Arabic)
    - Add to Profile Settings page
    - _Requirements: 88.4_

  - [x] 81.6 Wrap App in LanguageProvider and wire into i18next configuration
    - Extend existing task 40 (i18n foundation) with RTL support
    - _Requirements: 88.1, 88.6_

- [x] 82. Implement Progressive Web App (PWA)
  - [x] 82.1 Create web app manifest (`/public/manifest.json`)
  ...
  - [x] 82.2 Create service worker (`/public/sw.js`)
  ...
  - [x] 82.3 Register service worker in app entry point
    - Register in `src/main.tsx` with scope `/`
    - _Requirements: 89.2_

  - [x] 82.4 Create PWAInstallPrompt component (`/src/components/shared/PWAInstallPrompt.tsx`)
    - Listen for `beforeinstallprompt` event
    - Show install banner on mobile devices
    - Store dismissal in localStorage (suppress for 30 days)
    - _Requirements: 89.3_

  - [x] 82.5 Add PWA meta tags to `index.html`
    - `<link rel="manifest" href="/manifest.json">`
    - `<meta name="theme-color" content="#3b82f6">`
    - `<link rel="apple-touch-icon" href="/icons/icon-192.png">`
    - _Requirements: 89.4_

- [x] 83. Document Backup & Disaster Recovery Procedures
  - [x] 83.1 Create `/docs/disaster-recovery.md` runbook
  ...
  - [x] 83.2 Document monthly backup verification procedure
...
- [x] 84. Implement Edge Function Rate Limiting
  - [x] 84.1 Create rate limiter shared module (`/supabase/functions/_shared/rateLimiter.ts`)
  ...
  - [x] 84.2 Integrate rate limiter into all existing Edge Functions
    - Import shared module at the top of each Edge Function handler
    - Return HTTP 429 with `Retry-After` header when limit exceeded
    - _Requirements: 91.1, 91.2_

  - [x] 84.3 Log rate limit violations to audit_logs
    - `action = 'rate_limit_exceeded'`, `target_type = 'edge_function'`, function name in metadata
    - _Requirements: 91.4_

- [x] 85. Implement Security Headers
  - [x] 85.1 Add security headers to `vercel.json`
...
- [x] 86. Implement Cookie Consent / Privacy Banner
  - [x] 86.1 Create CookieConsentBanner component (`/src/components/shared/CookieConsentBanner.tsx`)
  ...
  - [x] 86.2 Create analytics consent gate utility (`/src/lib/analyticsConsent.ts`)
    - Block Sentry and any analytics scripts until consent is given
    - Initialize analytics only when `analytics: true` in consent
    - _Requirements: 93.3_

  - [x] 86.3 Add "Cookie Settings" link to app footer
    - Opens the Manage Preferences dialog
    - _Requirements: 93.5_

- [x] 87. Implement Terms of Service & Privacy Policy Pages
  - [x] 87.1 Create public routes `/terms` and `/privacy`
  ...
  - [x] 87.2 Add footer links to ToS and Privacy Policy on all pages
    - _Requirements: 94.3_

  - [x] 87.3 Apply database migration: add `tos_accepted_at` column to `profiles`
    - `ALTER TABLE profiles ADD COLUMN tos_accepted_at timestamptz`
    - Regenerate TypeScript types
    - _Requirements: 94.5_

  - [x] 87.4 Create ToSAcceptanceDialog component (`/src/components/shared/ToSAcceptanceDialog.tsx`)
...
- [x] 88. Implement Admin Impersonation / Support Mode
  - [x] 88.1 Create ImpersonationProvider (`/src/providers/ImpersonationProvider.tsx`)
  ...
  - [x] 88.2 Create ImpersonationBanner component (`/src/components/shared/ImpersonationBanner.tsx`)
    - "You are viewing as [user_name] — [role]. Click to exit."
    - Prominent banner at top of every page during impersonation
    - _Requirements: 95.3_

  - [x] 88.3 Add "View as User" button to Admin user detail page
    - Only visible for admin role, only for users within same institution
    - _Requirements: 95.1, 95.5_

  - [x] 88.4 Log impersonation sessions to audit_logs
    - `impersonation_start` and `impersonation_end` actions with actor_id and target_id
    - _Requirements: 95.4_

  - [x] 88.5 Create impersonation TanStack Query hooks (`/src/hooks/useImpersonation.ts`)
    - Mutation for starting/stopping impersonation
    - Query for impersonated user profile
    - _Requirements: 95_

- [x] 89. Implement Bulk Data Operations
  - [x] 89.1 Create bulk grade export Edge Function (`/supabase/functions/bulk-grade-export/`)
  ...
  - [x] 89.2 Create grade export UI on Teacher gradebook page
    - Export button with course/section selector
    - Loading state during generation, download link on completion
    - _Requirements: 96.1_

  - [x] 89.3 Create bulk enrollment import/export for Coordinators
  ...
  - [x] 89.4 Create semester transition tool (`/src/pages/coordinator/courses/SemesterTransition.tsx`)
  ...
  - [x] 89.5 Create bulk data cleanup tool for Admins
  ...
  - [x] 89.6 Create bulk operations TanStack Query hooks (`/src/hooks/useBulkOperations.ts`)
    - Mutations for grade export, enrollment import/export, semester transition, data cleanup
    - _Requirements: 96_

- [x] 90. Document Database Connection Pooling Configuration
  - [x] 90.1 Create `/docs/connection-pooling.md`
  ...
  - [x] 90.2 Configure Edge Functions to use pooler connection URL
    - Update Supabase client initialization in Edge Functions to use pooler URL
    - _Requirements: 97.2_

  - [x] 90.3 Add pool status to health check endpoint
    - Include `pool_status` field in health check response
    - _Requirements: 97.3_

- [x] 91. Implement Image/Asset Optimization
  - [x] 91.1 Create image compressor utility (`/src/lib/imageCompressor.ts`)
    - Client-side compression using canvas API
    - Max 500KB, 256×256 pixels, quality 0.8
    - Returns compressed File object
    - _Requirements: 98.1_

  - [x] 91.2 Wire image compressor into avatar upload flow
    - Compress before uploading to Supabase Storage
    - Show warning if compression fails, upload original as fallback
    - _Requirements: 98.1_

  - [x] 91.3 Add lazy loading to all images
    - `loading="lazy"` on avatars, badge icons, material thumbnails
    - _Requirements: 98.2_

  - [x] 91.4 Configure Supabase Storage image transformations for avatar thumbnails
    - 64×64 and 128×128 thumbnail sizes
    - _Requirements: 98.3_

  - [x] 91.5 Configure Vercel CDN caching headers for static assets
    - `Cache-Control: public, max-age=31536000, immutable` for hashed assets in `vercel.json`
    - _Requirements: 98.4_

- [x] 92. Implement Global Search
  - [x] 92.1 Apply database migration: add full-text search indexes
  ...
  - [x] 92.2 Create SearchCommand component (`/src/components/shared/SearchCommand.tsx`)
  ...
  - [x] 92.3 Create global search TanStack Query hooks (`/src/hooks/useGlobalSearch.ts`)
  ...
  - [x] 92.4 Add SearchCommand to app header/navbar
    - Search icon button + keyboard shortcut hint
    - _Requirements: 99.1_

- [x] 93. Implement Plagiarism Awareness Placeholder
  - [x] 93.1 Apply database migration: add `plagiarism_score` column to `submissions`
  ...
  - [x] 93.2 Create PlagiarismPlaceholder component (`/src/components/shared/PlagiarismPlaceholder.tsx`)
    - Show "Plagiarism check: Not configured" when score is null
    - Show score badge when configured (future)
    - _Requirements: 100.2_

  - [x] 93.3 Add PlagiarismPlaceholder to Grading Interface
    - Display alongside submission details
    - _Requirements: 100.2_

  - [x] 93.4 Document integration points in code comments
...
- [x] 94. Implement Granular In-App Notification Preferences
  - [x] 94.1 Apply database migration: add `notification_preferences` column to `profiles`
  ...
  - [x] 94.2 Create Notification Preferences page (`/src/pages/shared/NotificationPreferences.tsx`)
  ...
  - [x] 94.3 Create notification preferences TanStack Query hooks (`/src/hooks/useNotificationPreferences.ts`)
    - Query current preferences from `profiles.notification_preferences`
    - Mutation for updating preferences
    - _Requirements: 101_

  - [x] 94.4 Update Notification_Service to respect preferences
...
- [x] 95. Implement Session Management UI
  - [x] 95.1 Create Session Management page (`/src/pages/shared/SessionManagement.tsx`)
  ...
  - [x] 95.2 Create session management TanStack Query hooks (`/src/hooks/useSessionManagement.ts`)
    - Query active sessions via Supabase Auth admin API
    - Mutations for signing out other/all sessions
    - _Requirements: 102.5_

  - [x] 95.3 Log session termination to audit_logs
    - Record action with count of terminated sessions
    - _Requirements: 102.6_

- [x] 96. Write tests for production readiness features
  - [x]* 96.1 Write property-based tests (Properties 66–80)
  ...
  - [x]* 96.2 Write unit tests for production readiness modules
  ...
  - [x]* 96.3 Write integration tests for production readiness flows
...
- [x] 97. Implement Sentry Initialization & Global Error Boundary
  - [x] 97.1 Configure Sentry SDK initialization in `src/main.tsx`
  ...
  - [x] 97.2 Create global ErrorBoundary wrapping the app (`src/components/shared/ErrorBoundary.tsx`)
  ...
  - [x] 97.3 Add `VITE_SENTRY_DSN` to `.env.example` and document in README
    - _Requirements: NFR_

  - [x] 97.4 Configure Sentry source map upload in CI pipeline
...
- [x] 98. Implement End-to-End (E2E) Testing with Playwright
  - [x] 98.1 Install and configure Playwright
  ...
  - [x] 98.2 Create E2E test: login → dashboard redirect
  ...
  - [x] 98.3 Create E2E test: assignment creation → submission → grading → evidence chain
  ...
  - [x] 98.4 Create E2E test: XP award → level up → badge check
  ...
  - [x] 98.5 Create E2E test: student enrollment → course visibility
  ...
  - [x] 98.6 Add E2E test step to CI pipeline (GitHub Actions)
    - Run Playwright tests after unit/property tests pass
    - Upload test artifacts (screenshots, traces) on failure
    - _Requirements: NFR_

- [x] 99. Enhance CI/CD Pipeline
  - [x] 99.1 Add Sentry source map upload step to CI
  ...
  - [x] 99.2 Add Lighthouse CI performance budget check
  ...
  - [x] 99.3 Add bundle size tracking to CI
  ...
  - [x] 99.4 Add E2E test step to CI (reference task 98.6)
    - Run Playwright tests in CI with headless browsers
    - _Requirements: NFR_

  - [x] 99.5 Add Supabase migration check to CI
    - Verify `supabase db diff` produces no uncommitted changes
    - Run `supabase db lint` for migration validation
    - _Requirements: NFR_

  - [x] 99.6 Document branch protection rules in `/docs/ci-cd.md`
    - Required checks: lint, typecheck, test, build, e2e, lighthouse, bundle-size
    - Require PR reviews, no direct pushes to main
    - _Requirements: NFR_

- [x] 100. Implement Database Backup & PITR Activation
  - [x] 100.1 Document Supabase PITR activation steps in `/docs/disaster-recovery.md`
  ...
  - [x] 100.2 Create automated monthly backup verification script (`/scripts/verify-backup.sh`)
  ...
  - [x] 100.3 Document backup monitoring alerts
    - Configure Supabase dashboard alerts for backup failures
    - Add backup status check to health monitoring endpoint (task 46)
    - _Requirements: 90_

- [x] 101. Implement Environment & Secrets Management
  - [x] 101.1 Create comprehensive `.env.example` with all required variables
  ...
  - [x] 101.2 Create Edge Function secrets provisioning guide (`/docs/secrets-management.md`)
  ...
  - [x] 101.3 Add startup validation for required environment variables
...
- [x] 102. Implement Performance Budgets & Bundle Analysis
  - [x] 102.1 Configure bundle analysis tooling
  ...
  - [x] 102.2 Implement code splitting for role-specific page modules
  ...
  - [x] 102.3 Create performance budget configuration (`/lighthouserc.js`)
  ...
  - [x] 102.4 Add performance budget check to CI (reference task 99.2)
    - Fail build if any budget is exceeded
    - _Requirements: NFR_

- [x] 103. Implement Automated Accessibility Testing
  - [x] 103.1 Integrate `@axe-core/react` in development mode
  ...
  - [x] 103.2 Add `vitest-axe` for component-level a11y assertions
  ...
  - [x] 103.3 Add Lighthouse accessibility score ≥ 90 to CI budget
    - Include in `lighthouserc.js` assertions (reference task 102.3)
    - _Requirements: NFR_

  - [x]* 103.4 Write a11y-focused unit tests for critical interactive components
...
- [x] 104. Create Local Development Seed Script
  - [x] 104.1 Create `supabase/seed.sql` for rapid local development onboarding
  ...
  - [x] 104.2 Add npm script for seeding: `"seed": "supabase db reset"`
    - Document in README: `npm run seed` for fresh local environment
    - _Requirements: NFR_

- [x] 105. Implement Client-Side Request Throttling
  - [x] 105.1 Configure TanStack Query global defaults (`/src/providers/QueryProvider.tsx`)
  ...
  - [x] 105.2 Add global error handler for HTTP 429 responses
  ...
  - [x] 105.3 Configure request deduplication for concurrent identical queries
...
- [x] 106. Implement Security Headers in vercel.json
mak  - [x] 106.1 Add comprehensive security headers configuration to `vercel.json`
  ...
  - [x] 106.2 Add CDN caching headers for static assets in `vercel.json`
    - `Cache-Control: public, max-age=31536000, immutable` for `/assets/**`
    - `Cache-Control: no-cache` for `index.html` and service worker
    - _Requirements: 98.4_

- [x] 107. Implement Mobile Responsiveness Testing Strategy
  - [x] 107.1 Document responsive breakpoints and testing strategy (`/docs/responsive-testing.md`)
  ...
  - [x] 107.2 Verify viewport meta tag in `index.html`
    - Ensure `<meta name="viewport" content="width=device-width, initial-scale=1.0">` is present
    - _Requirements: NFR_

  - [x] 107.3 Add responsive viewport screenshots to E2E tests
  ...
  - [x]* 107.4 Write responsive layout assertions in E2E tests
...
- [x] 108. Implement Data Migration Tooling from Existing LMS
  - [x] 108.1 Create CSV import templates (`/docs/import-templates/`)
  ...
  - [x] 108.2 Create `bulk-data-import` Edge Function (`/supabase/functions/bulk-data-import/`)
  ...
  - [x] 108.3 Create Data Import UI (`/src/pages/admin/import/DataImportPage.tsx`)
  ...
  - [x] 108.4 Create data import TanStack Query hooks (`/src/hooks/useDataImport.ts`)
    - Mutation for uploading and processing CSV imports
    - Query for import history/status
    - _Requirements: NFR_


- [x] 109. OBE Engine — Sub-CLO Database Migration and Core Logic
  - [x] 109.1 Apply Sub-CLO database migration via Supabase MCP `apply_migration`
  ...
  - [x] 109.2 Create Sub-CLO Zod schemas (`/src/lib/schemas/subCLO.ts`)
  ...
  - [x] 109.3 Create Sub-CLO TanStack Query hooks (`/src/hooks/useSubCLOs.ts`)
  ...
  - [x] 109.4 Implement Sub-CLO weighted rollup logic in `calculate-attainment-rollup` Edge Function
  ...
  - [x]* 109.5 Write property tests for Sub-CLO weight sum constraint (`/src/__tests__/properties/sub-clo.property.test.ts`)
    - **Property 81: Sub-CLO weight sum constraint**
    - **Validates: Requirements 103.1, 103.2**

  - [x]* 109.6 Write property tests for Sub-CLO weighted rollup accuracy
    - **Property 82: Sub-CLO weighted rollup accuracy**
    - **Validates: Requirements 103.4**

  - [x]* 109.7 Write property test for Sub-CLO deletion protection
    - **Property 83: Sub-CLO deletion protection**
    - **Validates: Requirements 103.5**


- [x] 110. OBE Engine — Sub-CLO UI Components
  - [x] 110.1 Create SubCLOManager page component (`/src/pages/teacher/outcomes/SubCLOManager.tsx`)
  ...
  - [x] 110.2 Create SubCLORow shared component (`/src/components/shared/SubCLORow.tsx`)
  ...
  - [x] 110.3 Update CLO list views to display Sub-CLOs as expandable children
  ...
  - [x] 110.4 Update RubricBuilder to support Sub-CLO linkage
...
- [x] 111. Checkpoint — Sub-CLO Implementation
  - Ensure all tests pass, ask the user if questions arise.


- [x] 112. OBE Engine — Graduate Attribute Database and Core Logic
  - [x] 112.1 Apply Graduate Attribute database migration via Supabase MCP `apply_migration`
  ...
  - [x] 112.2 Create Graduate Attribute Zod schemas (`/src/lib/schemas/graduateAttribute.ts`)
  ...
  - [x] 112.3 Create Graduate Attribute TanStack Query hooks (`/src/hooks/useGraduateAttributes.ts`)
  ...
  - [x] 112.4 Implement GA weighted rollup in `calculate-attainment-rollup` Edge Function
  ...
  - [x]* 112.5 Write property tests for Graduate Attribute rollup (`/src/__tests__/properties/graduate-attributes.property.test.ts`)
    - **Property 84: Graduate Attribute weighted rollup accuracy**
    - **Validates: Requirements 104.2, 104.3**

  - [x]* 112.6 Write property test for Graduate Attribute audit logging
    - **Property 85: Graduate Attribute audit logging**
    - **Validates: Requirements 104.6**


- [x] 113. OBE Engine — Graduate Attribute UI Components
  - [x] 113.1 Create GraduateAttributeManager page (`/src/pages/admin/graduate-attributes/GraduateAttributeManager.tsx`)
  ...
  - [x] 113.2 Create GraduateAttributeCard shared component (`/src/components/shared/GraduateAttributeCard.tsx`)
  ...
  - [x] 113.3 Add GA attainment overview card to Admin Dashboard
    - Institution-wide attainment percentages per attribute
    - Uses GraduateAttributeCard in a grid layout
    - _Requirements: 104.5_

  - [x] 113.4 Update Report Generator Edge Function to include GA summary in accreditation PDF
    - Add Graduate Attribute attainment summary section to PDF report
    - _Requirements: 104.4_

- [x] 114. OBE Engine — Competency Framework Database and Core Logic
  - [x] 114.1 Apply Competency Framework database migration via Supabase MCP `apply_migration`
  ...
  - [x] 114.2 Create Competency Framework Zod schemas (`/src/lib/schemas/competencyFramework.ts`)
  ...
  - [x] 114.3 Create Competency Framework TanStack Query hooks (`/src/hooks/useCompetencyFrameworks.ts`)
  ...
  - [x] 114.4 Create CSV import Edge Function for competency frameworks (`/supabase/functions/import-competency-csv/`)
  ...
  - [x]* 114.5 Write property tests for competency hierarchy (`/src/__tests__/properties/competency-frameworks.property.test.ts`)
    - **Property 86: Competency hierarchy level consistency**
    - **Validates: Requirements 105.1**

  - [x]* 114.6 Write property test for CSV import round-trip
    - **Property 87: Competency CSV import round-trip**
    - **Validates: Requirements 105.4**

  - [x]* 114.7 Write property test for unmapped indicator flagging
    - **Property 88: Unmapped competency indicator flagging**
    - **Validates: Requirements 105.6**


- [x] 115. OBE Engine — Competency Framework UI Components
  - [x] 115.1 Create CompetencyFrameworkManager page (`/src/pages/admin/competency-frameworks/CompetencyFrameworkManager.tsx`)
  ...
  - [x] 115.2 Create CompetencyTree shared component (`/src/components/shared/CompetencyTree.tsx`)
  ...
  - [x] 115.3 Create CompetencyAlignmentMatrix shared component (`/src/components/shared/CompetencyAlignmentMatrix.tsx`)
  ...
  - [x] 115.4 Create mapping interface for competency indicators to outcomes
  ...
  - [x] 115.5 Update Report Generator to produce competency alignment matrix report
...
- [x] 116. Checkpoint — Graduate Attributes and Competency Frameworks
  - Ensure all tests pass, ask the user if questions arise.


- [x] 117. OBE Engine — Visualization Data Hooks and Utilities
  - [x] 117.1 Create Sankey data transformation utility (`/src/lib/sankeyTransform.ts`)
  ...
  - [x] 117.2 Create Gap Analysis classification utility (`/src/lib/gapAnalysis.ts`)
  ...
  - [x] 117.3 Create Coverage Heatmap data utility (`/src/lib/coverageHeatmap.ts`)
  ...
  - [x] 117.4 Create visualization TanStack Query hooks (`/src/hooks/useVisualizationData.ts`)
  ...
  - [x] 117.5 Create Zod filter schemas for visualizations (`/src/lib/schemas/`)
  ...
  - [x]* 117.6 Write property test for Sankey data transformation (`/src/__tests__/properties/sankey-diagram.property.test.ts`)
    - **Property 89: Sankey data transformation correctness**
    - **Validates: Requirements 106.1, 106.4**

  - [x]* 117.7 Write property test for Gap status classification (`/src/__tests__/properties/gap-analysis.property.test.ts`)
    - **Property 90: Gap status classification correctness**
    - **Validates: Requirements 107.1, 107.2, 107.3**

  - [x]* 117.8 Write property test for Coverage heatmap data integrity (`/src/__tests__/properties/coverage-heatmap.property.test.ts`)
    - **Property 91: Coverage heatmap data integrity**
    - **Validates: Requirements 108.1, 108.2**


- [x] 118. OBE Engine — Sankey Diagram UI
  - [x] 118.1 Create SankeyChart shared component (`/src/components/shared/SankeyChart.tsx`)
  ...
  - [x] 118.2 Create SankeyDiagramView page (`/src/pages/coordinator/sankey/SankeyDiagramView.tsx`)
  ...
  - [x] 118.3 Add Sankey diagram route and navigation
...
- [x] 119. OBE Engine — Gap Analysis UI
  - [x] 119.1 Create GapStatusBadge shared component (`/src/components/shared/GapStatusBadge.tsx`)
  ...
  - [x] 119.2 Create GapAnalysisView page (`/src/pages/coordinator/gap-analysis/GapAnalysisView.tsx`)
  ...
  - [x] 119.3 Add PDF export for Gap Analysis
  ...
  - [x] 119.4 Add Gap Analysis route and navigation
    - Add route `/coordinator/gap-analysis` to AppRouter
    - Add navigation link in Coordinator sidebar
    - _Requirements: 107.1_


- [x] 120. OBE Engine — Coverage Heatmap UI
  - [x] 120.1 Create HeatmapGrid shared component (`/src/components/shared/HeatmapGrid.tsx`)
  ...
  - [x] 120.2 Create CoverageHeatmapView page (`/src/pages/coordinator/coverage-heatmap/CoverageHeatmapView.tsx`)
  ...
  - [x] 120.3 Add Coverage Heatmap route and navigation
    - Add route `/coordinator/coverage-heatmap` to AppRouter
    - Add navigation link in Coordinator sidebar
    - _Requirements: 108.1_

- [x] 121. Checkpoint — OBE Visualizations (Sankey, Gap Analysis, Heatmap)
  - Ensure all tests pass, ask the user if questions arise.


- [x] 122. OBE Engine — Semester Trends Database and Core Logic
  - [x] 122.1 Apply semester attainment snapshots migration via Supabase MCP `apply_migration`
  ...
  - [x] 122.2 Create semester trend Zod schemas and hooks
  ...
  - [x] 122.3 Create declining trend detection utility (`/src/lib/trendDetection.ts`)
  ...
  - [x]* 122.4 Write property tests for semester trends (`/src/__tests__/properties/semester-trends.property.test.ts`)
    - **Property 92: Semester attainment snapshot completeness**
    - **Validates: Requirements 109.1**

  - [x]* 122.5 Write property test for declining trend detection
    - **Property 93: Declining trend detection**
    - **Validates: Requirements 109.3**

- [x] 123. OBE Engine — Semester Trend UI
  - [x] 123.1 Create TrendLineChart shared component (`/src/components/shared/TrendLineChart.tsx`)
  ...
  - [x] 123.2 Create DecliningTrendBadge shared component (`/src/components/shared/DecliningTrendBadge.tsx`)
    - Warning badge for ≥10pp attainment decline
    - Red/amber styling with warning icon
    - _Requirements: 109.3_

  - [x] 123.3 Create SemesterTrendView page (`/src/pages/coordinator/trends/SemesterTrendView.tsx`)
  ...
  - [x] 123.4 Add Semester Trend route and navigation
    - Add route `/coordinator/trends` to AppRouter
    - Add navigation link in Coordinator sidebar
    - _Requirements: 109.2_


- [x] 124. OBE Engine — Cohort Comparison Analytics
  - [x] 124.1 Create cohort comparison Zod schemas and hooks
  ...
  - [x] 124.2 Create Cohen's d effect size utility (`/src/lib/cohortStats.ts`)
  ...
  - [x] 124.3 Create CohortBarChart shared component (`/src/components/shared/CohortBarChart.tsx`)
  ...
  - [x] 124.4 Create SignificantGapLabel shared component (`/src/components/shared/SignificantGapLabel.tsx`)
    - Red "Significant Gap" label with warning styling
    - _Requirements: 110.5_

  - [x] 124.5 Create CohortComparisonView page (`/src/pages/coordinator/cohort-comparison/CohortComparisonView.tsx`)
  ...
  - [x] 124.6 Add Cohort Comparison route and navigation
  ...
  - [x]* 124.7 Write property tests for cohort comparison (`/src/__tests__/properties/cohort-comparison.property.test.ts`)
    - **Property 94: Cohort comparison average attainment and gap detection**
    - **Validates: Requirements 110.2, 110.5**

  - [x]* 124.8 Write property test for Cohen's d calculation
    - **Property 95: Cohen's d effect size calculation**
    - **Validates: Requirements 110.3**


- [x] 125. OBE Engine — Historical Evidence Analysis
  - [x] 125.1 Apply historical evidence materialized view migration via Supabase MCP `apply_migration`
  ...
  - [x] 125.2 Create historical evidence Zod schemas and hooks
  ...
  - [x] 125.3 Create StackedAreaChart shared component (`/src/components/shared/StackedAreaChart.tsx`)
  ...
  - [x] 125.4 Create HistoricalEvidenceDashboard page (`/src/pages/admin/historical-evidence/HistoricalEvidenceDashboard.tsx`)
  ...
  - [x] 125.5 Create "Continuous Improvement Report" PDF generation
  ...
  - [x] 125.6 Add Historical Evidence route and navigation
    - Add route `/admin/historical-evidence` to AppRouter
    - Add navigation link in Admin sidebar
    - _Requirements: 111.1_

  - [x]* 125.7 Write property test for historical evidence distribution (`/src/__tests__/properties/historical-evidence.property.test.ts`)
    - **Property 96: Historical evidence attainment distribution**
    - **Validates: Requirements 111.1, 111.2**

- [x] 126. Checkpoint — Semester Trends, Cohort Comparison, Historical Evidence
  - Ensure all tests pass, ask the user if questions arise.


- [x] 127. Habit Engine — Extended Habit Types Database and Core Logic
  - [x] 127.1 Apply extended habit types migration via Supabase MCP `apply_migration`
  ...
  - [x] 127.2 Update shared type definitions (`/src/types/app.ts`)
    - Update `HabitType` to include all 8 types: `'login' | 'submit' | 'journal' | 'read' | 'collaborate' | 'practice' | 'review' | 'mentor'`
    - _Requirements: 112.1_

  - [x] 127.3 Implement extended habit completion triggers
  ...
  - [x] 127.4 Update Perfect Day logic
  ...
  - [x] 127.5 Update HabitTracker component (`/src/components/shared/HabitTracker.tsx`)
  ...
  - [x]* 127.6 Write property tests for extended habits (`/src/__tests__/properties/extended-habits.property.test.ts`)
    - **Property 97: Extended habit type validation**
    - **Validates: Requirements 112.1**

  - [x]* 127.7 Write property test for extended habit completion triggers
    - **Property 98: Extended habit completion triggers**
    - **Validates: Requirements 112.2, 112.3, 112.4, 112.5, 112.6**

  - [x]* 127.8 Write property test for updated Perfect Day threshold
    - **Property 99: Updated Perfect Day threshold**
    - **Validates: Requirements 112.8**


- [x] 128. Habit Engine — Team Management Database and Core Logic
  - [x] 128.1 Apply teams database migration via Supabase MCP `apply_migration`
  ...
  - [x] 128.2 Create Team Zod schemas (`/src/lib/schemas/team.ts`)
  ...
  - [x] 128.3 Create Team TanStack Query hooks (`/src/hooks/useTeams.ts`)
  ...
  - [x] 128.4 Implement auto-generate balanced teams logic
  ...
  - [x]* 128.5 Write property tests for team membership constraints (`/src/__tests__/properties/teams.property.test.ts`)
    - **Property 100: Team membership constraints**
    - **Validates: Requirements 115.2, 115.3**

  - [x]* 128.6 Write property test for auto-generated team balance
    - **Property 101: Auto-generated team balance**
    - **Validates: Requirements 115.4**


- [x] 129. Habit Engine — Team Management UI
  - [x] 129.1 Create TeamManager page (`/src/pages/teacher/teams/TeamManager.tsx`)
  ...
  - [x] 129.2 Create TeamDashboardCard shared component (`/src/components/shared/TeamDashboardCard.tsx`)
  ...
  - [x] 129.3 Add team display to Student Dashboard course card
    - Show team name and member avatars on enrolled course cards
    - _Requirements: 115.6_

  - [x] 129.4 Add Team Manager route and navigation
    - Add route `/teacher/teams` to AppRouter
    - Add navigation link in Teacher sidebar
    - _Requirements: 115.1_

- [x] 130. Checkpoint — Extended Habits and Team Management
  - Ensure all tests pass, ask the user if questions arise.


- [x] 131. Habit Engine — Team XP Pool and Leaderboard
  - [x] 131.1 Update `award-xp` Edge Function for team XP contribution
  ...
  - [x] 131.2 Apply xp_transactions column additions via Supabase MCP `apply_migration`
  ...
  - [x] 131.3 Create Team Leaderboard TanStack Query hooks (`/src/hooks/useTeamLeaderboard.ts`)
  ...
  - [x] 131.4 Create TeamLeaderboard page (`/src/pages/student/leaderboard/TeamLeaderboard.tsx`)
  ...
  - [x] 131.5 Add Team Leaderboard tab to existing LeaderboardView
  ...
  - [x]* 131.6 Write property test for Team XP split (`/src/__tests__/properties/team-xp.property.test.ts`)
    - **Property 102: Team XP split correctness**
    - **Validates: Requirements 116.1, 116.4**

  - [x]* 131.7 Write property test for Team Leaderboard ordering (`/src/__tests__/properties/team-leaderboard.property.test.ts`)
    - **Property 103: Team leaderboard ordering and completeness**
    - **Validates: Requirements 117.1, 117.2**


- [x] 132. Habit Engine — Team Badges and Streaks
  - [x] 132.1 Apply badges table column additions via Supabase MCP `apply_migration`
  ...
  - [x] 132.2 Update `check-badges` Edge Function for team badges
  ...
  - [x] 132.3 Create TeamBadgeDisplay shared component (`/src/components/shared/TeamBadgeDisplay.tsx`)
  ...
  - [x] 132.4 Update `process-streak` Edge Function for team streaks
  ...
  - [x] 132.5 Create `team-streak-risk-cron` Edge Function
  ...
  - [x] 132.6 Add team streak display to TeamDashboardCard
  ...
  - [x]* 132.7 Write property test for team badge idempotency (`/src/__tests__/properties/team-badges.property.test.ts`)
    - **Property 104: Team badge idempotency**
    - **Validates: Requirements 118.2, 118.5**

  - [x]* 132.8 Write property tests for team streak calculation (`/src/__tests__/properties/team-streaks.property.test.ts`)
    - **Property 105: Team streak calculation**
    - **Validates: Requirements 119.1, 119.2**

  - [x]* 132.9 Write property test for team streak milestone rewards
    - **Property 106: Team streak milestone rewards**
    - **Validates: Requirements 119.5**

- [x] 133. Checkpoint — Team XP, Leaderboard, Badges, and Streaks
  - Ensure all tests pass, ask the user if questions arise.


- [x] 134. Habit Engine — Social Challenges Database and Core Logic
  - [x] 134.1 Apply social challenges database migration via Supabase MCP `apply_migration`
  ...
  - [x] 134.2 Create Challenge Zod schemas (`/src/lib/schemas/challenge.ts`)
  ...
  - [x] 134.3 Create Challenge TanStack Query hooks (`/src/hooks/useChallenges.ts`)
  ...
  - [x] 134.4 Create `challenge-progress-update` Edge Function
  ...
  - [x] 134.5 Create `challenge-completion` Edge Function (pg_cron daily)
  ...
  - [x] 134.6 Implement 90% notification trigger for course-wide challenges
...
- [x] 135. Habit Engine — Social Challenges UI
  - [x] 135.1 Create ChallengeManager page (`/src/pages/teacher/challenges/ChallengeManager.tsx`)
  ...
  - [x] 135.2 Create ChallengeListView page (`/src/pages/student/challenges/ChallengeListView.tsx`)
...
- [x] 136. Checkpoint — Social Challenges
  - Ensure all tests pass, ask the user if questions arise.


- [x] 137. Habit Engine — Adaptive XP Engine Core Logic
  - [x] 137.1 Create Adaptive XP calculation utility (`/src/lib/adaptiveXP.ts`)
  ...
  - [x] 137.2 Create Adaptive XP Zod schemas (`/src/lib/schemas/adaptiveXP.ts`)
  ...
  - [x] 137.3 Update `award-xp` Edge Function with adaptive XP calculation
  ...
  - [x] 137.4 Create diminishing returns tracking hooks (`/src/hooks/useAdaptiveXP.ts`)
...
- [x] 138. Habit Engine — Adaptive XP UI Components
  - [x] 138.1 Create AdaptiveXPDisplay shared component (`/src/components/shared/AdaptiveXPDisplay.tsx`)
  ...
  - [x] 138.2 Add difficulty bonus display to assignment detail page
  ...
  - [x] 138.3 Integrate AdaptiveXPDisplay into Student Dashboard
    - Add multiplier display to existing gamification card
    - Show diminishing returns warning when applicable
    - _Requirements: 120.5, 122.3_

- [x] 139. Habit Engine — Improvement Bonus
  - [x] 139.1 Create `improvement-bonus-check` Edge Function
  ...
  - [x] 139.2 Create Improvement Bonus Zod schema (`/src/lib/schemas/improvementBonus.ts`)
    - `improvementBonusSchema`: validates clo_id, previous_percent, current_percent, bonus_xp
    - _Requirements: 123.1_

  - [x] 139.3 Update `check-badges` Edge Function for Comeback Kid badge
  ...
  - [x] 139.4 Create ImprovementBonusCelebration shared component (`/src/components/shared/ImprovementBonusCelebration.tsx`)
...
- [x] 140. Checkpoint — Adaptive XP Engine and Improvement Bonus
  - Ensure all tests pass, ask the user if questions arise.


- [x] 141. Integration — Wire OBE Enhancements into Existing Dashboards
  - [x] 141.1 Update Coordinator Dashboard with OBE visualization links
  ...
  - [x] 141.2 Update Admin Dashboard with new sections
  ...
  - [x] 141.3 Update Student Dashboard with team and challenge sections
  ...
  - [x] 141.4 Update Teacher Dashboard with team and challenge management links
    - Add Team Manager link in sidebar
    - Add Challenge Manager link in sidebar
    - _Requirements: 113.1, 115.1_

  - [x] 141.5 Update AppRouter with all new routes
  ...
  - [x] 141.6 Update TanStack Query key factory with all new keys
...
- [x] 142. Final Checkpoint — Sections U & V Complete
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 143. Streak Recovery — Comeback Challenge Database and Core Logic
  - [x] 143.1 Apply Comeback Challenge database migration via Supabase MCP `apply_migration`
  ...
  - [x] 143.2 Create Comeback Challenge Zod schemas (`/src/lib/schemas/comebackChallenge.ts`)
    - `comebackChallengeStateSchema`: validates is_active, days_completed (0–3), streak_to_restore
    - _Requirements: 124.1_

  - [x] 143.3 Update `process-streak` Edge Function for Comeback Challenge logic
  ...
  - [x] 143.4 Create Comeback Challenge TanStack Query hooks (`/src/hooks/useComebackChallenge.ts`)
...
- [x] 144. Streak Recovery — Streak Sabbatical and Range Display
  - [x] 144.1 Update `institution_settings` schema to include `streak_sabbatical_enabled` boolean (default false)
  ...
  - [x] 144.2 Update `process-streak` Edge Function for Streak Sabbatical
  ...
  - [x] 144.3 Update StreakDisplay component (`/src/components/shared/StreakDisplay.tsx`)
  ...
  - [x] 144.4 Create ComebackChallengeBanner component (`/src/components/shared/ComebackChallengeBanner.tsx`)
  ...
  - [x] 144.5 Wire Comeback Challenge banner and updated StreakDisplay into Student Dashboard
    - _Requirements: 124.5, 126.1, 126.2_

  - [x]* 144.6 Write property test for Streak Sabbatical weekend exclusion (`/src/__tests__/properties/streak-sabbatical.property.test.ts`)
    - **Property 102: Streak Sabbatical weekend exclusion**
    - **Validates: Requirements 125.1, 125.2**


- [x] 145. Habit Difficulty Levels — Database and Core Logic
  - [x] 145.1 Apply Habit Difficulty Level database migration via Supabase MCP `apply_migration`
  ...
  - [x] 145.2 Create Habit Difficulty Level Zod schema (`/src/lib/schemas/habitDifficulty.ts`)
    - `habitDifficultyLevelSchema`: validates level (1, 2, 3), habit_level_streak
    - _Requirements: 127.1_

  - [x] 145.3 Create Habit Difficulty Level utility (`/src/lib/habitDifficulty.ts`)
  ...
  - [x] 145.4 Update `process-streak` Edge Function for Habit Difficulty Level tracking
  ...
  - [x] 145.5 Update `perfect-day-nudge-cron` Edge Function for level-relative thresholds
  ...
  - [x] 145.6 Create HabitDifficultyIndicator component (`/src/components/shared/HabitDifficultyIndicator.tsx`)
    - Displays current level with progress toward next level
    - Level icons: Seedling (1), Sprout (2), Tree (3)
    - _Requirements: 127.6_

  - [x] 145.7 Create Habit Difficulty Level TanStack Query hook (`/src/hooks/useHabitDifficulty.ts`)
    - `useHabitDifficultyLevel(studentId)`: fetches current level and streak
    - Add query keys to `/src/lib/queryKeys.ts`
    - _Requirements: 127.4_

  - [x] 145.8 Wire HabitDifficultyIndicator into Student Dashboard
    - Display below or alongside HabitTracker component
    - _Requirements: 127.6_

  - [x]* 145.9 Write property tests for Habit Difficulty Level (`/src/__tests__/properties/habit-difficulty.property.test.ts`)
...
- [x] 146. Checkpoint — Streak Recovery and Habit Difficulty Levels
  - Ensure all tests pass, ask the user if questions arise.


- [x] 147. Leaderboard Enhancements — Personal Best and Most Improved
  - [x] 147.1 Create Personal Best leaderboard utility (`/src/lib/personalBestLeaderboard.ts`)
  ...
  - [x] 147.2 Create Most Improved leaderboard utility (`/src/lib/mostImprovedLeaderboard.ts`)
  ...
  - [x] 147.3 Create Personal Best and Most Improved TanStack Query hooks
  ...
  - [x] 147.4 Update LeaderboardPage with Personal Best tab
  ...
  - [x] 147.5 Update LeaderboardPage with Most Improved tab
  ...
  - [x] 147.6 Update `check-badges` Edge Function for Rising Star badge
...
- [x] 148. Leaderboard Enhancements — Percentile Bands and League Tiers
  - [x] 148.1 Create Percentile Band utility (`/src/lib/percentileBand.ts`)
  ...
  - [x] 148.2 Create League Tier utility (`/src/lib/leagueTier.ts`)
  ...
  - [x] 148.3 Update `institution_settings` schema to include `league_thresholds` configuration
  ...
  - [x] 148.4 Create League Tier TanStack Query hooks
  ...
  - [x] 148.5 Update LeaderboardPage with Percentile Bands in Top XP mode
...
- [x] 149. Checkpoint — Leaderboard Enhancements
  - Ensure all tests pass, ask the user if questions arise.
...
- [x] 155. Final Checkpoint — Section W Complete
  - Ensure all tests pass, ask the user if questions arise.

...
| Q2 Why is it failing? | Systemic-outcome-gap pattern contracts (lib + migration) — 0 pattern rows; no root-cause model | L3 arch; L0 operational | No root-cause taxonomy (student/teacher/assessment/prereq/curriculum); no diagnostics | Problem-taxonomy engine + AI explanation from authorized evidence only | 8.9 |
...
| Q5 Who performs it? | CQI `responsible_person` free-text field only | L1 | No role-based ownership routing | Ownership assignment in 8.9 decision engine | 8.9 |

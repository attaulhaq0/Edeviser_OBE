...
export function validatePurchase(
  ...
  const { balance, studentLevel, item, alreadyOwned, currentStreakFreezes, effectivePrice } = input;
  ...
  if (item.stock_type === 'limited' && (item.stock_quantity === null || item.stock_quantity <= 0)) {

import type { HeatmapDay, WellnessHabitType, DayOfWeekData } from '@/types/habits';
...
export const computeConsistencyScore = (days: HeatmapDay[], totalDays: number): number => {
...
export const computeDayOfWeekAverages = (days: HeatmapDay[]): DayOfWeekData[] => {
  ...
  return dayNames.map((name, i) => ({
    day: name,
    avgCompletions: (counts[i] ?? 0) > 0 ? Math.round(((sums[i] ?? 0) / (counts[i] ?? 1)) * 100) / 100 : 0,
  }));
};
...
export const computeCellSize = (containerWidth: number, numWeeks: number): number => {
...
export const generateMonthLabels = (
  ...
  while (cursor <= end) {
    ...
    if (month !== currentMonth) {
      currentMonth = month;
      const daysSinceStart = Math.floor((cursor.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
...
export const getFilterOptions = (enabledWellnessHabits: WellnessHabitType[]): string[] => {

...
export const buildHeatmapMatrix = (
  ...
  clos.forEach((c) => { clo_labels[c.id] = c.title; });
  courses.forEach((c) => { course_labels[c.id] = c.name; });
  ...
  for (const e of evidence) {
    ...
    if (existing) {
      ...
      existing.avg_attainment = ((existing.avg_attainment * existing.evidence_count) + e.score_percent) / newCount;
      existing.evidence_count = newCount;
    } else {
      cells.set(key, { clo_id: e.clo_id, course_id: e.course_id, evidence_count: 1, avg_attainment: e.score_percent });
    }
  }

...
export const classifyGapStatus = (outcome: OutcomeWithMappings): GapStatus => {
  if (outcome.mapped_children_count === 0 && outcome.type !== "CLO")
    return "unmapped";
  if (outcome.evidence_count === 0) return "no_evidence";
  if (outcome.type === "PLO" && outcome.mapped_children_count < 2)
...
export const generateRecommendation = (
  ...
  if (status === "unmapped")
    return `Map this ${type} to child outcomes to establish assessment coverage.`;
  if (flag === "under_mapped")
    return "Add more CLO mappings to ensure adequate coverage (minimum 2 recommended).";
  if (flag === "unassessed")
    return "Link at least one assessment to this CLO in the current semester.";
  if (status === "no_evidence")
    return "No student evidence exists yet. Ensure assessments are graded.";
  return null;
};

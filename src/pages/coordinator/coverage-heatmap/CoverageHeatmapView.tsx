...
import { getEvidenceCountColor, getAttainmentColor } from '@/lib/coverageHeatmap';
...
const CoverageHeatmapView = () => {
  ...
  const { data: matrix, isLoading } = useCoverageHeatmap(programId || undefined);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-1">
            <Button size="sm" variant={colorMode === 'evidence' ? 'default' : 'outline'} onClick={() => setColorMode('evidence')} className="text-xs">Evidence Count</Button>
            <Button size="sm" variant={colorMode === 'attainment' ? 'default' : 'outline'} onClick={() => setColorMode('attainment')} className="text-xs">Attainment %</Button>
          </div>
      ...
      <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
        <div className="px-6 py-4 flex items-center gap-2" style={{ background: 'linear-gradient(93.65deg, #14B8A6 5.37%, #0382BD 78.89%)' }}>
          ...
          <h2 className="text-lg font-bold tracking-tight text-white">CLO × Course Matrix</h2>
        </div>
        <div className="p-4 overflow-x-auto">
          {!programId ? (
            <p className="text-sm text-slate-400 text-center py-12">Select a program to view coverage.</p>
          ) : isLoading ? (
                  ...
                  <tr key={cloId}>
                    ...
                    {matrix.course_ids.map((courseId) => {
                      ...
                      const bgColor = cell
                        ...
                        : '#ffffff';
                      return (
                        <td key={courseId} className="p-2 border border-slate-200 text-center" style={{ backgroundColor: bgColor }}>
                          {cell ? (
                            colorMode === 'evidence' ? cell.evidence_count : `${Math.round(cell.avg_attainment)}%`
                          ) : (

...
import { Award, BookOpen, PenLine, TrendingUp, BarChart3, Copy, Check } from 'lucide-react';
...
export default function StudentPortfolio() {
  ...
  const handleTogglePublic = () => {
    ...
    toggleMutation.mutate(
      ...
      {
        onSuccess: () => toast.success(isPublic ? 'Portfolio set to private' : 'Portfolio is now public'),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Switch id="portfolio-public" checked={isPublic} onCheckedChange={handleTogglePublic} disabled={toggleMutation.isPending} aria-label="Toggle public portfolio" />
            <Label htmlFor="portfolio-public" className="text-sm font-medium text-gray-600">Public Profile</Label>
          </div>
          {isPublic && (
            <button onClick={handleCopyLink} className="flex items-center gap-1.5 text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors" aria-label="Copy shareable link">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard icon={TrendingUp} label="Total XP" value={(data?.totalXP ?? 0).toLocaleString()} />
        <KPICard icon={Award} label="Level" value={data?.level ?? 1} />
        <KPICard icon={BookOpen} label="CLOs Mastered" value={data?.clos.length ?? 0} />
        <KPICard icon={Award} label="Badges Earned" value={data?.badges.length ?? 0} />
      </div>

      <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
        <GradientCardHeader icon={BookOpen} title="CLO Mastery" />
        <div className="p-6 space-y-6">
          {closByCourse.length === 0 && <p className="text-sm text-gray-400">No CLO attainment data yet.</p>}
          {closByCourse.map(([courseName, clos]) => (
            <div key={courseName}>
              <h3 className="text-sm font-bold text-gray-700 mb-3">{courseName}</h3>
              <div className="space-y-2">
                {clos.map((clo) => (
                  <div key={clo.clo_id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-100 p-3">
                    <div className="flex items-center gap-2 min-w-0">
                      <BloomsPill level={clo.blooms_level} />
                      <span className="text-sm font-medium text-gray-800 truncate">{clo.clo_title}</span>
                    </div>
                    <span className={cn('inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-bold shrink-0', ATTAINMENT_STYLES[clo.attainment_level])}>
                      {clo.attainment_level.replace('_', ' ')} ({clo.attainment_percent}%)
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="p-6">
          {(data?.journals ?? []).length === 0 ? (
            ...
            <div className="space-y-3">
              {data?.journals.map((j) => (
                <div key={j.id} className="flex items-center justify-between rounded-lg border border-slate-100 p-3">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-800 truncate">{j.content_preview}</p>
                    <p className="text-xs text-gray-500">{j.course_name ?? 'General'}</p>
                  </div>
                  <span className="text-xs text-gray-400 shrink-0">{format(new Date(j.created_at), 'MMM d, yyyy')}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {(data?.xpTimeline ?? []).length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={data?.xpTimeline}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#64748b' }} tickFormatter={(v) => format(new Date(v as string), 'MMM d')} />
                <YAxis tick={{ fontSize: 11, fill: '#64748b' }} />
                <Tooltip labelFormatter={(v) => format(new Date(v as string), 'MMM d, yyyy')} formatter={(value) => [`${Number(value).toLocaleString()} XP`, 'Cumulative XP']} />
                <Line type="monotone" dataKey="cumulative_xp" stroke="#3b82f6" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {(data?.semesterAttainments ?? []).length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={data?.semesterAttainments}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis dataKey="semester_name" tick={{ fontSize: 11, fill: '#64748b' }} />
                <YAxis domain={[0, 100]} tick={{ fontSize: 11, fill: '#64748b' }} tickFormatter={(v) => `${v}%`} />
                <Tooltip formatter={(value) => [`${value}%`, 'Avg Attainment']} />
                <Bar dataKey="average_attainment" fill="#14b8a6" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>

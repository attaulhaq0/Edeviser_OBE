...
const AssignmentDetailPage = () => {
  ...
  const { data: paginatedSubmissions, isLoading: submissionsLoading } =
    useSubmissions({
      assignmentId: id,
    });
  ...
  const mySubmission = submissions?.find((s) => s.profiles?.id === profile?.id);
  ...
  const handleFileChange = (file: File | undefined) => {
    ...
    try {
      ...
      draftManager.saveDraft(draftKey, {
        fileName: file.name,
        fileSize: file.size,
      });
    } catch (err) {
      ...
      try {
        ...
        if (isNetworkError && retries < MAX_UPLOAD_RETRIES) {
          ...
          if (!navigator.onLine) {
            ...
            toast.info(
              `Upload failed — retrying (${retries + 1}/${MAX_UPLOAD_RETRIES})…`
            );
              ...
              {deadlineStatus?.window === "late_window" && (
                <div className="flex items-center gap-2 p-3 rounded-lg bg-orange-50 border border-orange-200">
                  ...
                  <p className="text-xs text-orange-700">
                    The deadline has passed. Submitting now will be marked as
                    late.
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                {draftHint && !selectedFile && (
                  <p className="text-xs text-amber-600 mt-1">
                    {draftHint} — please re-select your file
                  </p>
                )}

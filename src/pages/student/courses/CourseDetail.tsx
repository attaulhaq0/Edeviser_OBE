...
import { useCourseModules, useCourseMaterials, type CourseModule, type CourseMaterial } from '@/hooks/useCourseModules';
...
const ModuleSection = ({ module: mod, onMaterialClick }: ModuleSectionProps) => {
    ...
    <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
      ...
      >
        ...
        <Badge variant="outline" className="text-xs text-gray-400">
          {publishedMaterials.length} item{publishedMaterials.length !== 1 ? 's' : ''}
        </Badge>
      </div>
      {expanded && (
        <div className="border-t border-slate-100 px-2 py-2">
          {isLoading ? (
            ...
            <p className="text-xs text-gray-400 px-4 py-2">No materials available yet.</p>
          ) : (
...
const MaterialViewer = ({ material, onClose }: MaterialViewerProps) => {
  ...
  return (
    <Card className="bg-white border-0 shadow-md rounded-xl p-6">
      ...
      {material.type === 'file' && (material.content_url || material.file_path) && (
        <a
...
const CourseDetail = () => {
  ...
  const { data: announcements, isLoading: announcementsLoading } = useAnnouncements(courseId);
      ...
      {!viewingMaterial && (
        <>
          {announcementsLoading ? (
            ...
            <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
              ...
              >
                ...
                <h2 className="text-lg font-bold tracking-tight text-white">Announcements</h2>
              </div>
          ...
          <div className="space-y-3">
            <h2 className="text-lg font-bold tracking-tight">Course Materials</h2>
            {modulesLoading ? (
              <div className="space-y-3">
              ...
              <Card className="bg-white border-0 shadow-md rounded-xl p-8 text-center">
                ...
                <p className="text-sm text-gray-500">No course materials available yet.</p>
              </Card>
            ) : (

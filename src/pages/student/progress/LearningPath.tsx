...
const StatusIcon = ({ status }: { status: LearningPathNode["status"] }) => {
  switch (status) {
    ...
    case "submitted":
      return (
        <Circle
          className="h-5 w-5 text-blue-500 fill-blue-100"
          aria-hidden="true"
        />
      );
    case "available":
      return (
        <span className="relative flex h-5 w-5">
          ...
          <Circle
            className="relative h-5 w-5 text-blue-500"
            aria-hidden="true"
          />
        </span>
      );
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1 space-y-2">
                  ...
                  <div className="h-1.5 w-full rounded-full bg-gray-100">
                    <div
                      className={cn(
                        ...
                        node.attainment_percent >= 70 &&
                          node.attainment_percent < 85 &&
                          "bg-blue-500",
                        node.attainment_percent >= 50 &&
                          node.attainment_percent < 70 &&
                          "bg-yellow-500",
                        node.attainment_percent < 50 && "bg-red-500"
                      )}
                      style={{
                        width: `${Math.min(node.attainment_percent, 100)}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              {isLocked && node.prerequisite && (
                ...
                >
                  Complete {node.prerequisite.clo_title} at{" "}
                  {node.prerequisite.required_attainment}% to unlock
                </p>
              )}

...
import { Trophy, Flame, Medal, TrendingUp, Star, BarChart3, Shield } from 'lucide-react';
...
const LeaderboardRow = ({ entry, isCurrentUser, totalStudents }: LeaderboardRowProps) => {
  ...
  const bandResult = totalStudents
    ...
    : null;
  const displayRank = bandResult ? formatPercentileBand(bandResult) : `#${entry.rank}`;
...
const MyRankCard = ({ rank, xpTotal, level, isLoading, totalStudents }: MyRankCardProps) => {
  if (isLoading) {
    return <Shimmer className="h-24 rounded-xl" />;
  }

  // Calculate percentile band for display (Req 131.1, 131.2, 131.4)
  const bandResult = rank && totalStudents
    ? calculatePercentileBand(rank, totalStudents)
    : null;
  const displayRank = bandResult ? formatPercentileBand(bandResult) : (rank ? `#${rank}` : '—');
    ...
    <div className="space-y-4">
      ...
      <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
        ...
        >
          ...
          <h2 className="text-lg font-bold tracking-tight text-white">Your Weekly XP</h2>
        </div>
        <div className="p-6">
...
const MostImprovedList = ({ courseId, currentUserId }: MostImprovedListProps) => {
  ...
  if (!entries || entries.length === 0) {
    return (
      <p className="text-sm text-gray-500 text-center py-8">
        No improvement data available yet. Keep earning XP to see who is improving the most!
      </p>
    );
  }
  ...
  return (
    <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
      ...
      >
        ...
        <h2 className="text-lg font-bold tracking-tight text-white">Most Improved — Top 20</h2>
      </div>
        ...
        {entries.map((entry, index) => {
          ...
          return (
            <div
              ...
              className={cn(
                'flex items-center gap-4 px-4 py-3 rounded-xl border transition-colors',
                isTop3 ? 'bg-green-50 border-green-200' : 'bg-white border-slate-100',
                isCurrentUser && 'ring-2 ring-blue-400',
              )}
            >
              <div className="flex items-center justify-center w-10 shrink-0">
              ...
              <div className="text-right shrink-0">
                <span
                  className={cn(
                    'text-sm font-bold',
                    entry.improvement_percent >= 0 ? 'text-green-600' : 'text-red-500',
                  )}
                >
...
const LeagueLeaderboardList = ({ courseId, currentUserId }: LeagueLeaderboardListProps) => {
  const { data: tierData, isLoading: isLoadingTier } = useStudentLeagueTier(currentUserId);
  const currentTier = tierData?.tier ?? 'Bronze';
  const { data: entries, isLoading: isLoadingEntries } = useLeagueLeaderboard(courseId, currentTier);
                ...
                >
                  <div className="flex items-center justify-center w-10 shrink-0">
                    {getMedalColor(entry.rank) ? (
                      <Medal className="h-6 w-6" style={{ color: getMedalColor(entry.rank)! }} />
                    ) : (
...
const LeaderboardPage = () => {
  ...
  // URL-persisted state
  const [mode, setMode] = useQueryState('mode', parseAsString.withDefault('individual'));
  ...
  const [timeframe, setTimeframe] = useQueryState('timeframe', parseAsString.withDefault('all_time'));

  const { courses, programs, isLoading: isLoadingContext } = useStudentCourseProgram(userId);

  const [selectedCourseId, setSelectedCourseId] = useQueryState('courseId', parseAsString.withDefault(''));
  const [selectedProgramId, setSelectedProgramId] = useQueryState('programId', parseAsString.withDefault(''));
      ...
      {mode === 'teams' ? (
        ...
        <>
          ...
          <div className="flex gap-2 flex-wrap">
            {[
              { value: 'top_xp' as LeaderboardTab, label: 'Top XP', icon: Trophy },
              { value: 'personal_best' as LeaderboardTab, label: 'Personal Best', icon: BarChart3 },
              { value: 'most_improved' as LeaderboardTab, label: 'Most Improved', icon: TrendingUp },
              { value: 'league' as LeaderboardTab, label: 'League', icon: Shield },
            ].map((opt) => {
              const Icon = opt.icon;
              return (
                <button
          ...
          {tab === 'personal_best' ? (
            ...
            <MostImprovedList courseId={effectiveCourseId} currentUserId={userId} />
          ) : tab === 'league' ? (
            <LeagueLeaderboardList courseId={effectiveCourseId} currentUserId={userId} />
          ) : (
            <>

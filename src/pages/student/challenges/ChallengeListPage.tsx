...
const ChallengeCard = ({ challenge, onClick }: { challenge: Challenge; onClick: () => void }) => {
...
const ChallengeListPage = () => {
  ...
  const active = (challenges ?? []).filter(
    (c) => c.status === 'active' || (new Date(c.start_date) <= now && new Date(c.end_date) >= now),
  );
  const upcoming = (challenges ?? []).filter(
    (c) => c.status === 'draft' || new Date(c.start_date) > now,
  );
  const completed = (challenges ?? []).filter(
    (c) => c.status === 'completed' || (c.status !== 'active' && new Date(c.end_date) < now),
  );

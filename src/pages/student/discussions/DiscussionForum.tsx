...
import { createThreadSchema, type CreateThreadFormData } from '@/lib/schemas/discussion';
  ...
  return (
    <div className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="Thread title..." {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Textarea placeholder="What would you like to discuss?" rows={4} {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                <Button type="submit" disabled={createThread.isPending} className="bg-gradient-to-r from-teal-500 to-blue-600 active:scale-95 transition-transform duration-100">
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Search threads..." className="ps-9" value={search} readOnly />
      </div>
      ...
      {isLoading ? (
        ...
        <Card className="bg-white border-0 shadow-md rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-gray-500">No discussion threads yet. Start a conversation!</p>
        </Card>
      ) : (
        <div className="space-y-3">
          {filtered.map((thread) => (
            <DiscussionThreadCard
              ...
              onClick={() => navigate(`/student/courses/${courseId}/discussions/${thread.id}`)}
            />
          ))}
        </div>
      )}

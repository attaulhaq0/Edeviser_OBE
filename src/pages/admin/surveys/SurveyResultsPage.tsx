...
const QuestionResult = ({ question, responses, index }: QuestionResultProps) => {
  ...
  if (question.question_type === 'likert') {
    ...
    return (
      <Card className="bg-white border-0 shadow-md rounded-xl p-4 space-y-3">
        ...
        <p className="text-xs text-gray-500">{totalResponses} response{totalResponses !== 1 ? 's' : ''}</p>
      </Card>
    );
  }
      ...
      <div className="max-h-48 overflow-y-auto space-y-2">
        {questionResponses.length === 0 ? (
          ...
          questionResponses.map((r) => (
            <div key={r.id} className="bg-slate-50 rounded-lg px-3 py-2 text-xs text-gray-700">
              {r.response_value}
            </div>
          ))
        )}
    ...
    <div className="space-y-6">
      ...
      {surveysLoading ? (
        ...
        <Card className="bg-white border-0 shadow-md rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-gray-500">Select a survey to view results.</p>
        </Card>
      ) : (
        <>
          {/* Summary KPIs */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="bg-white border-0 shadow-md rounded-xl p-4">
              <p className="text-[10px] font-black tracking-widest uppercase text-gray-500">Total Responses</p>
              <p className="text-2xl font-black mt-1">{responses?.length ?? 0}</p>
            </Card>
            <Card className="bg-white border-0 shadow-md rounded-xl p-4">
              <p className="text-[10px] font-black tracking-widest uppercase text-gray-500">Respondents</p>
              <p className="text-2xl font-black mt-1">{uniqueRespondents}</p>
            </Card>
            <Card className="bg-white border-0 shadow-md rounded-xl p-4">
              <p className="text-[10px] font-black tracking-widest uppercase text-gray-500">Questions</p>
              <p className="text-2xl font-black mt-1">{questions?.length ?? 0}</p>
            </Card>
            <Card className="bg-white border-0 shadow-md rounded-xl p-4">
              <p className="text-[10px] font-black tracking-widest uppercase text-gray-500">Completion Rate</p>
              <p className="text-2xl font-black mt-1">
                {questions?.length && uniqueRespondents
                  ? `${Math.min(100, Math.round((responses?.length ?? 0) / (questions.length * uniqueRespondents) * 100))}%`
                  : '—'}
              </p>
            </Card>
          </div>
          ...
          <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
            ...
            >
              ...
              <h2 className="text-lg font-bold tracking-tight text-white">Question Breakdown</h2>
            </div>
            <div className="p-6 space-y-4">
              ...
              {!questions?.length && (
                <p className="text-sm text-gray-500 text-center py-4">No questions found for this survey.</p>
              )}
            </div>
          </Card>
        </>

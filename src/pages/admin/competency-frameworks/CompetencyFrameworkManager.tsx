...
const CompetencyFrameworkManager = () => {
  ...
  const handleCSVImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    importMutation.mutate(
      { framework_id: selectedFramework, csv_content: text },
      {
        onSuccess: (result) =>
  ...
  const handleAddMapping = async () => {
    ...
    const { error } = await supabase
      ...
      .insert({
        competency_item_id: mappingIndicatorId,
        outcome_id: mappingOutcomeId,
      } as never);
    if (error) {
      ...
      queryClient.invalidateQueries({
        queryKey: ["competencyOutcomeMappings"],
      });
    }
  };
      ...
      {selectedFramework && (
        <Card className="bg-white border-0 shadow-md rounded-xl p-6">
          ...
          <p className="text-xs text-slate-500 mb-3">
            CSV columns: domain_code, domain_title, competency_code,
            competency_title, indicator_code, indicator_title
          </p>
        ...
        <Card className="bg-white border-0 shadow-md rounded-xl p-6">
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              <label
                htmlFor="mapping-indicator"
                className="text-xs font-medium text-slate-600 mb-1 block"
              >
                Indicator
              </label>
              <Select
                value={mappingIndicatorId}
                onValueChange={setMappingIndicatorId}
              >
                <SelectTrigger id="mapping-indicator" className="bg-white">
            ...
            <div className="flex-1">
              <label
                htmlFor="mapping-outcome"
                className="text-xs font-medium text-slate-600 mb-1 block"
              >
                Outcome (ILO/PLO)
              </label>
              <Select
                value={mappingOutcomeId}
                onValueChange={setMappingOutcomeId}
              >
                <SelectTrigger id="mapping-outcome" className="bg-white">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g. ABET, HEC" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}

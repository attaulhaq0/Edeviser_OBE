...
const ParentInvitePage = () => {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: { parent_email: '', parent_name: '', student_id: '', relationship: 'parent' },
  });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      await logAuditEvent({
        ...
        entity_id: parentId,
        changes: { parent_email: data.parent_email, student_id: data.student_id },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">Invite Parent / Guardian</h1>
      </div>
      ...
      <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
        ...
        >
          ...
          <h2 className="text-lg font-bold tracking-tight text-white">New Parent Invite</h2>
        </div>
        <div className="p-6">
          <p className="text-sm text-slate-500 mb-6">
            Create a parent account and link it to a student. The parent will receive read-only access to their child's academic data.
          </p>
          {studentsLoading ? (

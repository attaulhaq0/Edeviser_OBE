...
import { useAcademicCalendarEvents, useCreateAcademicEvent, useDeleteAcademicEvent } from '@/hooks/useAcademicCalendar';
...
const EVENT_TYPES = ['semester_start', 'semester_end', 'exam_period', 'holiday', 'registration', 'other'];
...
const AcademicCalendarManager = () => {
  ...
  const form = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { title: '', event_type: '', start_date: '', end_date: '' } });

  const onSubmit = (data: FormData) => {
    createMutation.mutate(data, {
      onSuccess: () => { toast.success('Event created'); form.reset(); },
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
        <div className="px-6 py-4 flex items-center gap-2" style={{ background: 'linear-gradient(93.65deg, #14B8A6 5.37%, #0382BD 78.89%)' }}>
          ...
          <h2 className="text-lg font-bold tracking-tight text-white">Events</h2>
        </div>
        <div className="p-6">
          {isLoading ? <Shimmer className="h-32 rounded-lg" /> : events.length === 0 ? (
              ...
              {events.map((e) => (
                <div key={e.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:bg-slate-50">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setDeleteTarget(e.id)} className="h-7 w-7 p-0 text-slate-400 hover:text-red-500">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <FormField control={form.control} name="end_date" render={({ field }) => (
                <FormItem><FormLabel>End (optional)</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
            </div>
            <Button type="submit" disabled={createMutation.isPending} className="bg-gradient-to-r from-teal-500 to-blue-600 active:scale-95">
              {createMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Add Event
            </Button>
          </form>
        </Form>
      </Card>

      <ConfirmDialog open={!!deleteTarget} onOpenChange={(o: boolean) => !o && setDeleteTarget(null)} title="Delete event?" description="This will permanently remove this calendar event." onConfirm={() => { if (deleteTarget) deleteMutation.mutate(deleteTarget, { onSuccess: () => { toast.success('Deleted'); setDeleteTarget(null); } }); }} isPending={deleteMutation.isPending} />
    </div>
  );
};

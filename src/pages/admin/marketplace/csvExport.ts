...
export async function exportPurchaseHistoryCSV(): Promise<void> {
  try {
    ...
    const rows: PurchaseRow[] = ((purchases ?? []) as Array<Record<string, unknown>>).map((row) => {
    ...
    // Build CSV
    const headers = ['Purchase ID', 'Student ID', 'Student Name', 'Item', 'Category', 'XP Cost', 'Status', 'Date'];
    const csvLines = [
      headers.join(','),
      ...rows.map((r) =>
        [
    ...
    link.href = url;
    link.download = `marketplace-purchases-${new Date().toISOString().slice(0, 10)}.csv`;

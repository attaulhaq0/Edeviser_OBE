...
const XPEconomistDashboard = () => {
  ...
  const { data: ratioData, isLoading: ratioLoading } = useEarnSpendRatio(institutionId);
  const { data: velocityData, isLoading: velocityLoading } = useXPVelocity(institutionId, 8);

  const isLoading = ratioLoading || velocityLoading;

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold tracking-tight">XP Economist Dashboard</h1>
        ...
        <>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-black mt-1">
                    {ratioData?.ratio !== null ? `${ratioData?.ratio}:1` : 'N/A'}
                  </p>
                </div>
            ...
            <Card className="bg-white border-0 shadow-md rounded-xl p-4 group">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge className={`mt-1 ${STATUS_COLORS[ratioData?.status ?? 'no_spending']}`}>
          ...
          <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
            ...
            >
              ...
              <h2 className="text-lg font-bold tracking-tight text-white">XP Velocity</h2>
            </div>
            <div className="p-6">
              <div className="space-y-2">
                {(velocityData ?? []).map((point) => (
                  <div key={point.week} className="flex items-center gap-4 text-sm">
                    <span className="w-24 text-xs text-gray-500">{point.week}</span>
                    <div className="flex-1 flex items-center gap-2">
                      <div
                        className="h-4 rounded bg-green-400"
                        style={{ width: `${Math.min(100, (point.earned / Math.max(1, ...((velocityData ?? []).map((v) => v.earned)))) * 100)}%` }}
                      />
                      <span className="text-xs text-green-600">+{point.earned}</span>
                    </div>
                    <div className="flex-1 flex items-center gap-2">
                      <div
                        className="h-4 rounded bg-red-400"
                        style={{ width: `${Math.min(100, (point.spent / Math.max(1, ...((velocityData ?? []).map((v) => v.spent)))) * 100)}%` }}
                      />
                      <span className="text-xs text-red-600">-{point.spent}</span>
                    </div>
                  </div>
                ))}
              </div>

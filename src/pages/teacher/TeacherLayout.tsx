...
const TeacherLayout = () => {
  ...
  // The dashboard is the only teacher page whose prototype defines a right rail
  // (shared.js `railHTML()` teacher case). Render it there and reserve its width
  // so the feed sits in a true 3-column shell (sidebar · feed · rail) instead of
  // a centered, floating middle. Other teacher pages keep the rail-less layout
  // until their own rails are built (per-page, next increment).
  const onDashboard = location.pathname === "/teacher/dashboard";
  ...
  return (
    <SidebarProvider>
      ...
      <div className="flex">
        ...
        {onDashboard && <TeacherDashboardRail />}
      </div>
      <GuidedTour />
    </SidebarProvider>
  );

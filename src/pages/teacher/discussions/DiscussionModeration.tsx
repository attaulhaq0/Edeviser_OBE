...
const DiscussionModeration = () => {
  ...
  const handlePin = (threadId: string, currentlyPinned: boolean) => {
    ...
    togglePin.mutate(
      ...
      {
        onSuccess: () => toast.success(currentlyPinned ? 'Thread unpinned' : 'Thread pinned'),
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <Card className="bg-white border-0 shadow-md rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-gray-500">No discussion threads in this course.</p>
        </Card>
      ) : (
        <div className="space-y-3">
          {(threads ?? []).map((thread) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => navigate(`/teacher/courses/${courseId}/discussions/${thread.id}`)}
                  onKeyDown={(e) => { if (e.key === 'Enter') navigate(`/teacher/courses/${courseId}/discussions/${thread.id}`); }}
                >
                  <div className="flex items-center gap-2">
                    {thread.is_pinned && <Pin className="h-3 w-3 text-blue-500 shrink-0" />}
                    <h3 className="text-sm font-bold truncate">{thread.title}</h3>
                    {thread.is_resolved && (
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {format(new Date(thread.created_at), 'MMM d, yyyy')} · {thread.reply_count ?? 0} replies
                  </p>
                </div>

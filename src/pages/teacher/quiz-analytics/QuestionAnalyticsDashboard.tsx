...
import { useQuestionAnalytics, type QuestionAnalyticsRow } from '@/hooks/useQuestionAnalytics';
...
const getSuggestedAction = (flag: string | null): { label: string; description: string } => {
  switch (flag) {
    case 'too_easy':
      return {
        label: 'Increase Difficulty',
        description: 'This question is answered correctly by >95% of students. Consider adding more challenging distractors or replacing it with a higher Bloom\'s level question.',
      };
    case 'too_hard':
      return {
        label: 'Simplify or Replace',
        description: 'This question has a success rate below 10%. Review the question text for clarity, check distractors for ambiguity, or replace with a lower difficulty question.',
      };
    case 'low_discrimination':
      return {
        label: 'Review Distractors',
        description: 'This question does not differentiate well between high and low performers. Revise distractors to target common misconceptions more effectively.',
      };
    default:
      return {
...
const QuestionAnalyticsDashboard = () => {
  ...
  const [selectedQuestion, setSelectedQuestion] = useState<QuestionAnalyticsRow | null>(null);

  // URL-persisted filter state
  const [cloFilter, setCloFilter] = useQueryState('clo', parseAsString.withDefault(''));
  const [bloomFilter, setBloomFilter] = useQueryState('bloom', parseAsString.withDefault(''));
  const [qualityFilter, setQualityFilter] = useQueryState('quality', parseAsString.withDefault(''));

  // Data fetching
  const { data: analyticsRows, isLoading } = useQuestionAnalytics(courseId ?? '', {
  ...
  const suggestedAction = selected ? getSuggestedAction(selected.quality_flag) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Select value={cloFilter} onValueChange={(val) => setCloFilter(val === 'all' ? '' : val)}>
        ...
        <Select value={bloomFilter} onValueChange={(val) => setBloomFilter(val === 'all' ? '' : val)}>
        ...
        <Select value={qualityFilter} onValueChange={(val) => setQualityFilter(val === 'all' ? '' : val)}>
      ...
      {/* Data Table */}
      <DataTable columns={columns} data={analyticsRows ?? []} isLoading={isLoading} />

      {/* Flagged Question Detail Panel */}
      <Sheet open={!!selected} onOpenChange={(open) => !open && setSelectedQuestion(null)}>
              ...
              <div className="space-y-6 px-4 pb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-lg font-bold">{selected.total_attempts}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge
                    className={cn(
                      ...
                      BLOOM_COLORS[selected.question_bank.bloom_level] ?? 'bg-gray-200 text-gray-700',
                    )}
                  >
                    {BLOOM_LABELS[selected.question_bank.bloom_level] ?? `L${selected.question_bank.bloom_level}`}
                  </Badge>
                  <DifficultyBadge
                    difficulty={selected.calibrated_difficulty ?? selected.question_bank.difficulty_rating}
                  />
                  <QuestionQualityIndicator
                    qualityFlag={selected.quality_flag as 'good' | 'low_discrimination' | 'too_easy' | 'too_hard' | null}
                  />
                </div>
                ...
                {suggestedAction && (
                  <Card className="bg-white border-0 shadow-md rounded-xl p-4 space-y-2">
                    ...
                    <p className="text-sm font-bold text-blue-600">{suggestedAction.label}</p>

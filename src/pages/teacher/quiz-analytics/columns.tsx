...
export const createAnalyticsColumns = (
...
): ColumnDef<QuestionAnalyticsRow>[] => [
  {
    ...
    cell: ({ row }) => {
      ...
      const isFlagged =
        row.original.quality_flag !== null && row.original.quality_flag !== 'good';
      return (
        <button
          type="button"
          className={cn(
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <span className="text-sm font-medium">{(rate * 100).toFixed(1)}%</span>;
    },
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-sm">{row.getValue('total_attempts') as number}</span>
    ),
  },
  {
    id: 'difficulty',
    ...
    cell: ({ row }) => {
      ...
      return (
        <QuestionQualityIndicator
          qualityFlag={flag as 'good' | 'low_discrimination' | 'too_easy' | 'too_hard' | null}
        />
      );
    },
  },

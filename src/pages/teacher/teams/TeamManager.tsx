...
import { useTeams, useCreateTeam, useAutoGenerateTeams } from '@/hooks/useTeams';
...
const TeamManager = () => {
  ...
  const onSubmit = (data: z.infer<typeof teamSchema>) => {
    createMutation.mutate({ ...data, created_by: user?.id ?? '' }, {
      onSuccess: () => { toast.success('Team created'); form.reset(); },
      onError: (err) => toast.error(err.message),
    });
  };

  const onAutoGenerate = (data: z.infer<typeof autoGenSchema>) => {
    autoGenMutation.mutate({ ...data, created_by: user?.id ?? '' }, {
      onSuccess: (result) => toast.success(`Created ${result.teams_created} teams with ${result.students_assigned} students`),
      onError: (err) => toast.error(err.message),
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Select value={selectedCourse} onValueChange={setSelectedCourse}>
          <SelectTrigger className="w-64 bg-white"><SelectValue placeholder="Select course" /></SelectTrigger>
          <SelectContent>
      ...
      <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
        ...
        <div className="p-6">
          {!selectedCourse ? (
            <p className="text-sm text-slate-400 text-center py-6">Select a course to manage teams.</p>
          ) : isLoading ? (
            ...
            <div className="space-y-2">
              {teams.map((team) => (
                <div key={team.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:bg-slate-50">
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <Card className="bg-white border-0 shadow-md rounded-xl p-6">
          <h2 className="text-lg font-bold tracking-tight mb-4">Auto-Generate Teams</h2>
          <Form {...autoForm}>
            <form onSubmit={autoForm.handleSubmit(onAutoGenerate)} className="space-y-4">
              <FormField control={autoForm.control} name="course_id" render={({ field }) => (
                <FormItem><FormLabel>Course</FormLabel><FormControl>
                  <Select value={field.value} onValueChange={field.onChange}>
                    <SelectTrigger className="bg-white"><SelectValue placeholder="Select" /></SelectTrigger>
                    <SelectContent>{courses.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
              ...
              <FormField control={autoForm.control} name="team_size" render={({ field }) => (
                <FormItem><FormLabel>Team Size (2–6)</FormLabel><FormControl>
                  <Input type="number" min={2} max={6} {...field} onChange={(e) => field.onChange(parseInt(e.target.value) || 4)} />
                </FormControl><FormMessage /></FormItem>
              )} />
              <Button type="submit" disabled={autoGenMutation.isPending} variant="outline" className="gap-2">
                {autoGenMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Shuffle className="h-4 w-4" />} Generate
              </Button>
            </form>
          </Form>
        </Card>

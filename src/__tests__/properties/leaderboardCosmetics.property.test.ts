...
function resolveLeaderboardCosmetics(student: LeaderboardStudent): ResolvedLeaderboardEntry {
...
const studentArb: fc.Arbitrary<LeaderboardStudent> = fc.record({
  ...
  equipped_frame: fc.option(fc.constantFrom('golden_laurel', 'silver_ring', 'neon_glow'), { nil: null }),
  equipped_title: fc.option(fc.constantFrom('The Scholar', 'Night Owl', 'Speed Demon'), { nil: null }),
});
    ...
    fc.assert(
      fc.property(
        ...
        (maxAttempts, hasToken) => {
          ...
          const attemptCount = fc.sample(fc.integer({ min: 0, max: maxAttempts - 1 }), 1)[0]!;
          expect(canAttemptQuiz(attemptCount, maxAttempts, hasToken)).toBe(true);
        },
      ),
      { numRuns: 100 },
    );

...
function checkScopedAccess(
  ...
  if (role === 'teacher' && (table === 'courses' || table === 'learning_outcomes_clo')) {
    return userId === recordOwnerId ? 'allow' : 'deny';
  }

  // Student: scoped to own data
  if (role === 'student' && ['submissions', 'grades', 'student_gamification', 'xp_transactions', 'journal_entries'].includes(table)) {
    return userId === recordOwnerId ? 'allow' : 'deny';
  }
  ...
  it('P5a: student cannot access ILO records', () => {
    fc.assert(
      fc.property(institutionIdArb, (instId) => {
        const decision = checkAccess('student', 'learning_outcomes_ilo', instId, instId);
        expect(decision).toBe('deny');
      }),
      { numRuns: 100 },
    );
  ...
  it('P5b: teacher can only see CLOs from own courses', () => {
    fc.assert(
      fc.property(
        userIdArb,
        userIdArb,
        (teacherId, courseOwnerId) => {
          const decision = checkScopedAccess('teacher', 'learning_outcomes_clo', teacherId, courseOwnerId);
  ...
  it('P5c: student can only see own submissions', () => {
    fc.assert(
      fc.property(
        userIdArb,
        userIdArb,
        (studentId, recordOwnerId) => {
          const decision = checkScopedAccess('student', 'submissions', studentId, recordOwnerId);

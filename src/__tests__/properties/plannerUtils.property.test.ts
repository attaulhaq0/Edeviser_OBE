...
const priorityArb: fc.Arbitrary<TaskPriority> = fc.constantFrom('low', 'medium', 'high');
...
const timeStringArb = fc.tuple(hourArb, minuteArb).map(
...
const timelineItemArb = (time: string | null): fc.Arbitrary<TimelineItem> =>
  fc.record({
    id: fc.uuid(),
    type: fc.constantFrom('session' as const, 'task' as const, 'deadline' as const),
...
describe('plannerUtils property tests', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (startedAt, activeMs, pauseFraction) => {
            ...
            const result = calculateActualDuration(startedAt, endedAt, totalPausedMs);
            const expected = Math.max(0, Math.round((endedAt - startedAt - totalPausedMs) / 60000));
            expect(result).toBe(expected);
            expect(result).toBeGreaterThanOrEqual(0);
            expect(result).toBeLessThanOrEqual(Math.round((endedAt - startedAt) / 60000));
          },
        ),
        { numRuns: 100 },
      );
      ...
      fc.assert(
        fc.property(
          ...
          (hoursUntilDue) => {
            ...
            const due = new Date(now.getTime() + hoursUntilDue * 60 * 60 * 1000);
  ...
  describe('Property 8: Session missed detection', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (durationMin, minutesPastEnd) => {
            ...
            sessionEnd.setMinutes(sessionEnd.getMinutes() + durationMin + minutesPastEnd);
    ...
    it('should return false for non-planned sessions regardless of time', () => {
      fc.assert(
        fc.property(
          fc.constantFrom('in_progress' as const, 'completed' as const, 'cancelled' as const),
          (status) => {
            const session: StudySession = {
              id: 'test-id',
              studentId: 'student-id',
      ...
      fc.assert(
        fc.property(
          ...
          (baseDate, dayOffset) => {
            ...
            const today = new Date(weekEnd.getTime() + dayOffset * 24 * 60 * 60 * 1000);
  ...
  describe('Property 13: Word count accuracy', () => {
    it('should count whitespace-separated tokens correctly', () => {
      fc.assert(
        fc.property(
          fc.array(
            fc.string({ minLength: 1, maxLength: 15, unit: 'grapheme' }).filter((s) => !/\s/.test(s)),
    ...
    it('should return 0 for empty or whitespace-only strings', () => {
      fc.assert(
        fc.property(
          fc.array(fc.constantFrom(' ', '\t', '\n'), { minLength: 0, maxLength: 20 }).map((arr) =>
            arr.join(''),
          ),
          (text) => {
        ...
        fc.property(
          ...
          (weekCount) => {
            ...
            for (let i = 1; i < result.length; i++) {
              expect(result[i]!.weekStartDate > result[i - 1]!.weekStartDate).toBe(true);
            }
          },
        ),
        { numRuns: 100 },
        ...
        fc.property(
          fc.array(
            fc.record({
              ...
              actualDurationMinutes: fc.oneof(fc.constant(null), fc.integer({ min: 15, max: 120 })),
              timerMode: fc.constant('pomodoro' as const),
              status: fc.constantFrom('planned' as const, 'completed' as const, 'cancelled' as const),
              satisfactionRating: fc.constant(null),
              cloIds: fc.constant(null),
              createdAt: fc.constant('2025-06-01T00:00:00Z'),
            }),

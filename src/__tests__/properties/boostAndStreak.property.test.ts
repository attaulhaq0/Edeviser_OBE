...
function canActivateBoost(existingBoosts: ActiveBoost[], now: Date): boolean {
  return !existingBoosts.some((b) => new Date(b.expires_at).getTime() > now.getTime());
}

function incrementStreakFreezes(current: number): { newCount: number; rejected: boolean } {
  if (current >= 3) {
...
describe('Property 19 — One active XP boost at a time', () => {
  it('P19a: cannot activate boost when one is already active (not expired)', () => {
    fc.assert(
      fc.property(fc.uuid(), fc.double({ min: 1.5, max: 3.0 }), (studentId, multiplier) => {

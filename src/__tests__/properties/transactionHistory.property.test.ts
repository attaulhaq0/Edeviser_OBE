...
function filterEntries(
  ...
  return entries.filter((e) => (filter === 'earnings' ? e.type === 'earning' : e.type === 'spending'));
}
...
const earningArb: fc.Arbitrary<EarningEntry> = fc.record({
  ...
  source_label: fc.constantFrom('Login Bonus', 'Assignment Submission', 'Badge Earned', 'Streak Milestone'),
});
...
const spendingArb: fc.Arbitrary<SpendingEntry> = fc.record({
  ...
  item_name: fc.constantFrom('Ocean Blue Theme', 'Extra Quiz Attempt', '2x XP Boost', 'Streak Shield'),
  item_category: fc.constantFrom('cosmetic', 'educational_perk', 'power_up'),
});

const entryArb: fc.Arbitrary<TransactionEntry> = fc.oneof(earningArb, spendingArb);
...
describe('Property 5 — Transaction history filtering', () => {
  it('P5: filtering by "earnings" returns only earning entries, "spending" only spending, "all" returns all', () => {
    fc.assert(
      fc.property(
        fc.array(entryArb, { minLength: 1, maxLength: 30 }),
        fc.constantFrom<'all' | 'earnings' | 'spending'>('all', 'earnings', 'spending'),
        (entries, filter) => {
          ...
          if (filter === 'all') {
            ...
            expect(filtered.length).toBe(entries.filter((e) => e.type === 'earning').length);
          } else {
            expect(filtered.every((e) => e.type === 'spending')).toBe(true);
            expect(filtered.length).toBe(entries.filter((e) => e.type === 'spending').length);
          }
        },
      ),
      { numRuns: 100 },

...
function validateCSVRow(
  ...
  if (row.program_id && !existingProgramIds.includes(row.program_id)) {
    errors.push({
      row: rowIndex,
      field: "program_id",
      message: "Non-existent program_id",
    });
  }
...
const validEmailArb = fc
  .tuple(
    ...
    fc.constantFrom("test.com", "uni.edu")
  )
...
const validRoleArb = fc.constantFrom(
  "admin",
  "coordinator",
  "teacher",
  "student"
...
const validRowArb = (programIds: string[]) =>
  fc.record({
    ...
    role: validRoleArb,
    program_id:
      programIds.length > 0 ? fc.constantFrom(...programIds) : fc.constant(""),
  });
    ...
    fc.assert(
      fc.property(
        ...
        (programIds) => {
          return fc.assert(
            fc.property(
              fc.array(validRowArb(programIds), {
                minLength: 1,
                maxLength: 10,
              }),
              (rows) => {
  ...
  it("P13b: invalid email produces validation error", () => {
    fc.assert(
      fc.property(
        ...
        (email, name, role) => {
          const rows: CSVRow[] = [
            { email, full_name: name, role, program_id: "" },
          ];
  ...
  it("P13d: non-existent program_id produces validation error", () => {
    fc.assert(
      fc.property(
        ...
        validRoleArb,
        programIdArb,
        (email, name, role, badProgramId) => {
          const rows: CSVRow[] = [
            { email, full_name: name, role, program_id: badProgramId },
          ];
          const result = processBulkImport(rows, []); // empty existing programs
          expect(result.errors.some((e) => e.field === "program_id")).toBe(
            true
          );
        }
      ),
...
describe("Property 14 — Bulk import atomicity", () => {
  it("P14a: CSV exceeding 1000 rows is rejected entirely", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1001, max: 1500 }), (rowCount) => {
  ...
  it("P14b: CSV within 1000 rows is not rejected for size", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 1000 }), (rowCount) => {
        const rows: CSVRow[] = Array.from({ length: rowCount }, (_, i) => ({
          email: `user${i}@test.com`,
          full_name: `User ${i}`,
          role: "student",

...
import { canSendTutorMessage, type HintTokenStatus } from '@/lib/hintTokenChecker';

// ─── Arbitraries ────────────────────────────────────────────────────────────

const hintTokenStatusArb: fc.Arbitrary<HintTokenStatus> = fc.record({
...
describe('Property 17 — Hint tokens expire at midnight UTC', () => {
  it('P17: a token purchased at any time T expires after midnight UTC of that day', () => {
    fc.assert(
      fc.property(
        fc.integer({ min: new Date('2024-01-01').getTime(), max: new Date('2025-12-31').getTime() }).map((ts) => new Date(ts)),
        (purchaseDate) => {

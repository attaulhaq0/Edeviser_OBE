...
const frameArb: fc.Arbitrary<FrameMetadata> = fc.record({
  ...
  border_style: fc.option(fc.constantFrom("solid", "dashed", "double"), {
    nil: undefined,
  }),
  border_radius: fc.option(fc.constantFrom("4px", "8px", "9999px"), {
    nil: undefined,
  }),
  box_shadow: fc.option(fc.constant("0 0 12px rgba(0,0,0,0.3)"), {
    nil: undefined,
  }),
});

type Slot = "profile_theme" | "avatar_frame" | "display_title";
const slotArb = fc.constantFrom<Slot>(
  "profile_theme",
  "avatar_frame",
  "display_title"
);

    ...
    fc.assert(
      fc.property(
        ...
        (currentTier, category, metric) => {
          const result = checkBadgeTierProgression(currentTier, category, metric);
          if (result && result.shouldUpgrade) {
            expect(tierRank(result.newTier)).toBeGreaterThan(tierRank(currentTier));
          }
        },
      ),
      { numRuns: 100 },
  ...
  it('upgrade is idempotent — same metric yields same result', () => {
    fc.assert(
      fc.property(
        ...
        fc.integer({ min: 0, max: 200 }),
        (currentTier, category, metric) => {
          const result1 = checkBadgeTierProgression(currentTier, category, metric);
          const result2 = checkBadgeTierProgression(currentTier, category, metric);
    ...
    fc.assert(
      fc.property(
        ...
        (category, metric) => {
          ...
          if (finalResult) {
            expect(tierRank(currentTier)).toBeGreaterThanOrEqual(tierRank(finalResult.newTier));
          }
        },
      ),
      { numRuns: 100 },

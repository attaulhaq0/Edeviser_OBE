...
function canStudentSendInvitation(
  mode: FormationMode,
  isCaptain: boolean
): boolean {
  ...
  it("teacher_assigned mode blocks student team creation", () => {
    fc.assert(
      fc.property(fc.boolean(), (isEnrolled) => {
        expect(canStudentCreateTeam("teacher_assigned", isEnrolled)).toBe(
          false
        );
      }),
      { numRuns: 100 }
  ...
  it("teacher_assigned mode blocks student invitations", () => {
    fc.assert(
      fc.property(fc.boolean(), (isCaptain) => {
        expect(canStudentSendInvitation("teacher_assigned", isCaptain)).toBe(
          false
        );
      }),
      { numRuns: 100 }

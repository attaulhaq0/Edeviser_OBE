 ...
 */
const attemptsWithExactFailures = (
  failCount: number,
  totalMin: number,
  totalMax: number
) =>
...
describe("Mastery failure count triggers recovery — property-based tests", () => {
  it("P21a: exactly 2 failures triggers recovery (shouldActivateRecovery returns true)", () => {
    fc.assert(
      fc.property(attemptsWithExactFailures(2, 2, 15), (attempts) => {
        const failures = countMasteryFailures(
          attempts,
          CLO_ID,
          MASTERY_THRESHOLD
        );
    ...
    fc.assert(
      fc.property(
        ...
        (scores) => {
          ...
          const result = countMasteryFailures(
            attempts,
            CLO_ID,
            MASTERY_THRESHOLD
          );
  ...
  it("P21f: attempts missing the target CLO are counted as failures (score defaults to 0)", () => {
    fc.assert(
      fc.property(
        fc.array(fc.record({ clo_scores: fc.constant({ "other-clo": 90 }) }), {
          minLength: 1,
          maxLength: 10,
        }),
        (attempts) => {

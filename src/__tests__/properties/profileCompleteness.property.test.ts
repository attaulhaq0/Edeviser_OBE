...
const profileCompletenessInputArb: fc.Arbitrary<ProfileCompletenessInput> =
  fc.record({
    ...
    fc.assert(
      fc.property(profileCompletenessInputArb, (input) => {
        ...
        if (input.personality_items < 25) {
          const more = {
            ...input,
            personality_items: input.personality_items + 1,
          };
          expect(calculateProfileCompleteness(more)).toBeGreaterThanOrEqual(
            base
          );
        }
        ...
        if (input.self_efficacy_items < 6) {
          const more = {
            ...input,
            self_efficacy_items: input.self_efficacy_items + 1,
          };
          expect(calculateProfileCompleteness(more)).toBeGreaterThanOrEqual(
            base
          );
        ...
        if (input.study_strategy_items < 8) {
          const more = {
            ...input,
            study_strategy_items: input.study_strategy_items + 1,
          };
          expect(calculateProfileCompleteness(more)).toBeGreaterThanOrEqual(
            base
          );
        ...
        if (input.learning_style_items < 16) {
          const more = {
            ...input,
            learning_style_items: input.learning_style_items + 1,
          };
          expect(calculateProfileCompleteness(more)).toBeGreaterThanOrEqual(
            base
          );

...
function detectLanguage(
  ...
  if (userPref && (SUPPORTED as readonly string[]).includes(userPref)) return userPref;
  if (institutionDefault && (SUPPORTED as readonly string[]).includes(institutionDefault))
...
const unsupportedLangArb = fc.constantFrom('fr', 'de', 'zh', 'ja', 'ko', 'es', null);
const browserLangArb = fc.constantFrom('en-US', 'ar-QA', 'fr-FR', 'de-DE', null);
...
describe('Property 1 — Language detection priority chain always returns valid supported language', () => {
  it('P1a: result is always a supported language', () => {
    fc.assert(
      fc.property(anyLangArb, anyLangArb, browserLangArb, (userPref, instDefault, browser) => {
  ...
  it('P1b: user preference takes highest priority when valid', () => {
    fc.assert(
      fc.property(supportedLangArb, anyLangArb, browserLangArb, (userPref, instDefault, browser) => {
        const result = detectLanguage(userPref, instDefault, browser);
  ...
  it('P1c: institution default is used when user pref is invalid', () => {
    fc.assert(
      fc.property(unsupportedLangArb, supportedLangArb, browserLangArb, (userPref, instDefault, browser) => {
        const result = detectLanguage(userPref, instDefault, browser);
  ...
  it('P1d: falls back to English when all inputs are invalid', () => {
    fc.assert(
      fc.property(unsupportedLangArb, unsupportedLangArb, (userPref, instDefault) => {

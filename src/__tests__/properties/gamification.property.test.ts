      ...
      fc.property(
        ...
        (currentStreak, lastLogin) => {
          ...
          const result = calculateStreakUpdate(
            { streak_count: currentStreak, last_login_date: lastLogin, streak_freezes_available: 0 },
            today,
          );
...
describe('Property 28 — Leaderboard ordering', () => {
  it('P28a: leaderboard is sorted by xp_total descending', () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.record({ student_id: studentIdArb, xp_total: fc.integer({ min: 0, max: 50000 }) }),
          { minLength: 1, maxLength: 20 },
        ),
        (students) => {
          const sorted = [...students].sort((a, b) => b.xp_total - a.xp_total);
          for (let i = 1; i < sorted.length; i++) {
            expect(sorted[i]!.xp_total).toBeLessThanOrEqual(sorted[i - 1]!.xp_total);
          }
        },
      ),
      { numRuns: 100 },
      ...
      fc.property(
        ...
        (students) => {
          ...
          const ranked = sorted.map((s, i) => {
            const rank = i === 0 ? 1 : (s.xp_total === sorted[i - 1]!.xp_total ? i : i + 1);
            return { ...s, rank };
          });
...
describe('Property 29 — Habit tracker Perfect Day detection', () => {
  ...
  function calculatePerfectDayXP(completedHabits: HabitType[], alreadyAwarded: boolean): number {

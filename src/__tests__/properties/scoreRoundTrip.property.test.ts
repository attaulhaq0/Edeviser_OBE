...
const fullPersonalityArb: fc.Arbitrary<PersonalityResponse[]> = fc
  .tuple(
    ...BIG_FIVE_DIMENSIONS.map((dim) =>
      fc.array(
        fc.record({
          dimension: fc.constant(dim),
...
const fullVarkArb: fc.Arbitrary<VARKResponseInput[]> = fc
  .array(fc.constantFrom(...MODALITIES), { minLength: 16, maxLength: 16 })

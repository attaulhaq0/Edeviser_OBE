...
function autoEnroll(
  ...
  const participants =
    challenge.participation_mode === "team" ? teamIds : studentIds;
...
const statusArb = fc.constantFrom<ChallengeStatus>(
  "draft",
  "active",
  "ended",
  "cancelled"
    ...
    fc.assert(
      fc.property(
        ...
        (challenges, enrolledCourseIds) => {
          const filtered = filterChallengesForStudent(
            challenges,
            enrolledCourseIds
          );
          const courseSet = new Set(enrolledCourseIds);

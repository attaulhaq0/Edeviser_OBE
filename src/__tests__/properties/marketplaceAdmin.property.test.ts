...
function computeAnalytics(purchases: PurchaseRecord[]) {
  ...
  for (const p of purchases) {
    ...
    categoryTotals.set(p.category, { count: existing.count + 1, xp: existing.xp + p.xp_cost });
  }

  return { totalXPSpent, totalPurchases, uniqueBuyers, categoryTotals };
}

function getMostPopularItems(purchases: PurchaseRecord[]): Array<{ item_id: string; count: number }> {
...
describe('Property 23 — Analytics aggregation correctness', () => {
  it('P23a: total XP spent equals SUM of all xp_cost', () => {
    fc.assert(
      fc.property(fc.array(purchaseArb, { minLength: 1, maxLength: 50 }), (purchases) => {
  ...
  it('P23c: most popular items are sorted by purchase count descending', () => {
    fc.assert(
      fc.property(fc.array(purchaseArb, { minLength: 2, maxLength: 50 }), (purchases) => {
        const popular = getMostPopularItems(purchases);
        for (let i = 1; i < popular.length; i++) {
          expect(popular[i - 1]!.count).toBeGreaterThanOrEqual(popular[i]!.count);
        }
      }),
      { numRuns: 100 },
    );

...
import { getCurrentWellnessTip, computeWellnessProgress } from '@/lib/wellnessTips';
...
const tipsForTypeArb = (habitType: WellnessHabitType): fc.Arbitrary<WellnessTip[]> =>
  fc
      ...
      fc.assert(
        fc.property(wellnessHabitTypeArb, (type) => {
          const onboardingOnly: WellnessTip[] = [
            { id: 'onboard-1', habitType: type, text: 'Onboarding tip', isOnboarding: true },
          ];
      ...
      fc.assert(
        fc.property(
          ...
          (loggedValue, targetValue) => {
            ...
            const expected = Math.min(Math.round((loggedValue / targetValue) * 100), 100);

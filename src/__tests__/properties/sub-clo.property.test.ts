...
describe("Property 81: Sub-CLO weight sum constraint", () => {
  ...
  fcTest.prop(
    [
      fc.array(fc.double({ min: 0.01, max: 0.3, noNaN: true }), {
        minLength: 2,
        maxLength: 5,
      }),
    ],
    { numRuns: 100 }
  )("weights that do NOT sum to 1.0 fail validation", (weights) => {
...
describe("Property 83: Sub-CLO deletion protection", () => {
  ...
  fcTest.prop([fc.double({ min: 1.01, max: 100, noNaN: true })], {
    numRuns: 100,
  })("Sub-CLO rejects weight > 1.0", (weight) => {
    const result = subCLOSchema.safeParse({
      title: "Test",
      code: "SC1",
      weight,

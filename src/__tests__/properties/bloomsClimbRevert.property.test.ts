...
describe("Bloom's Climb revert on incorrect at new level — property-based tests", () => {
  it('P29a: returns previousLevel when wasCorrect = false and justAdvanced = true', () => {
    fc.assert(
      fc.property(bloomLevelArb, bloomLevelArb, (currentLevel, previousLevel) => {
        const result = handleBloomRevert(currentLevel, previousLevel, false, true);
    ...
    fc.assert(
      fc.property(
        ...
        (currentLevel, previousLevel, justAdvanced) => {
          const result = handleBloomRevert(currentLevel, previousLevel, true, justAdvanced);
    ...
    fc.assert(
      fc.property(
        ...
        (currentLevel, previousLevel, wasCorrect) => {
          const result = handleBloomRevert(currentLevel, previousLevel, wasCorrect, false);
          expect(result).toBe(currentLevel);
        },
      ),
      { numRuns: 100 },
    ...
    fc.assert(
      fc.property(
        ...
        (currentLevel, previousLevel, wasCorrect, justAdvanced) => {
          const result = handleBloomRevert(currentLevel, previousLevel, wasCorrect, justAdvanced);

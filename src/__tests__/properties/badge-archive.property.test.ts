...
function shouldArchive(badge: BadgeRecord, now: Date): boolean {
  ...
  const daysSinceUpgrade =
    (now.getTime() - lastUpgraded.getTime()) / (1000 * 60 * 60 * 24);
...
describe("Property 113: Badge archive threshold", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (lastUpgraded) => {
          ...
          const daysSince =
            (now.getTime() - new Date(lastUpgraded).getTime()) /
            (1000 * 60 * 60 * 24);
  ...
  it("recently upgraded badges are not archived", () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 89 }), (daysAgo) => {
...
describe("Property 114: Badge pin limit enforcement", () => {
  it("allows pinning when fewer than 3 badges pinned", () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 2 }), (pinnedCount) => {
  ...
  it("rejects pinning when 3 or more badges already pinned", () => {
    fc.assert(
      fc.property(fc.integer({ min: 3, max: 20 }), (pinnedCount) => {

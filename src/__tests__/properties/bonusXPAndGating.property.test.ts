...
describe("Property 30 — Bonus XP event multiplier application", () => {
  ...
  it("P30b: XP outside bonus event window is unaffected (multiplier = 1)", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 500 }), (baseXP) => {
      ...
      fc.property(
        ...
        (startDate, durationHours) => {
          const endDate = new Date(
            startDate.getTime() + durationHours * 3600_000
          );
          const midpoint = new Date(
            startDate.getTime() + (durationHours * 3600_000) / 2
          );
      ...
      fc.property(
        ...
        (cloId, threshold) => {
          const studentAttainment =
            threshold + fc.sample(fc.integer({ min: 0, max: 10 }), 1)[0]!;
          const result = canSubmitAssignment(
            [{ clo_id: cloId, min_attainment_percent: threshold }],
            { [cloId]: studentAttainment }
          );
...
describe("Property 32 — First-Attempt Bonus idempotency", () => {
  ...
  it("P32a: first submission scoring >= 50% awards exactly 25 XP", () => {
    fc.assert(
      fc.property(fc.double({ min: 50, max: 100, noNaN: true }), (score) => {
  ...
  it("P32c: first submission scoring < 50% does not award bonus", () => {
    fc.assert(
      fc.property(fc.double({ min: 0, max: 49.99, noNaN: true }), (score) => {
        const bonus = calculateFirstAttemptBonus(score, true, false);
        expect(bonus).toBe(0);
      }),
      { numRuns: 100 }
    ...
    fc.assert(
      fc.property(
        ...
        (id, name) => {
          ...
          const visibility = getBadgeVisibility(badge, {
            badge_id: id,
            earned: true,
          });

...
const completedSessionArb = (
  durationMinutes: number
): fc.Arbitrary<StudySession> =>
  fc.record({
    id: fc.uuid(),
    studentId: fc.constant("student-1"),
    ...
    it("should compute correct progress for study_hours goals", () => {
      fc.assert(
        fc.property(
          ...
          fc.array(fc.integer({ min: 15, max: 120 }), {
            minLength: 0,
            maxLength: 10,
          }),
          (target, durations) => {
      ...
      fc.assert(
        fc.property(
          ...
          (target, completedCount) => {
            ...
            const sessions: StudySession[] = Array.from(
      ...
      fc.assert(
        fc.property(
          ...
          (target, completedCount, pendingCount) => {
            ...
            const completedTasks: PlannerTask[] = Array.from(
              { length: completedCount },
              (_, i) => ({
                id: `task-c-${i}`,
                studentId: "student-1",
            ...
            const pendingTasks: PlannerTask[] = Array.from(
              { length: pendingCount },
              (_, i) => ({
                id: `task-p-${i}`,
                studentId: "student-1",

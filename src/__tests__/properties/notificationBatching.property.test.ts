        ...
        (studentId, count) => {
          ...
          for (let i = 0; i < count; i++) {
            notifications.push({
              ...
              metadata: null,
              created_at: new Date(
                refTime.getTime() - TWENTY_FOUR_HOURS - (i + 1) * 60_000
              ).toISOString(),
            });
          }
          ...
          expect(hasReachedDailyLimit(notifications, studentId, refTime)).toBe(
            false
          );
        }
      ),
        ...
        fc.array(
          fc.record({
            ...
            created_at: fc
              ...
              .map((offset) =>
                new Date(Date.UTC(2025, 0, 1 + offset)).toISOString()
              ),
          }),

...
describe("1. XP Schedule fault condition", () => {
  ...
  it("XP_SCHEDULE values match domain specification for all affected sources", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...affectedSources), (source: string) => {
...
describe("2. Query key consistency fault condition", () => {
  ...
  it("all dashboard hooks import and use queryKeys factory (not ad-hoc arrays)", () => {
    fc.assert(
      fc.property(fc.constantFrom(...dashboardHookFiles), (relPath: string) => {
...
// Live student_gamification schema uses `streak_current` (confirmed via
// generated database.ts on 2026-04-26). Hooks SHALL read streak_current.
// useStudentDashboard must read attainment_percent (not score_percent).

describe("3. Column name fault condition", () => {
  it("useStudentDashboard reads streak_current from student_gamification", () => {
    const filePath = path.join(srcRoot, "src/hooks/useStudentDashboard.ts");
    const source = fs.readFileSync(filePath, "utf-8");

    // The gamification select query must include streak_current (matches live schema)
    ...
    expect(selectColumns).toContain("streak_current");
  });
  ...
  it("useParentDashboard reads streak_current from student_gamification", () => {
...
describe("4. PostgREST filter sanitization fault condition", () => {
  ...
  it("sanitizePostgrestValue escapes all PostgREST special characters", () => {
    ...
    const stringArb = fc
      .array(charArb, { minLength: 1, maxLength: 50 })
      .map((chars) => chars.join(""));

    fc.assert(
      fc.property(stringArb, (input: string) => {
...
describe("5. Audit logging coverage fault condition", () => {
  const hookFiles: Array<{
  ...
  }> = [
    {
      ...
      label: "useCLOs",
      expectedEntityTypes: ["clo"],
    },
    {
      file: "src/hooks/useAssignments.ts",
      label: "useAssignments",
      expectedEntityTypes: ["assignment"],
    },
    {
      file: "src/hooks/useRubrics.ts",
      label: "useRubrics",
      expectedEntityTypes: ["rubric"],
    },
    {
      file: "src/hooks/useEnrollments.ts",
      label: "useEnrollments",
      expectedEntityTypes: ["enrollment"],
    },
    {
      file: "src/hooks/useGrades.ts",
      label: "useGrades",
      expectedEntityTypes: ["grade"],
    },
    {
      file: "src/hooks/useSubmissions.ts",
      label: "useSubmissions",
      expectedEntityTypes: ["submission"],
    },
  ];

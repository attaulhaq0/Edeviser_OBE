...
function sortLeaderboard(entries: LeaderboardEntry[]): LeaderboardEntry[] {
  return [...entries].sort((a, b) => {
    if (b.current_progress !== a.current_progress) return b.current_progress - a.current_progress;
...
const entryArb = fc.record({
  ...
  completed_at: fc.oneof(
    fc.integer({ min: 1700000000000, max: 1800000000000 }).map((ms) => new Date(ms).toISOString()),
    fc.constant(null),
  ),
      ...
      fc.property(
        ...
        (entries) => {
          ...
          for (let i = 1; i < sorted.length; i++) {
            expect(sorted[i]!.current_progress).toBeLessThanOrEqual(sorted[i - 1]!.current_progress);
          }
        },
      ),
      { numRuns: 100 },
      ...
      fc.property(
        ...
        (teams, courseId) => {
          ...
          for (let i = 1; i < result.length; i++) {
            expect(result[i]!.xp_total).toBeLessThanOrEqual(result[i - 1]!.xp_total);
          }
        },
      ),
      { numRuns: 100 },

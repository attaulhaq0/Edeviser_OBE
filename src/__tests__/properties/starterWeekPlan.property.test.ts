...
import {
  STARTER_SESSION_TIERS,
  SELF_EFFICACY_TIER_THRESHOLDS,
...
function getEfficacyTier(selfEfficacyScore: number): EfficacyTier {
  ...
  if (selfEfficacyScore <= SELF_EFFICACY_TIER_THRESHOLDS.moderate)
    return "moderate";
  return "high";
}
      ...
      fc.property(
        ...
        (selfEfficacy, startDate) => {
          ...
          for (const session of sessions) {
            ...
            const startDateNorm = new Date(
              startDate.toISOString().slice(0, 10) + "T00:00:00Z"
            ).getTime();

...
import {
  ...
  type BaselineResponseInput,
} from "@/lib/scoreCalculator";

const cloIdArb = fc.uuid();

const baselineResponseArb = (
  cloId: string
): fc.Arbitrary<BaselineResponseInput> =>
  fc.record({
...
const baselineResponseSetArb: fc.Arbitrary<BaselineResponseInput[]> = fc
  .array(cloIdArb, { minLength: 1, maxLength: 5 })
  .chain((cloIds) => {
    ...
    fc.assert(
      fc.property(baselineResponseSetArb, (responses) => {
        ...
        for (const result of results) {
          const cloResponses = responses.filter(
            (r) => r.clo_id === result.clo_id
          );
          const correctCount = cloResponses.filter(
            (r) => r.selected_option === r.correct_option
          ).length;
          const expectedScore = Math.round(
            (correctCount / cloResponses.length) * 100
          );
    ...
    fc.assert(
      fc.property(
        ...
        (cloId, answeredCount, unansweredCount) => {
          ...
          // Answered correctly
          const answered: BaselineResponseInput[] = Array.from(
          ...
          const unanswered: BaselineResponseInput[] = Array.from(
          ...
          expect(result!.score).toBe(
            Math.round(
              (answeredCount / (answeredCount + unansweredCount)) * 100
            )
          );
        }
      ),

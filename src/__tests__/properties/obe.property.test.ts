...
function validatePLOWeights(ploId: string, mappings: OutcomeMapping[]): { valid: boolean; totalWeight: number; warning: boolean } {
...
function canLinkCLOToAssignment(cloId: string, mappings: OutcomeMapping[]): boolean {
...
function validateRubricStructure(rubric: RubricStructure): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  if (rubric.criteria_count < 2) errors.push('Minimum 2 criteria required');
  if (rubric.level_count < 2) errors.push('Minimum 2 performance levels required');
...
function calculateWeightedAttainment(
  ...
  const weighted = childAttainments.reduce((sum, c) => sum + c.attainment * c.weight, 0);
  ...
  it('P19a: warns when total weight < 0.5', () => {
    fc.assert(
      fc.property(
        idArb,
        fc.array(
          fc.record({ child_id: fc.constant('plo-1'), parent_id: idArb, weight: fc.double({ min: 0.01, max: 0.1, noNaN: true }) }),
  ...
  it('P20a: CLO with PLO mapping can be linked to assignment', () => {
    fc.assert(
      fc.property(idArb, idArb, weightArb, (cloId, ploId, weight) => {
        const mappings: OutcomeMapping[] = [{ child_id: cloId, parent_id: ploId, weight }];
  ...
  it('P21a: rubric with >= 2 criteria and >= 2 levels is valid', () => {
    fc.assert(
      fc.property(
        ...
        (criteria, levels) => {
          const result = validateRubricStructure({ criteria_count: criteria, level_count: levels });
    ...
    fc.assert(
      fc.property(
        ...
        (children) => {
          ...
          const expected = children.reduce((s, c) => s + c.attainment * c.weight, 0) / totalWeight;

...
function resolveInventoryStatus(
  ...
  if (item.status === 'refunded') return { displayStatus: 'Refunded', isUsable: false };
  if (item.status === 'consumed') return { displayStatus: 'Consumed', isUsable: false };
  if (item.status === 'expired') return { displayStatus: 'Expired', isUsable: false };

  // Active — check if boost has expired
  if (item.boost_expires_at && new Date(item.boost_expires_at).getTime() < now.getTime()) {
...
const statusArb = fc.constantFrom<PurchaseStatus>('active', 'consumed', 'expired', 'refunded');
...
const inventoryItemArb: fc.Arbitrary<InventoryItem> = fc.record({
  ...
  item_sub_category: fc.constantFrom('profile_theme', 'extra_quiz_attempt', 'xp_boost'),
  xp_cost: fc.integer({ min: 1, max: 5_000 }),
  status: statusArb,
  purchased_at: fc.integer({ min: new Date('2024-01-01').getTime(), max: new Date('2025-12-31').getTime() }).map((ts) => new Date(ts).toISOString()),
  boost_expires_at: fc.option(
    fc.integer({ min: new Date('2024-01-01').getTime(), max: new Date('2026-12-31').getTime() }).map((ts) => new Date(ts).toISOString()),
    { nil: null },
  ),
});

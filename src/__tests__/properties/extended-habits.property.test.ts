...
describe("Extended Habit Properties", () => {
  ...
  it("all 8 habit types are recognized", () => {
    fc.assert(
      fc.property(fc.constantFrom(...ALL_HABITS), (habit: HabitType) => {
  ...
  it("extended habits award 15 XP each", () => {
    const EXTENDED_HABITS: HabitType[] = [
      "collaborate",
      "practice",
      "review",
      "mentor",
    ...
    fc.assert(
      fc.property(fc.constantFrom(...EXTENDED_HABITS), (habit) => {
  ...
  it("Perfect Day triggers at 6+ of 8 habits completed", () => {
    ...
    fc.assert(
      fc.property(
        fc.array(fc.constantFrom(...ALL_HABITS), {
          minLength: 0,
          maxLength: 8,
        }),
        (completedHabits) => {

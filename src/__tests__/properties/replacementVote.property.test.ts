  ...
  it('vote expires after 48 hours', () => {
    fc.assert(
      fc.property(voteArb, (vote) => {
        const now = new Date(vote.createdAt.getTime() + (VOTE_EXPIRY_HOURS + 1) * 60 * 60 * 1000);
    ...
    fc.assert(
      fc.property(
        ...
        (totalVoters, hoursElapsed) => {
          ...
          const now = new Date(vote.createdAt.getTime() + hoursElapsed * 60 * 60 * 1000);
    ...
    fc.assert(
      fc.property(
        ...
        (daysAfter) => {
          ...
          const now = new Date(resolved.getTime() + daysAfter * 24 * 60 * 60 * 1000);

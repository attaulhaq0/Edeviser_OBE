...
const dateStringArb = fc.integer({ min: 0, max: 3650 }).map((offset) => {
...
const dateRangeArb = fc
  .tuple(fc.integer({ min: 0, max: 730 }), fc.integer({ min: 1, max: 180 }))
...
const heatmapDaysArb = fc
  .tuple(fc.integer({ min: 0, max: 150 }), fc.integer({ min: 1, max: 120 }))
...
const wellnessHabitTypes: WellnessHabitType[] = [
  "meditation",
  "hydration",
  "exercise",
  "sleep",
...
describe("Habit Heatmap Properties", () => {
  ...
  it("Property 6: single-habit filter produces binary intensity (0 or 1 only)", () => {
    fc.assert(
      fc.property(fc.integer({ min: 0, max: 1 }), (completed) => {

...
const day1PersonalityArb: fc.Arbitrary<PersonalityResponse[]> = fc
  .tuple(
    fc.record({
      dimension: fc.constant("openness"),
      selected_option: fc.integer({ min: 1, max: 5 }),
      weight: fc.constantFrom(1, -1),
    }),
    fc.record({
      dimension: fc.constant("conscientiousness"),
      selected_option: fc.integer({ min: 1, max: 5 }),
      weight: fc.constantFrom(1, -1),
    }),
    fc.record({
      dimension: fc.constant("extraversion"),
      selected_option: fc.integer({ min: 1, max: 5 }),
      weight: fc.constantFrom(1, -1),
    })
  )
...
const day1SelfEfficacyArb: fc.Arbitrary<SelfEfficacyResponseInput[]> = fc
  .tuple(
    fc.record({
      domain: fc.constant("general_academic"),
      selected_option: fc.integer({ min: 1, max: 5 }),
    }),
    fc.record({
      domain: fc.constant("self_regulated_learning"),
      selected_option: fc.integer({ min: 1, max: 5 }),
    })
  )
  .map(([a, b]) => [a, b]);
  ...
  it("P27: Day 1 produces valid preliminary profile with partial scores", () => {
    fc.assert(
      fc.property(
        day1PersonalityArb,
        day1SelfEfficacyArb,
        (personalityResponses, selfEfficacyResponses) => {
          ...
          const selfEfficacy = calculateSelfEfficacyScores(
          ...
          expect(selfEfficacy.self_regulated_learning).toBeGreaterThanOrEqual(
            0
          );

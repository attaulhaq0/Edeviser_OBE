...
type SuggestionType = 'module_suggestion' | 'at_risk_prediction' | 'feedback_draft';
...
function validateAIFeedbackRecord(
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          suggestion_text: fc.string({ minLength: 1, maxLength: 500 }).filter(s => s.trim().length > 0),
          student_id: fc.uuid(),
    ...
    fc.assert(
      fc.property(
        ...
        (record) => {
          ...
          expect(result.errors.some((e) => e.includes('suggestion_text'))).toBe(true);
        },
      ),
      { numRuns: 100 },
    );

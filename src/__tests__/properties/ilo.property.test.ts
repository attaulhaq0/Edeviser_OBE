...
function canDeleteILO(iloId: string, mappings: PLOMapping[]): { allowed: boolean; dependentPLOs: string[] } {
  const dependentPLOs = mappings.filter((m) => m.ilo_id === iloId).map((m) => m.plo_id);
  return { allowed: dependentPLOs.length === 0, dependentPLOs };
}

function reorderILOs(ilos: ILO[], fromIndex: number, toIndex: number): ILO[] {
  if (fromIndex < 0 || fromIndex >= ilos.length || toIndex < 0 || toIndex >= ilos.length) {
    return ilos;
  }
  const result = [...ilos];
...
function checkILOLimit(currentCount: number): { warning: boolean; message?: string } {
  if (currentCount >= ILO_SOFT_LIMIT) {
    return { warning: true, message: `Soft limit of ${ILO_SOFT_LIMIT} ILOs reached` };
  }
  ...
  it('P16a: ILO with mapped PLOs cannot be deleted', () => {
    fc.assert(
      fc.property(
        ...
        (iloId, ploIds) => {
          const mappings: PLOMapping[] = ploIds.map((ploId) => ({ plo_id: ploId, ilo_id: iloId }));
  ...
  it('P16b: ILO with no mapped PLOs can be deleted', () => {
    fc.assert(
      fc.property(
        iloIdArb,
        fc.array(
          fc.record({ plo_id: fc.uuid(), ilo_id: fc.uuid() }),

...
const heatmapDaysArb = fc
  .tuple(fc.integer({ min: 0, max: 150 }), fc.integer({ min: 1, max: 60 }))
...
describe("Habit Analytics Properties", () => {
        ...
        (totalCompleted, possiblePerDay, daysInPeriod) => {
          ...
          if (possiblePerDay === 0 || daysInPeriod === 0) {
            ...
            const expected = Math.min(
              Math.round(
                (totalCompleted / (possiblePerDay * daysInPeriod)) * 100
              ),
              100
            );
  ...
  it("Property 19: computeDayOfWeekAverages returns 7 entries; getBestDay returns highest average", () => {
    fc.assert(
      fc.property(heatmapDaysArb, (days) => {
        ...
        if (bestDay !== null) {
          for (const avg of averages) {
            expect(bestDay.avgCompletions).toBeGreaterThanOrEqual(
              avg.avgCompletions
            );
          }
        }
  ...
  it("Property 20: correlation insights array has at most 3 items with non-causal language", () => {
    const insightArb: fc.Arbitrary<CorrelationInsight> = fc.record({
      id: fc.uuid(),
      habitType: fc.constantFrom(
        "meditation" as const,
        "exercise" as const,
        "login" as const,
        "submit" as const
      ),
      academicMetric: fc.constantFrom(
        "submission_rate",
        "grade_average",
        "attendance"
      ),
    ...
    fc.assert(
      fc.property(
        fc.array(insightArb, { minLength: 0, maxLength: 10 }),
        (allInsights) => {
          // Cap at 3

...
import { buildHeatmapMatrix, getEvidenceCountColor, getAttainmentColor } from '@/lib/coverageHeatmap';
  ...
  it('matrix dimensions match input CLOs and courses', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.record({ id: fc.uuid(), name: fc.string({ minLength: 1 }) }), { minLength: 1, maxLength: 10 }),
        (clos, courses) => {

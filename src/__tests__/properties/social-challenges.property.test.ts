...
const challengeTypeArb = fc.constantFrom('team', 'course_wide') as fc.Arbitrary<'team' | 'course_wide'>;
const goalMetricArb = fc.constantFrom(
  'total_xp', 'habits_completed', 'assignments_submitted', 'quiz_score_avg',
) as fc.Arbitrary<'total_xp' | 'habits_completed' | 'assignments_submitted' | 'quiz_score_avg'>;
const rewardTypeArb = fc.constantFrom('xp_bonus', 'badge') as fc.Arbitrary<'xp_bonus' | 'badge'>;
...
const validChallengeArb = fc.record({
  ...
  course_id: fc.uuid(),
  start_date: fc.integer({ min: 0, max: 500 })
    .map((offset) => {
      const d = new Date('2024-01-01T00:00:00Z');
      d.setUTCDate(d.getUTCDate() + offset);
      return d.toISOString();
    }),
  end_date: fc.integer({ min: 501, max: 1000 })
    .map((offset) => {
      const d = new Date('2024-01-01T00:00:00Z');
      d.setUTCDate(d.getUTCDate() + offset);
      return d.toISOString();
    ...
    fc.assert(
      fc.property(
        ...
        (memberIds, rewardSucceeds) => {
          ...
          expect(rewarded.length === memberIds.length || rewarded.length === 0).toBe(true);
        },
      ),
      { numRuns: 100 },
    );

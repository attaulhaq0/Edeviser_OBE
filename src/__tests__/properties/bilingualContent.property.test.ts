...
import { resolveBilingualContent, createBilingualField } from '@/lib/bilingualContent';

const nonEmptyStringArb = fc.string({ minLength: 1, maxLength: 100 }).filter((s) => s.trim().length > 0).map((s) => s.trim());
...
describe('Property 5 — Bilingual content resolver returns non-empty string when at least one language provided', () => {
  ...
  it('P5b: returns active language version when both are provided', () => {
    fc.assert(
      fc.property(nonEmptyStringArb, nonEmptyStringArb, langArb, (en, ar, lang) => {

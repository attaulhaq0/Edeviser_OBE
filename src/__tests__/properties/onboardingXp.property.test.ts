...
const sectionsArb = fc.subarray(
  ["personality", "learning_style", "self_efficacy", "study_strategy"] as const,
    ...
    fc.assert(
      fc.property(
        ...
        (sections, courseCount) => {
          const transactions = computeOnboardingXp(
            [...sections],
            courseCount,
            1
          );
          ...
          const baselineCount = transactions.filter(
            (t) => t.source === "onboarding_baseline"
          ).length;
          ...
          const actualTotal = transactions.reduce(
            (sum, t) => sum + t.amount,
            0
          );

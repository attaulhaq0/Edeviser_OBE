...
const studyStrategyResponseArb = (
  dimension: string
): fc.Arbitrary<StudyStrategyResponseInput> =>
  fc.record({
...
const fullStudyStrategyArb: fc.Arbitrary<StudyStrategyResponseInput[]> = fc
  .tuple(
    ...SS_DIMENSIONS.map((d) =>
      fc.array(studyStrategyResponseArb(d), { minLength: 2, maxLength: 2 })
    )
  )
  .map((a) => a.flat());

/** Partial set: 0-3 items per dimension */
const partialStudyStrategyArb: fc.Arbitrary<StudyStrategyResponseInput[]> = fc
  .tuple(
    ...SS_DIMENSIONS.map((d) =>
      fc.array(studyStrategyResponseArb(d), { minLength: 0, maxLength: 3 })
    )
  )
  .map((a) => a.flat())

...
function calculateBadgeXPWithSpotlight(
  ...
  const multiplier = calculateSpotlightMultiplier(badgeCategory, spotlightCategory);
  return Math.floor(baseXP * multiplier);
}

// ─── Generators ──────────────────────────────────────────────────────────────

const categoryArb = fc.constantFrom('academic', 'engagement', 'streak', 'wellness', 'social');
    ...
    fc.assert(
      fc.property(
        ...
        (baseXP, badgeCategory, spotlightCategory) => {
          ...
          const xp = calculateBadgeXPWithSpotlight(baseXP, badgeCategory, spotlightCategory);
  ...
  it('spotlight XP is always >= standard XP', () => {
    fc.assert(
      fc.property(
        ...
        fc.oneof(categoryArb, fc.constant(null)),
        (baseXP, badgeCategory, spotlightCategory) => {
          const spotlightXP = calculateBadgeXPWithSpotlight(baseXP, badgeCategory, spotlightCategory);
          const standardXP = calculateBadgeXPWithSpotlight(baseXP, badgeCategory, null);

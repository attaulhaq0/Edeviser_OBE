...
const basePortfolioData = {
  ...
  badges: [
    { badge_key: 'early_bird', badge_name: 'Early Bird', emoji: '🐦', awarded_at: '2024-06-10T10:00:00Z' },
  ],
  journals: [
    { id: 'j1', content_preview: 'Today I learned about polymorphism...', created_at: '2024-06-10T10:00:00Z', course_name: 'CS101' },
  ],
  xpTimeline: [
  ...
  semesterAttainments: [
    { semester_name: 'Fall 2024', average_attainment: 78 },
  ],
  totalXP: 350,
  level: 3,
};
...
describe('StudentPortfolio', () => {
  ...
  it('renders page heading', async () => {
    mockUsePortfolio.mockReturnValue({ data: basePortfolioData, isLoading: false });
  ...
  it('renders journal entries', async () => {
    mockUsePortfolio.mockReturnValue({ data: basePortfolioData, isLoading: false });
    await renderPortfolio();
    expect(screen.getByText('Today I learned about polymorphism...')).toBeInTheDocument();
  });

  it('shows empty messages when no data', async () => {
    mockUsePortfolio.mockReturnValue({
      data: { ...basePortfolioData, clos: [], badges: [], journals: [], xpTimeline: [], semesterAttainments: [] },
      isLoading: false,
    });
    await renderPortfolio();
  ...
  it('renders public profile toggle', async () => {
    mockUsePortfolio.mockReturnValue({ data: basePortfolioData, isLoading: false });
    await renderPortfolio();
    expect(screen.getByLabelText('Toggle public portfolio')).toBeInTheDocument();

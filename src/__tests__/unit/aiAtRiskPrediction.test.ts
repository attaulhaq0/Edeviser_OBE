...
function classifyLoginFrequency(daysSinceLastLogin: number): 'low' | 'medium' | 'high' {
  if (daysSinceLastLogin <= 3) return 'high';
  if (daysSinceLastLogin <= 7) return 'medium';
  return 'low';
}

function scoreAttainmentTrend(trend: 'improving' | 'declining' | 'stagnant'): number {
  switch (trend) {
...
function scoreSubmissionPattern(pattern: 'early' | 'on_time' | 'late' | 'missed'): number {
  switch (pattern) {
    case 'early': return 5;
...
describe('calculateRiskProbability', () => {
  ...
  it('produces high risk for worst-case signals with low attainment', () => {
    ...
    const result = calculateRiskProbability(loginScore, trendScore, patternScore, 20);
  ...
  it('produces low risk for best-case signals with high attainment', () => {
    ...
    const result = calculateRiskProbability(loginScore, trendScore, patternScore, 95);
...
describe('buildPredictionText', () => {
  it('includes student name, CLO title, and probability', () => {
    const text = buildPredictionText('Alice', 'CLO-1: Data Structures', 75, 'high', 'improving', 'on_time');
  ...
  it('includes key signals for low login frequency', () => {
    const text = buildPredictionText('Bob', 'CLO-2', 80, 'low', 'declining', 'missed');
  ...
  it('includes stagnant trend as a signal', () => {
    const text = buildPredictionText('Carol', 'CLO-3', 60, 'medium', 'stagnant', 'late');
  ...
  it('omits signal text when no concerning signals', () => {
    const text = buildPredictionText('Dave', 'CLO-4', 55, 'high', 'improving', 'on_time');
    expect(text).not.toContain('Key signals');
  });

  it('formats probability as percentage in text', () => {
    const text = buildPredictionText('Eve', 'CLO-5', 92, 'low', 'declining', 'missed');
...
describe('signal weight integration', () => {
  it('weights sum to 1.0', () => {
    expect(WEIGHT_LOGIN_FREQUENCY + WEIGHT_ATTAINMENT_TREND + WEIGHT_SUBMISSION_PATTERN).toBe(1.0);
  });

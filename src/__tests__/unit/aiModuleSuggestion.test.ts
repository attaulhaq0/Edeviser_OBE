...
describe("ai-module-suggestion helpers", () => {
  describe("BLOOMS_ORDER", () => {
    ...
    it("should maintain correct ordering", () => {
      ...
      for (let i = 0; i < levels.length - 1; i++) {
        expect(BLOOMS_ORDER[levels[i]!]!).toBeLessThan(
          BLOOMS_ORDER[levels[i + 1]!]!
        );
      }
    });
  });
  ...
  describe("buildSuggestionText", () => {
    ...
    it("should generate fallback text when prereq has no blooms level", () => {
      const result = buildSuggestionText("Evaluate Results", "evaluating", 45, {
        id: "prereq-2",
        title: "Some CLO",
        blooms_level: null,
      });
    ...
    it("should include attainment percentage in all cases", () => {
      const withPrereq = buildSuggestionText("CLO A", "analyzing", 30, {
        id: "1",
        title: "CLO B",

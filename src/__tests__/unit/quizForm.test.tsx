...
vi.mock('@/hooks/useCourses', () => ({
  useCourses: vi.fn().mockReturnValue({
    data: { data: [{ id: 'course-1', code: 'CS101', name: 'Intro to CS' }], count: 1 },
    isLoading: false,
  }),
}));
...
vi.mock('@/hooks/useCLOs', () => ({
  useCLOs: vi.fn().mockReturnValue({
    data: { data: [{ id: 'clo-1', title: 'CLO 1: Understand basics' }], count: 1 },
    isLoading: false,
  }),
}));
...
const createWrapper = (initialRoute = '/teacher/courses/course-1/quizzes/new') => {
  ...
  return ({ children }: { children: React.ReactNode }) => (
    <QueryClientProvider client={queryClient}>
      <MemoryRouter initialEntries={[initialRoute]}>
        <Routes>
          <Route path="/teacher/courses/:courseId/quizzes/new" element={children} />
          <Route path="/teacher/courses/:courseId/quizzes/:id/edit" element={children} />
        </Routes>
      </MemoryRouter>
    </QueryClientProvider>
  );
...
describe('QuizForm — Adaptive mode toggle', () => {
  ...
  it('shows adaptation config fields when adaptive is toggled on', () => {
    render(<QuizForm />, { wrapper: createWrapper() });
    const adaptiveToggle = screen.getByRole('switch', { name: /enable adaptive mode/i });

...
describe('useCLOEvidence — queryFn logic', () => {
  ...
  it('queries evidence table filtered by clo_id and student_id', () => {
    ...
    expect(mockSelect).toHaveBeenCalledWith('id, score_percent, created_at, submission_id');
  ...
  it('maps evidence rows to CLOEvidenceRecord shape', () => {
    ...
    const evidenceRows = [
      { id: 'ev-1', score_percent: 85, created_at: '2025-01-15T10:00:00Z', submission_id: 'sub-1' },
      { id: 'ev-2', score_percent: 72, created_at: '2025-01-10T10:00:00Z', submission_id: 'sub-2' },
    ];
    ...
    expect(result).toEqual([
      { id: 'ev-1', assignment_title: 'Midterm Essay', score_percent: 85, created_at: '2025-01-15T10:00:00Z' },
      { id: 'ev-2', assignment_title: 'Lab Report 3', score_percent: 72, created_at: '2025-01-10T10:00:00Z' },
    ]);
  });

  it('returns "Unknown Assignment" when assignment lookup fails', () => {
    const evidenceRows = [
      { id: 'ev-1', score_percent: 60, created_at: '2025-01-15T10:00:00Z', submission_id: 'sub-missing' },
    ];

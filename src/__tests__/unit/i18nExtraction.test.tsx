...
vi.mock('@/hooks/useAdminDashboard', () => ({
  useAdminKPIs: () => ({
    data: { totalUsers: 42, activeUsers: 30, totalPrograms: 5, totalCourses: 12, usersByRole: { admin: 2, teacher: 10, student: 30 } },
    isLoading: false,
  }),
  useRecentAuditLogs: () => ({ data: [], isLoading: false }),
  useOnboardingAnalytics: () => ({ data: { completionRate: 100, totalStudents: 30, completedOnboarding: 30 } }),
}));

vi.mock('@/hooks/useAIPerformance', () => ({
  useAIPerformance: () => ({
    data: { suggestionAcceptanceRate: 75, suggestionTotal: 100, predictionAccuracyRate: 80, predictionTotal: 50, draftAcceptanceRate: 60, draftTotal: 20 },
    isLoading: false,
  }),
}));
...
vi.mock('@/hooks/useTeacherDashboard', () => ({
  useTeacherKPIs: () => ({
    data: { pendingSubmissions: 5, gradedThisWeek: 12, avgAttainment: 72, atRiskCount: 3 },
    isLoading: false,
  }),
...
vi.mock('@/hooks/useStudentDashboard', () => ({
  useStudentKPIs: () => ({
    data: { enrolledCourses: 4, completedAssignments: 8, avgAttainment: 78, currentStreak: 5, totalXP: 1200, currentLevel: 3 },
    isLoading: false,
  }),
...
vi.mock('@/hooks/useProfileCompleteness', () => ({
  useProfileCompleteness: () => ({ data: { profile_completeness: 100, day1_completed: true } }),
}));

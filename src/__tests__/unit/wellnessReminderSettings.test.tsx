...
const makeReminder = (overrides: Partial<WellnessReminderConfig> = {}): WellnessReminderConfig => ({
...
describe('WellnessReminderSettings', () => {
  const defaultProps = {
    reminders: [
      makeReminder({ habitType: 'meditation', reminderTime: '08:00', enabled: true }),
      makeReminder({ habitType: 'hydration', reminderTime: null, enabled: false }),
    ],
    onToggle: vi.fn(),
    onTimeChange: vi.fn(),
  };
  ...
  it('renders the reminders container', () => {
    ...
    expect(screen.getByTestId('wellness-reminder-settings')).toBeInTheDocument();
  });
  ...
  it('renders nothing when reminders array is empty', () => {
    const { container } = render(
      <WellnessReminderSettings reminders={[]} onToggle={vi.fn()} onTimeChange={vi.fn()} />,
    );
  ...
  it('shows correct time value for enabled reminder', () => {
    render(<WellnessReminderSettings {...defaultProps} />);
    const timeInput = screen.getByTestId('reminder-time-meditation') as HTMLInputElement;

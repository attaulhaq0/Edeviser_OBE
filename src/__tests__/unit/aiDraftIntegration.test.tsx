...
const mockDraftResponse = {
  criterion_drafts: [
    {
      criterion_id: "c1",
      criterion_title: "Clarity",
      draft_comment: "Excellent work on Clarity.",
    },
    {
      criterion_id: "c2",
      criterion_title: "Grammar",
      draft_comment: "Grammar needs improvement.",
    },
  ],
...
const renderGradingInterface = async () => {
  const GradingInterface = (
    await import("@/pages/teacher/grading/GradingInterface")
  ).default;
  return render(
...
const selectRubricCells = async (user: ReturnType<typeof userEvent.setup>) => {
  const clarityBtns = screen.getAllByRole("button", {
    name: /Clarity: Exemplary — 50 points/,
  });
  await user.click(clarityBtns[0]!);
  const grammarBtns = screen.getAllByRole("button", {
    name: /Grammar: Proficient — 30 points/,
  });
...
describe("AI Draft Integration in GradingInterface", () => {
  ...
  it("displays AI draft panel after successful generation", async () => {
    ...
    mockGenerateMutate.mockImplementation(
      (
        _input: unknown,
        opts: { onSuccess: (data: typeof mockDraftResponse) => void }
      ) => {
    ...
    expect(
      screen.getByText("Overall, good submission with areas for improvement.")
    ).toBeInTheDocument();
  }, 15_000);
  ...
  it("shows accept/edit/reject controls for pending drafts", async () => {
    ...
    expect(
      screen.getByLabelText("Accept draft for Clarity")
    ).toBeInTheDocument();
    expect(screen.getByLabelText("Edit draft for Clarity")).toBeInTheDocument();
    expect(
      screen.getByLabelText("Reject draft for Clarity")
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Accept draft for Grammar")
    ).toBeInTheDocument();
  });
  ...
  it("accepts a draft and populates feedback", async () => {
    ...
    expect((textarea as HTMLTextAreaElement).value).toContain(
      "[Clarity]: Excellent work on Clarity."
    );
  });
  ...
  it("applies overall draft to feedback textarea when Use button is clicked", async () => {
    ...
    expect(textarea).toHaveValue(
      "Overall, good submission with areas for improvement."
    );
    expect(mockToast.success).toHaveBeenCalledWith(
      "Overall draft applied to feedback"
    );
  });
  ...
  it("shows error toast when draft generation fails", async () => {
    ...
    expect(mockToast.error).toHaveBeenCalledWith(
      "Draft generation failed: Network error"
    );
  });

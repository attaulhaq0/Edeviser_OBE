...
describe('AIAtRiskWidget', () => {
  ...
  it('renders prediction rows with student names and CLO titles', () => {
    ...
    expect(screen.getByText(/Recall fundamental algorithms/)).toBeInTheDocument();
  });
  ...
  it('renders contributing signal badges', () => {
    ...
    expect(screen.getAllByText('Submissions: late').length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText('Trend: declining').length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('pre-fills nudge message with student name and CLO', () => {
    ...
    const nudgeButtons = screen.getAllByRole('button', { name: /send nudge/i });
    fireEvent.click(nudgeButtons[0]!);
    const textarea = screen.getByPlaceholderText(/Write a personalized message/);
    expect((textarea as HTMLTextAreaElement).value).toContain('Alice');
    expect((textarea as HTMLTextAreaElement).value).toContain('Apply data structures');
  });
  ...
  it('calls nudge mutation when Send Nudge is confirmed', async () => {
    ...
    const dialogSendButton = screen.getByRole('button', { name: /^Send Nudge$/i });

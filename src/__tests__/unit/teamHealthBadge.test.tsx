...
describe('TeamHealthBadge', () => {
  ...
  it('boundary: score 70 is healthy', () => {
    ...
    expect(screen.getByTestId('team-health-badge').getAttribute('data-status')).toBe('healthy');
  });

  it('boundary: score 69 is needs_attention', () => {
    render(<TeamHealthBadge score={69} />);
    expect(screen.getByTestId('team-health-badge').getAttribute('data-status')).toBe('needs_attention');
  });
  ...
  it('boundary: score 39 is at_risk', () => {
    ...
    expect(screen.getByTestId('team-health-badge').getAttribute('data-status')).toBe('at_risk');
  });

...
import { institutionSettingsSchema, gradeScaleSchema } from '@/lib/schemas/institutionSettings';
...
describe('institutionSettingsSchema', () => {
  ...
  it('rejects thresholds above 100', () => {
    const result = institutionSettingsSchema.safeParse({
      ...validSettings,
      attainment_thresholds: { excellent: 101, satisfactory: 70, developing: 50 },
    });
    expect(result.success).toBe(false);
  });
  ...
  it('rejects negative thresholds', () => {
    const result = institutionSettingsSchema.safeParse({
      ...validSettings,
      attainment_thresholds: { excellent: 85, satisfactory: -1, developing: 50 },
    });
    expect(result.success).toBe(false);
  });

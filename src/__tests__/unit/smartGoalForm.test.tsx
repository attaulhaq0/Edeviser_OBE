...
vi.mock("@/components/ui/select", () => ({
  Select: ({
    ...
    return (
      <div data-testid="mock-select-root" data-value={defaultValue || ""}>
        {children}
      </div>
    );
  },
  ...
  SelectItem: ({
    ...
    <div
      ...
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") mockSelectOnChange?.(value);
      }}
      tabIndex={0}
    >
      {children}
...
describe("SmartGoalForm", () => {
  ...
  describe("rendering", () => {
    ...
    it("renders course options in the Relevant select dropdown", () => {
      renderForm();
      expect(
        screen.getByTestId("select-item-Mathematics 101")
      ).toBeInTheDocument();
      expect(screen.getByTestId("select-item-Physics 201")).toBeInTheDocument();
      expect(
        screen.getByTestId("select-item-Chemistry 301")
      ).toBeInTheDocument();
    });
  });
  ...
  describe("form submission", () => {
    it("calls onSubmit with composed text on valid form submission", async () => {
      ...
      renderForm({
        onSubmit,
        defaultRelevant: "Mathematics 101",
        defaultTimebound: "2025-06-30",
      });

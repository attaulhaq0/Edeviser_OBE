...
const mockDiminishingData = { repeat_count: 0, next_multiplier: 1.0, is_diminished: false };
...
import { useStudentXPMultiplier, useDiminishingReturnsStatus } from '@/hooks/useAdaptiveXP';

describe('AdaptiveXPDisplay', () => {
  beforeEach(() => {
    vi.mocked(useStudentXPMultiplier).mockReturnValue({ data: mockMultiplierData } as ReturnType<typeof useStudentXPMultiplier>);
    vi.mocked(useDiminishingReturnsStatus).mockReturnValue({ data: mockDiminishingData } as ReturnType<typeof useDiminishingReturnsStatus>);
  });
  ...
  it('renders nothing when multiplier data is null', () => {
    vi.mocked(useStudentXPMultiplier).mockReturnValue({ data: undefined } as ReturnType<typeof useStudentXPMultiplier>);
  ...
  it('passes actionType to diminishing returns hook', () => {
    ...
    expect(useDiminishingReturnsStatus).toHaveBeenCalledWith('s1', 'quiz_completion');
  });

  it('defaults actionType to submission', () => {
    render(<AdaptiveXPDisplay studentId="s1" />);
    expect(useDiminishingReturnsStatus).toHaveBeenCalledWith('s1', 'submission');
  });
});

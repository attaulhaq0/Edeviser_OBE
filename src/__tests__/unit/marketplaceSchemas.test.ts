...
describe('createMarketplaceItemSchema', () => {
  ...
  it('rejects empty name', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, name: '' });
    expect(result.success).toBe(false);
  });

  it('rejects name exceeding 100 characters', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, name: 'a'.repeat(101) });
    expect(result.success).toBe(false);
  });

  it('rejects empty description', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, description: '' });
    expect(result.success).toBe(false);
  });

  it('rejects description exceeding 500 characters', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, description: 'a'.repeat(501) });
    expect(result.success).toBe(false);
  });

  it('rejects invalid category', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, category: 'invalid' });
    expect(result.success).toBe(false);
  });

  it('rejects invalid sub_category', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, sub_category: 'invalid' });
    expect(result.success).toBe(false);
  });

  it('rejects zero xp_price', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, xp_price: 0 });
    expect(result.success).toBe(false);
  });

  it('rejects negative xp_price', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, xp_price: -100 });
    expect(result.success).toBe(false);
  });

  it('rejects non-integer xp_price', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, xp_price: 99.5 });
    expect(result.success).toBe(false);
  });

  it('rejects negative level_requirement', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, level_requirement: -1 });
    expect(result.success).toBe(false);
  });

  it('rejects invalid stock_type', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, stock_type: 'rare' });
    expect(result.success).toBe(false);
  });

  it('rejects empty icon_identifier', () => {
    const result = createMarketplaceItemSchema.safeParse({ ...validItem, icon_identifier: '' });
    expect(result.success).toBe(false);
  });
...
describe('createSaleEventSchema', () => {
  ...
  it('rejects discount below 5', () => {
    const result = createSaleEventSchema.safeParse({ ...validSale, discount_percentage: 4 });
    expect(result.success).toBe(false);
  });

  it('rejects discount above 90', () => {
    const result = createSaleEventSchema.safeParse({ ...validSale, discount_percentage: 91 });
    expect(result.success).toBe(false);
  });
  ...
  it('rejects empty item_ids array', () => {
    const result = createSaleEventSchema.safeParse({ ...validSale, item_ids: [] });
    expect(result.success).toBe(false);
  });

  it('rejects non-UUID item_ids', () => {
    const result = createSaleEventSchema.safeParse({ ...validSale, item_ids: ['not-a-uuid'] });
    expect(result.success).toBe(false);
  });

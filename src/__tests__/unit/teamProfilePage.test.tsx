...
const mockProfile = {
  ...
  members: [
    { id: 'm1', student_id: 'user-1', role: 'captain', contribution_status: 'active' },
    { id: 'm2', student_id: 'user-2', role: 'member', contribution_status: 'warning' },
  ],
  badges: [
...
vi.mock('@/components/shared/CooperationScoreDisplay', () => ({
  default: ({ score }: { score: number }) => <div data-testid="cooperation-score">{score}</div>,
}));
...
vi.mock('@/components/shared/ChallengeProgressBar', () => ({
  default: ({ current, goal }: { current: number; goal: number }) => (
    <div data-testid="challenge-progress" role="progressbar" aria-valuenow={current} aria-valuemax={goal} />
  ),
}));

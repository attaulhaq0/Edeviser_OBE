...
vi.mock("@/hooks/useCLOs", () => ({
  useCLOs: (courseId?: string) => ({
    data:
      courseId === "c1"
        ? { data: mockCLOs, count: 1, page: 1, pageSize: 20 }
  ...
  generateJournalPrompt: () => ({
    ...
    questions: [
      { stage: "Reflective Observation", question: "What did you find easy?" },
      {
        ...
        question: "What principle connects?",
      },
    ],
  }),
}));
...
describe("JournalEditor", () => {
  ...
  it("shows contextual prompt when CLO is selected", async () => {
    ...
    const mathItem = mathOptions.find((el) =>
      el.closest('[data-slot="select-item"]')
    );
    ...
    // Select a CLO
    const cloTrigger = screen
      .getByText("Select a CLO for guided prompts")
      .closest("button");
    fireEvent.click(cloTrigger!);
    const cloOptions = screen.getAllByText("Understand derivatives");
    const cloItem = cloOptions.find((el) =>
      el.closest('[data-slot="select-item"]')
    );
    ...
    expect(
      screen.getByText("Reflect on your understanding of derivatives.")
    ).toBeInTheDocument();

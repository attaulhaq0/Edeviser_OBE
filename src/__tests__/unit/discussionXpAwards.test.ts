...
const mockAwardXP = vi.fn().mockResolvedValue({ success: true, xp_awarded: 10, new_total: 100, level_up: false, new_level: 2 });
...
describe('Discussion XP Awards (Task 67.4)', () => {
  beforeEach(() => {
    ...
    mockSingle.mockResolvedValue({ data: { id: 'thread-1', course_id: 'c-1', title: 'Q', content: 'body', author_id: 'student-1', is_pinned: false, is_resolved: false, created_at: '2024-01-01', updated_at: '2024-01-01' }, error: null });
  });
  ...
  describe('Requirement 77.4: Thread creation awards 10 XP', () => {
    it('calls awardXP with discussion_question source and 10 XP on thread creation', async () => {
      ...
      const variables = { course_id: 'c-1', author_id: 'student-1', title: 'Q', content: 'body' };

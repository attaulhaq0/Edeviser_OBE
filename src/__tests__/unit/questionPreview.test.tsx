...
describe("QuestionPreview — MCQ", () => {
  it("renders question text", () => {
    renderPreview();
    expect(
      screen.getByText("What data structure uses FIFO?")
    ).toBeInTheDocument();
  });
  ...
  it("shows correct/incorrect indicators when showCorrectAnswer is true", () => {
    ...
    const radios = screen.getAllByRole("radio");
    const correctOption = radios.find(
      (r) => r.getAttribute("aria-label") === "B. Array"
    );
    expect(correctOption?.className).toContain("green");
    // The incorrect selected option (A) should have red styling
    const incorrectOption = radios.find(
      (r) => r.getAttribute("aria-label") === "A. Linked list"
    );

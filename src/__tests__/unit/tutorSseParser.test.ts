...
describe('parseSSELine', () => {
  ...
  it('parses a citations event', () => {
    ...
    const line = `data: ${JSON.stringify({ type: 'citations', data: citations })}`;
    const result = parseSSELine(line);
  ...
  it('parses a plan_update event', () => {
    ...
    const line = `data: ${JSON.stringify({ type: 'plan_update', data: planData })}`;
    const result = parseSSELine(line);
  ...
  it('parses an independence_nudge event', () => {
    ...
    const line = `data: ${JSON.stringify({ type: 'independence_nudge', data: nudgeData })}`;
    const result = parseSSELine(line);
    expect(result).toEqual({ type: 'independence_nudge', data: nudgeData });
  });

  it('parses a handoff_suggestion event', () => {
    const handoffData = { reason: 'low_rag_confidence', message: 'Consider asking your teacher.' };
    const line = `data: ${JSON.stringify({ type: 'handoff_suggestion', data: handoffData })}`;
    const result = parseSSELine(line);

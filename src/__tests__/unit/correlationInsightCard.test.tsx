...
const makeInsight = (overrides: Partial<CorrelationInsight> = {}): CorrelationInsight => ({
  id: 'test-1',
...
describe('CorrelationInsightCard', () => {
  ...
  it('renders the strength percentage', () => {
    render(<CorrelationInsightCard insight={makeInsight({ strength: 0.85 })} />);
  ...
  it('renders with correct test id', () => {
    ...
    expect(screen.getByTestId('correlation-insight-abc-123')).toBeInTheDocument();
  });

  it('renders icon for meditation habit type', () => {
    const { container } = render(
      <CorrelationInsightCard insight={makeInsight({ habitType: 'meditation' })} />,
    );
  ...
  it('renders icon for exercise habit type', () => {
    const { container } = render(
      <CorrelationInsightCard insight={makeInsight({ habitType: 'exercise' })} />,
    );

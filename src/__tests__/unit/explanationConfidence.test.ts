...
describe("computeExplanationConfidence", () => {
  ...
  it("uses only the top 3 values when given more than 3", () => {
    ...
    const result = computeExplanationConfidence([0.5, 0.85, 0.95, 0.3, 0.8]);
    expect(result).toBeCloseTo((0.95 + 0.85 + 0.8) / 3, 10);
  });

  it("returns the correct value when all similarities are the same", () => {
    expect(computeExplanationConfidence([0.6, 0.6, 0.6, 0.6])).toBeCloseTo(
      0.6,
      10
    );
  });

...
const makeCLO = (overrides: Partial<CLOAttainment> = {}): CLOAttainment => ({
  clo_id: "clo-1",
  ...
  it("includes CLO progress and competency gaps for all CLOs", () => {
    const cloAttainments: CLOAttainment[] = [
      makeCLO({
        clo_id: "clo-1",
        clo_title: "CLO X",
        attainment_percentage: 40,
      }),
      makeCLO({
        clo_id: "clo-2",
        clo_title: "CLO Y",
        attainment_percentage: 55,
      }),
    ];
    const prompt = assembleSystemPrompt({ ...baseInput, cloAttainments });
    expect(prompt).toContain("STUDENT CLO PROGRESS");
...
describe("assembleSystemPrompt — mixed CLOs", () => {
  it("marks CLOs below 70% as gaps and above 70% as passing", () => {
    const cloAttainments: CLOAttainment[] = [
      makeCLO({
        clo_id: "clo-1",
        clo_title: "Passing CLO",
        attainment_percentage: 80,
      }),
      makeCLO({
        clo_id: "clo-2",
        clo_title: "Failing CLO",
        attainment_percentage: 45,
      }),
    ];
    const prompt = assembleSystemPrompt({ ...baseInput, cloAttainments });
    expect(prompt).toContain("STUDENT CLO PROGRESS");
...
describe("assembleSystemPrompt — retrieved chunks", () => {
  it("includes chunk text and source info", () => {
    const chunks: RetrievedChunk[] = [
      makeChunk({
        chunk_text: "Linked lists use pointers.",
        source_filename: "ch2.pdf",
      }),
      makeChunk({
        chunk_text: "Trees are hierarchical.",
        source_filename: "ch3.pdf",
      }),
    ];
    const prompt = assembleSystemPrompt({
      ...baseInput,

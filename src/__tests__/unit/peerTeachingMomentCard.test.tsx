...
const defaultProps = {
  ...
  explanationText:
    "Recursion is when a function calls itself. The key is to have a base case that stops the recursion.",
  authorName: "Alice",
...
describe("PeerTeachingMomentCard", () => {
  ...
  it("renders average clarity rating", () => {
    render(<PeerTeachingMomentCard {...defaultProps} avgClarityRating={4.5} />);
    expect(screen.getByText(/Clarity: 4.5/)).toBeDefined();
  });

  it("renders average helpfulness rating", () => {
    render(
      <PeerTeachingMomentCard {...defaultProps} avgHelpfulnessRating={3.8} />
    );
    expect(screen.getByText(/Helpful: 3.8/)).toBeDefined();
  });

  it("renders view count", () => {
    render(<PeerTeachingMomentCard {...defaultProps} viewCount={42} />);
  ...
  it("renders CLO title badge when provided", () => {
    render(<PeerTeachingMomentCard {...defaultProps} cloTitle="CLO 1.1" />);

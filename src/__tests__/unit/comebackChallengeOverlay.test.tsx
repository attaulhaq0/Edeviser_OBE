...
import type { HeatmapDay, DateRange, ComebackChallengeStatus } from '@/types/habits';
...
describe('Comeback Challenge Overlay', () => {
  ...
  it('renders dashed teal border overlay on comeback challenge days', () => {
    ...
    expect(screen.getByTestId('comeback-overlay-2025-01-08')).toBeInTheDocument();
    expect(screen.getByTestId('comeback-overlay-2025-01-09')).toBeInTheDocument();
    expect(screen.getByTestId('comeback-overlay-2025-01-10')).toBeInTheDocument();

    // Day 7 and 11 should NOT have overlay
    expect(screen.queryByTestId('comeback-overlay-2025-01-07')).not.toBeInTheDocument();
    expect(screen.queryByTestId('comeback-overlay-2025-01-11')).not.toBeInTheDocument();
  });
  ...
  it('does not render overlay when comeback challenge is inactive', () => {
    ...
    expect(screen.queryByTestId('comeback-overlay-2025-01-08')).not.toBeInTheDocument();
  });
...
describe('HeatmapTooltip — Comeback Day label', () => {
  it('shows "Comeback Day N/3" label when isComebackDay is true', () => {
    ...
    expect(screen.getByTestId('tooltip-comeback-label')).toHaveTextContent('Comeback Day 2/3');
  });
  ...
  it('does not show comeback label when isComebackDay is false', () => {
    ...
    expect(screen.queryByTestId('tooltip-comeback-label')).not.toBeInTheDocument();
  });

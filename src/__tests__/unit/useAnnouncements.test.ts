...
describe('useAnnouncements hooks — queryFn / mutationFn logic', () => {
  beforeEach(() => {
    ...
    mockThen.mockImplementation((cb: (v: unknown) => void) => { cb({ error: null }); return Promise.resolve(); });
  });
  ...
  describe('useAnnouncements queryFn', () => {
    ...
    it('orders by is_pinned DESC then created_at DESC', () => {
      ...
      chain.select('id, course_id, author_id, title, content, is_pinned, created_at, updated_at');
  ...
  describe('useUpdateAnnouncement mutationFn', () => {
    it('updates announcement by id', async () => {
      ...
      expect(mockUpdate).toHaveBeenCalledWith({ title: 'Updated', is_pinned: true });
    });
  });

...
describe('useBloomsProgression hooks — queryFn logic', () => {
  ...
  describe('useBloomsProgression queryFn', () => {
    it('queries blooms_progression for a student in a course with specific columns', () => {
      ...
      expect(mockOrder).toHaveBeenCalledWith('highest_bloom_level', { ascending: false });
    });

    it('returns progression records array', async () => {
      const rows = [
        sampleProgression,
        { ...sampleProgression, id: 'prog-2', clo_id: 'clo-2', highest_bloom_level: 2 },
      ];
      mockOrder.mockResolvedValue({ data: rows, error: null });

      const result = await chainObj.order('highest_bloom_level', { ascending: false });
  ...
  describe('useBloomsClimbState queryFn', () => {
    it('queries quiz_attempts for blooms_climb_state by attempt id', () => {
      mockMaybeSingle.mockResolvedValue({ data: sampleClimbState, error: null });
  ...
  describe('useBloomsPioneerBadges queryFn', () => {
    ...
    it('returns only rows with at least one badge awarded', async () => {
      ...
      const result = await chainObj.or('bloom_explorer_awarded.eq.true,bloom_challenger_awarded.eq.true,bloom_pioneer_awarded.eq.true');
      expect(result.data).toHaveLength(2);

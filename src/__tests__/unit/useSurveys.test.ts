...
describe('useSurveys hooks — queryFn / mutationFn logic', () => {
  ...
  describe('useSurveys queryFn', () => {
    ...
    it('calls select with specific columns and order', () => {
      ...
      chain.select('id, institution_id, title, type, target_outcomes, is_active, created_at');
  ...
  describe('useSurvey queryFn', () => {
    it('queries surveys by id with maybeSingle', async () => {
      ...
      mockMaybeSingle.mockResolvedValue({
        data: { id, title: 'Test Survey', type: 'course_exit', target_outcomes: [], is_active: true },
        error: null,
      });
      ...
      expect(result.data).toEqual(expect.objectContaining({ id, title: 'Test Survey' }));
    });
  });
  ...
  describe('useSurveyQuestions queryFn', () => {
    it('queries survey_questions by survey_id ordered by sort_order', () => {
      ...
      chain.select('id, survey_id, question_text, question_type, options, sort_order');
  ...
  describe('useSurveyResponses queryFn', () => {
    it('queries survey_responses by survey_id', () => {
      ...
      chain.select('id, survey_id, question_id, respondent_id, response_value, created_at');
      chain.eq('survey_id', surveyId);
      chain.order('created_at', { ascending: false });
  ...
  describe('useCreateSurvey mutationFn', () => {
    it('inserts into surveys and logs audit event', async () => {
      ...
      expect(data).toEqual(expect.objectContaining({ id: 'created-survey-id' }));
      ...
      await logAuditEvent({
        ...
        entity_type: 'survey',
        entity_id: data!.id,
        changes: { title: newSurvey.title, type: newSurvey.type, is_active: newSurvey.is_active },
      ...
      expect(logAuditEvent).toHaveBeenCalledWith({
        ...
        changes: { title: 'Course Exit Survey', type: 'course_exit', is_active: true },
        performed_by: 'admin-user-id',
      });
    });
  });
  ...
  describe('useUpdateSurvey mutationFn', () => {
    it('updates survey and logs audit event', async () => {
      ...
      mockSingle.mockResolvedValue({ data: { id: surveyId, ...changes }, error: null });
  ...
  describe('useSubmitSurveyResponse mutationFn', () => {
    it('inserts batch of responses into survey_responses', async () => {
      const rows = [
        { survey_id: 's-1', question_id: 'q-1', respondent_id: 'student-1', response_value: 'Agree' },
        { survey_id: 's-1', question_id: 'q-2', respondent_id: 'student-1', response_value: 'Good course' },
      ];
  ...
  describe('error handling', () => {
    ...
    it('returns error when insert fails', async () => {
      ...
      const chain = supabase.from('survey_responses');
      chain.insert({ survey_id: 's-1', question_id: 'q-1', respondent_id: 'stu-1', response_value: 'x' });
      chain.select();
      const { error } = await chain.single();

...
describe("CorrelationConfidenceBadge", () => {
  ...
  levels.forEach(({ level, label, colorClass }) => {
    ...
    it(`has correct test id for ${level}`, () => {
      ...
      expect(
        screen.getByTestId(`confidence-badge-${level}`)
      ).toBeInTheDocument();
    });
  ...
  it("displays the correct day count", () => {
    render(
      <CorrelationConfidenceBadge level="strong_pattern" dataPointCount={120} />
    );
...
describe("CorrelationDisclaimer", () => {
  it("renders the disclaimer text", () => {
    ...
    expect(
      screen.getByText(
        /These insights show patterns in your data, not cause-and-effect relationships/
      )
    ).toBeInTheDocument();
  });

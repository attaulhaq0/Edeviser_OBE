...
describe("Sabbatical Rest Day Overlay", () => {
  ...
  it("renders diagonal stripe overlay on Saturday and Sunday when sabbatical is enabled", () => {
    ...
    render(
      <HeatmapGrid data={days} semesterRange={range} sabbaticalEnabled={true} />
    );

    // Saturday Jan 11 and Sunday Jan 12 should have sabbatical overlay
    expect(
      screen.getByTestId("sabbatical-overlay-2025-01-11")
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("sabbatical-overlay-2025-01-12")
    ).toBeInTheDocument();

    // Saturday Jan 18 and Sunday Jan 19 should also have overlay
    expect(
      screen.getByTestId("sabbatical-overlay-2025-01-18")
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("sabbatical-overlay-2025-01-19")
    ).toBeInTheDocument();

    // Weekday should NOT have overlay
    expect(
      screen.queryByTestId("sabbatical-overlay-2025-01-06")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("sabbatical-overlay-2025-01-07")
    ).not.toBeInTheDocument();
  });
  ...
  it("does not render sabbatical overlay when sabbaticalEnabled is false", () => {
    ...
    expect(
      screen.queryByTestId("sabbatical-overlay-2025-01-11")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("sabbatical-overlay-2025-01-12")
    ).not.toBeInTheDocument();
  });
...
describe("HeatmapTooltip — Sabbatical Rest Day label", () => {
  it('shows "Rest Day (Sabbatical)" label when isSabbaticalRestDay is true', () => {
    ...
    expect(screen.getByTestId("tooltip-sabbatical-label")).toHaveTextContent(
      "Rest Day (Sabbatical)"
    );
  });
  ...
  it("does not show sabbatical label when isSabbaticalRestDay is false", () => {
    ...
    expect(
      screen.queryByTestId("tooltip-sabbatical-label")
    ).not.toBeInTheDocument();
  });
});

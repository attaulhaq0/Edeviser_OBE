...
describe('HeatmapFilters', () => {
  ...
  it('renders enabled wellness habits as additional filters', () => {
    ...
    expect(screen.getByTestId('filter-meditation')).toHaveTextContent('Meditation');
  ...
  it('renders the active filter based on current query state', () => {
    mockFilter = 'login';
    render(
      <HeatmapFilters enabledWellnessHabits={[]} />,
    );
  ...
  it('has 7 filters with 2 wellness habits enabled', () => {
    render(
      <HeatmapFilters enabledWellnessHabits={['meditation', 'sleep']} />,
    );
...
describe('HeatmapSummaryStats', () => {
  ...
  it('displays correct values', () => {
    ...
    expect(screen.getByTestId('kpi-current-streak-value')).toHaveTextContent('7');
    expect(screen.getByTestId('kpi-longest-streak-value')).toHaveTextContent('21');
    expect(screen.getByTestId('kpi-total-active-days-value')).toHaveTextContent('60');
  });
  ...
  it('displays zero values correctly', () => {
    ...
    expect(screen.getByTestId('kpi-current-streak-value')).toHaveTextContent('0');
    expect(screen.getByTestId('kpi-longest-streak-value')).toHaveTextContent('0');
    expect(screen.getByTestId('kpi-total-active-days-value')).toHaveTextContent('0');
  });

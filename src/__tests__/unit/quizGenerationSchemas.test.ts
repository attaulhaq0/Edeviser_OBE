...
describe("generateQuestionsSchema", () => {
  ...
  it("accepts question_count at boundaries (1 and 50)", () => {
    expect(
      generateQuestionsSchema.safeParse({ ...validInput, question_count: 1 })
        .success
    ).toBe(true);
    expect(
      generateQuestionsSchema.safeParse({ ...validInput, question_count: 50 })
        .success
    ).toBe(true);
  });
  ...
  it("rejects missing required fields", () => {
    ...
    expect(
      generateQuestionsSchema.safeParse({ course_id: validUUID }).success
    ).toBe(false);
  });
});
...
describe("questionBankEntrySchema", () => {
  const validEntry = {
    ...
    options: [
      {
        key: "A",
        text: "Translate source code to machine code",
        is_correct: true,
      },
  ...
  it("accepts difficulty_rating at boundaries (1.0 and 5.0)", () => {
    expect(
      questionBankEntrySchema.safeParse({
        ...validEntry,
  ...
  it("accepts bloom_level at boundaries (1 and 6)", () => {
    expect(
      questionBankEntrySchema.safeParse({ ...validEntry, bloom_level: 1 })
        .success
    ).toBe(true);
    expect(
      questionBankEntrySchema.safeParse({ ...validEntry, bloom_level: 6 })
        .success
    ).toBe(true);
  });
...
describe("adaptiveQuizConfigSchema", () => {
  ...
  it("accepts initial_difficulty at boundaries (1.0 and 5.0)", () => {
    expect(
      adaptiveQuizConfigSchema.safeParse({
        is_adaptive: true,
...
describe("recoverySessionSchema", () => {
  ...
  it("rejects missing required fields", () => {
    ...
    expect(
      recoverySessionSchema.safeParse({ student_id: validUUID }).success
    ).toBe(false);
  });
});
...
describe("verifiedExplanationSchema", () => {
  ...
  it("accepts valid explanation with all fields", () => {
    expect(() =>
      verifiedExplanationSchema.parse(validExplanation)
    ).not.toThrow();
  });

  it("accepts both valid source values", () => {
    expect(
      verifiedExplanationSchema.safeParse({
        ...validExplanation,
  ...
  it("rejects invalid source value", () => {
    const result = verifiedExplanationSchema.safeParse({
      ...validExplanation,
      source: "ai_generated",
    });
    expect(result.success).toBe(false);
  ...
  it("rejects missing required fields", () => {
    ...
    expect(
      verifiedExplanationSchema.safeParse({ question_id: validUUID }).success
    ).toBe(false);
  });
});
...
describe("practiceModeConfigSchema", () => {
  it("accepts practice_mode_enabled as true", () => {
    expect(() =>
      practiceModeConfigSchema.parse({ practice_mode_enabled: true })
    ).not.toThrow();
  });

  it("accepts practice_mode_enabled as false", () => {
    expect(() =>
      practiceModeConfigSchema.parse({ practice_mode_enabled: false })
    ).not.toThrow();
  });
...
describe("bloomsClimbStateSchema", () => {
  ...
  it("accepts current_bloom_level at boundaries (1 and 6)", () => {
    expect(
      bloomsClimbStateSchema.safeParse({
        ...validState,
  ...
  it("accepts consecutive_correct_at_level at boundaries (0 and 3)", () => {
    expect(
      bloomsClimbStateSchema.safeParse({
        ...validState,
        consecutive_correct_at_level: 0,
      }).success
    ).toBe(true);
    expect(
      bloomsClimbStateSchema.safeParse({
        ...validState,
        consecutive_correct_at_level: 3,
      }).success
    ).toBe(true);
  });
  ...
  it("rejects consecutive_correct_at_level above 3", () => {
    const result = bloomsClimbStateSchema.safeParse({
      ...validState,
      consecutive_correct_at_level: 4,
    });
    expect(result.success).toBe(false);
  });
  ...
  it("rejects missing required fields", () => {
    ...
    expect(
      bloomsClimbStateSchema.safeParse({ current_bloom_level: 2 }).success
    ).toBe(false);
  });
});

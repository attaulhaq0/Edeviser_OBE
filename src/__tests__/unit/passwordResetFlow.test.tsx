...
import "@/lib/i18n";
import { updatePasswordSchema, resetPasswordSchema } from "@/lib/schemas/auth";
...
describe("ResetPasswordPage", () => {
  ...
  it("shows success state after submitting a valid email", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByText(/password reset link has been sent/i).length
...
describe("UpdatePasswordPage", () => {
  ...
  it("updates password successfully and shows success state", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByText(/password has been updated/i).length
      ).toBeGreaterThanOrEqual(1);
    });

...
import {
  ...
  type GenerateCourseFileResult,
} from "@/hooks/useCourseFile";
...
describe("useGenerateCourseFile — mutationFn logic", () => {
  ...
  it("calls supabase.functions.invoke with correct function name and body", async () => {
    ...
    const { data, error } = await mockInvoke("generate-course-file", {
      body: input,
    });

    expect(mockInvoke).toHaveBeenCalledWith("generate-course-file", {
      body: input,
    });

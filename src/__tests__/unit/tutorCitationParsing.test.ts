...
describe("extractCitationMarkers", () => {
  it("returns empty array for text with no citations", () => {
    expect(
      extractCitationMarkers("This is a plain response with no references.")
    ).toEqual([]);
  });

  it("extracts a single citation marker", () => {
    expect(
      extractCitationMarkers("Arrays are stored contiguously [1].")
    ).toEqual([1]);
  });

  it("extracts multiple citation markers", () => {
    const text =
      "As explained in [1], linked lists [2] differ from arrays [3].";
  ...
  it("handles citations embedded in markdown bold", () => {
    const text =
      "**Important concept** [1] is explained in the **lecture notes** [2].";
    expect(extractCitationMarkers(text)).toEqual([1, 2]);
  });

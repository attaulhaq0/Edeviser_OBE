...
function validatePayload(
  payload: unknown,
): { valid: true; data: UpdateProgressPayload } | { valid: false; error: string } {
...
describe('update-challenge-progress Edge Function Logic', () => {
  describe('getMatchingChallengeTypes', () => {
    ...
    it('maps habit events to habit and cooperative', () => {
      expect(getMatchingChallengeTypes('habit')).toEqual(['habit', 'cooperative']);
    });

    it('maps xp events to xp_race and cooperative', () => {
      expect(getMatchingChallengeTypes('xp')).toEqual(['xp_race', 'cooperative']);
    });
  });
  ...
  describe('validatePayload', () => {
    it('accepts a valid payload with all required fields', () => {
      ...
      if (result.valid) {
        ...
        expect(result.data.student_id).toBe('123e4567-e89b-12d3-a456-426614174000');
        expect(result.data.course_id).toBe('223e4567-e89b-12d3-a456-426614174000');
      }
    });
    ...
    it('rejects missing course_id', () => {
      const result = validatePayload({ event_type: 'grade', student_id: 'abc' });
      expect(result.valid).toBe(false);
      if (!result.valid) {
  ...
  describe('countDistinctBloomsLevels (blooms_climb progress)', () => {
    ...
    it('ignores null values mixed with valid levels', () => {
      expect(countDistinctBloomsLevels(['Remembering', null, 'Applying', null])).toBe(2);
    });
  });

...
const mockMostImproved = [
  {
    student_id: "u1",
    student_name: "Alice",
    current_4_week_xp: 500,
    previous_4_week_xp: 200,
    improvement_percent: 150,
    xp_delta: 300,
  },
  {
    student_id: "u2",
    student_name: "Bob",
    current_4_week_xp: 400,
    previous_4_week_xp: 250,
...
vi.mock("@/hooks/useMostImprovedLeaderboard", () => ({
  useMostImprovedLeaderboard: () => ({
    data: mockMostImproved,
    isLoading: false,
  }),
}));
...
vi.mock("@/hooks/useLeagueLeaderboard", () => ({
  ...
  useStudentLeagueTier: () => ({
    data: { tier: "Silver", xpTotal: 1000 },
    isLoading: false,
  }),
  useStudentPercentileBand: () => ({
    data: {
      band: { type: "band", band: "Bottom 50%" },
      rank: 25,
      totalStudents: 4,
    },
    isLoading: false,
...
vi.mock("nuqs", () => {
  ...
  return {
    ...
    useQueryState: (_key: string, defaultVal: string) => {
      ...
      const setter = vi.fn((newVal: string) => {
        stateStore.set(_key, newVal);
      });
...
describe("LeaderboardPage", () => {
  ...
  it("renders My Rank card with rank, XP, and level", () => {
    ...
    if (rankCard) {
      ...
      expect(
        within(rankCard as HTMLElement).getByText("Bottom 50%")
      ).toBeInTheDocument();
      expect(
        within(rankCard as HTMLElement).getByText("1,000")
      ).toBeInTheDocument();
    }
  });
  ...
  it("shows streak flame for entries with active streaks", () => {
    ...
    const aliceRow = screen
      .getByText("Alice")
      .closest('div[class*="flex items-center gap-4"]');

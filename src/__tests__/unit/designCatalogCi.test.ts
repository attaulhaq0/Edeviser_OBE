...
describe("source-derived catalog CI wiring", () => {
  it("separates readonly validation from explicit artifact generation", () => {
    expect(packageJson.scripts["check:design-catalog"]).toBe(
      "node scripts/design-catalog/generate.mjs --check"
    );
    expect(packageJson.scripts["generate:design-catalog"]).toBe(
      "node scripts/design-catalog/generate.mjs --write"
    );
  });
  it("runs the readonly check after typechecking in the existing required job", () => {
    ...
    expect(packageJson.scripts["check:portable-tokens"]).toBe(
      "node scripts/design-tokens/export.mjs --check"
    );
    expect(packageJson.scripts["generate:portable-tokens"]).toBe(
      "node scripts/design-tokens/export.mjs --write"
    );
    expect(typecheck).toContain("run: npm run check:portable-tokens");
    expect(typecheck.indexOf("npm run check:portable-tokens")).toBeGreaterThan(
      typecheck.indexOf("npm run check:design-catalog")
    );
    expect(packageJson.scripts["check:route-inventory"]).toBe(
      "node scripts/audit/route-inventory-contract.mjs"
    );
    expect(typecheck).toContain("run: npm run check:route-inventory");
    expect(typecheck.indexOf("npm run check:route-inventory")).toBeGreaterThan(
      typecheck.indexOf("npm run check:design-catalog")
    );
    expect(typecheck.indexOf("npm run check:design-catalog")).toBeGreaterThan(
      typecheck.indexOf("npx tsc --noEmit")
    );

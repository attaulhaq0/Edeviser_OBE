...
const makeLog = (type: WellnessHabitType, value: number | null = null): WellnessHabitLog => ({
  id: `log-${type}`,
  studentId: 'student-1',
  date: '2024-03-15',
  wellnessType: type,
...
describe('WellnessHabitLogger', () => {
  it('renders enabled habits', () => {
    ...
    expect(screen.queryByTestId('wellness-habit-hydration')).not.toBeInTheDocument();
    expect(screen.queryByTestId('wellness-habit-sleep')).not.toBeInTheDocument();
  });
  ...
  it('renders nothing when no habits are enabled', () => {
    ...
    expect(screen.queryByTestId('wellness-habit-logger')).not.toBeInTheDocument();
  });
  ...
  it('shows checkmark for already-logged habits', () => {
    ...
    expect(screen.queryByTestId('wellness-toggle-meditation')).not.toBeInTheDocument();
  });

...
describe("Badge Definitions", () => {
  ...
  it("marks mystery badges with isMystery: true", () => {
    const mysteryBadges = BADGE_DEFINITIONS.filter(
      (b) => b.category === "mystery"
    );
  ...
  it("marks non-mystery badges with isMystery: false", () => {
    const nonMysteryBadges = BADGE_DEFINITIONS.filter(
      (b) => b.category !== "mystery"
    );
  ...
  it("includes the three required mystery badges", () => {
    const mysteryIds = BADGE_DEFINITIONS.filter((b) => b.isMystery).map(
      (b) => b.id
    );
  ...
  it("includes Bloom's progression badges", () => {
    const bloomsBadges = BADGE_DEFINITIONS.filter(
      (b) => b.category === "blooms"
    );

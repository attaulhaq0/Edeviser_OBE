...
vi.mock("@/hooks/useExplanationConfidence", () => ({
  useVerifiedExplanation: (...args: unknown[]) =>
    mockUseVerifiedExplanation(...args),
  useExplanationConfidence: (...args: unknown[]) =>
    mockUseExplanationConfidence(...args),
}));
      ...
      in: vi.fn().mockResolvedValue({
        data: [
          ...
          },
          {
            ...
            correct_answer: {
              value: "QuickSort",
              explanation: "QuickSort is a common sorting algorithm",
            },
            explanation: null,
            bloom_level: 3,
            clo_id: "clo-2",
...
const createWrapper = () => {
  ...
  return ({ children }: { children: React.ReactNode }) => (
    <QueryClientProvider client={queryClient}>
      <MemoryRouter
        initialEntries={["/student/quizzes/quiz-1/review/attempt-1"]}
      >
        <Routes>
          <Route
            path="/student/quizzes/:quizId/review/:attemptId"
            element={children}
          />
        </Routes>
      </MemoryRouter>
...
describe("PostQuizReview", () => {
  beforeEach(() => {
    ...
    mockUseExplanationConfidence.mockReturnValue({
      data: null,
      isLoading: false,
    });
  ...
  it('displays "Verified Explanation" label when verified explanation exists', async () => {
    mockUseVerifiedExplanation.mockReturnValue({
      data: {
        explanation_text: "Teacher-verified OOP explanation.",
        is_active: true,
      },
      isLoading: false,
    });
  ...
  it("prefers verified explanation text over AI-generated explanation", async () => {
    ...
    expect(
      screen.queryByText("OOP is a programming paradigm based on objects.")
  ...
  it("renders a BloomsProgressionLadder for each unique CLO in the quiz", async () => {
    ...
    const ladders = screen.getAllByRole("img", {
      name: /Bloom's progression ladder/i,
    });

...
const supabase = _supabase as unknown as { from: (table: string) => any; storage: { from: (bucket: string) => any } };
...
describe('useCourseModules hooks — queryFn / mutationFn logic', () => {
  ...
  describe('useCourseModules queryFn', () => {
    ...
    it('orders by sort_order ascending', () => {
      ...
      chain.select('id, course_id, title, description, sort_order, is_published, created_at');
  ...
  describe('useCourseMaterials queryFn', () => {
    ...
    it('orders materials by sort_order ascending', () => {
      ...
      chain.select('id, module_id, title, type, content_url, file_path, description, sort_order, is_published, clo_ids, created_at');
  ...
  describe('useCreateModule mutationFn', () => {
    ...
    it('calls logAuditEvent on module create', async () => {
      await (logAuditEvent as ReturnType<typeof vi.fn>)({
        action: 'create',
        entity_type: 'course_module',
        entity_id: 'mod-1',
        changes: { course_id: 'c-1', title: 'Week 1', sort_order: 0, is_published: false },
  ...
  describe('useCreateMaterial mutationFn', () => {
    it('inserts material with clo_ids', async () => {
      mockSingle.mockResolvedValue({
        data: { id: 'mat-1', module_id: 'mod-1', title: 'Lecture Slides', type: 'file', clo_ids: ['clo-1'] },
        error: null,
      });
  ...
  describe('uploadMaterialFile', () => {
    it('rejects files exceeding 50MB', async () => {
      ...
      await expect(uploadMaterialFile(bigFile, 'course-1')).rejects.toThrow('50MB');
    });

    it('rejects disallowed file extensions', async () => {
      const badFile = new File(['x'], 'script.exe', { type: 'application/octet-stream' });

      await expect(uploadMaterialFile(badFile, 'course-1')).rejects.toThrow('not allowed');
    });

    it('uploads valid file and returns public URL', async () => {
      const validFile = new File(['content'], 'slides.pdf', { type: 'application/pdf' });
      Object.defineProperty(validFile, 'size', { value: 1024 });

      mockUpload.mockResolvedValue({ error: null });
      mockGetPublicUrl.mockReturnValue({ data: { publicUrl: 'https://storage.example.com/slides.pdf' } });

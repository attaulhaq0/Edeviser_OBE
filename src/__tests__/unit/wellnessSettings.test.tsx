...
describe("WellnessSettingsPanel", () => {
  ...
  it("renders 4 wellness habit toggles", () => {
    ...
    expect(
      screen.getByTestId("wellness-setting-meditation")
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("wellness-setting-hydration")
    ).toBeInTheDocument();
  ...
  it("renders parent visibility toggle", () => {
    ...
    expect(
      screen.getByTestId("wellness-parent-visibility")
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("wellness-parent-visibility-toggle")
    ).toBeInTheDocument();
  });
  ...
  it("shows correct checked state for enabled habits", () => {
    ...
    const meditationToggle = screen.getByTestId(
      "wellness-setting-toggle-meditation"
    );
    const hydrationToggle = screen.getByTestId(
      "wellness-setting-toggle-hydration"
    );
  ...
  it("calls onToggleHabit when a habit toggle is clicked", () => {
    ...
    render(
      <WellnessSettingsPanel {...defaultProps} onToggleHabit={onToggleHabit} />
    );
  ...
  it("shows parent visibility as checked when enabled", () => {
    render(<WellnessSettingsPanel {...defaultProps} parentVisibility={true} />);
  ...
  it("renders habit descriptions", () => {
    ...
    expect(
      screen.getByText("Track daily meditation sessions (5+ minutes)")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Track daily water intake (8 glasses)")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Track daily physical activity (30+ minutes)")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Track nightly sleep duration (7+ hours)")
    ).toBeInTheDocument();
  });

...
describe("BloomsPioneerBadge", () => {
  ...
  it("renders correct aria-label for awarded badge", () => {
    ...
    expect(badge).toHaveAttribute(
      "aria-label",
      "Bloom's Challenger badge — awarded"
    );
  });
  ...
  it("renders correct aria-label for unawarded badge", () => {
    ...
    expect(badge).toHaveAttribute(
      "aria-label",
      "Bloom's Challenger badge — not yet earned"
    );
  });
  ...
  it("uses correct color for each badge type when awarded", () => {
    ...
    expect(
      screen.getByTestId("blooms-pioneer-badge-explorer").className
    ).toMatch(/bg-yellow-500/);

    rerender(<BloomsPioneerBadge type="challenger" awarded />);
    expect(
      screen.getByTestId("blooms-pioneer-badge-challenger").className
    ).toMatch(/bg-orange-500/);

    rerender(<BloomsPioneerBadge type="pioneer" awarded />);
    expect(
      screen.getByTestId("blooms-pioneer-badge-pioneer").className
    ).toMatch(/bg-red-500/);
  });
});

...
describe("shouldBatch", () => {
  ...
  it("returns false for non-peer_milestone types", () => {
    ...
    expect(shouldBatch(notifications, "student-1", "grade_released")).toBe(
      false
    );
  });
});
...
describe("hasReachedDailyLimit", () => {
  ...
  it("ignores notifications older than 24 hours", () => {
    ...
    expect(hasReachedDailyLimit(oldNotifications, "student-1", now)).toBe(
      false
    );
  });
});
...
describe("groupNotifications", () => {
  ...
  it("groups different types independently", () => {
    const grades = makeNotifications(4, {
      type: "grade_released",
      title: "Grade",
    });
    const badges = makeNotifications(4, {
      type: "badge_earned",
      title: "Badge",
    });
    const assignments = makeNotifications(2, {
      type: "new_assignment",
      title: "Assignment",
    });
...
describe("createBatchedPeerMilestonePayload", () => {
  it("creates a single-milestone payload", () => {
    const payload = createBatchedPeerMilestonePayload("student-1", [
      "Alice hit Level 5!",
    ]);
  ...
  it("creates a multi-milestone payload", () => {
    const descriptions = [
      "Alice hit Level 5!",
      "Bob earned Speed Demon!",
      "Carol hit Level 10!",
    ];
    const payload = createBatchedPeerMilestonePayload(
      "student-1",
      descriptions
    );

...
describe("Insight Generation — Non-Causal Language", () => {
  ...
  it('generates description with "more likely" for submission_timeliness', () => {
    const desc = generateInsightDescription(
      "exercise",
  ...
  it("uses correct habit labels", () => {
    expect(
      generateInsightDescription("meditation", "submission_rate")
    ).toContain("meditate");
    expect(
      generateInsightDescription("hydration", "submission_rate")
    ).toContain("stay hydrated");
    expect(generateInsightDescription("exercise", "submission_rate")).toContain(
      "exercise"
    );
    expect(generateInsightDescription("sleep", "submission_rate")).toContain(
      "get enough sleep"
    );
  });
});
...
describe("Causal Language Detection", () => {
  it('detects "because"', () => {
    expect(containsCausalLanguage("You submit more because you meditate")).toBe(
      true
    );
  });

  it('detects "causes"', () => {
    expect(containsCausalLanguage("Meditation causes better grades")).toBe(
      true
    );
  });
  ...
  it('detects "results in"', () => {
    expect(containsCausalLanguage("Exercise results in better focus")).toBe(
      true
    );
  });

  it('detects "leads to"', () => {
    expect(containsCausalLanguage("Sleep leads to better performance")).toBe(
      true
    );
  });
  ...
  it("allows non-causal language", () => {
    expect(
      containsCausalLanguage(
        "On days when you meditate, you tend to submit more"
      )
    ).toBe(false);
    expect(containsCausalLanguage("You're more likely to submit on time")).toBe(
      false
    );
    expect(
      containsCausalLanguage("Your activity level tends to be higher")
    ).toBe(false);
  });
});
...
describe("Correlation Insights Computation", () => {
  ...
  it("returns insights sorted by strength descending", () => {
    ...
    if (insights.length > 1) {
      for (let i = 1; i < insights.length; i++) {
        expect(insights[i]!.strength).toBeLessThanOrEqual(
          insights[i - 1]!.strength
        );
      }
    }
  });
  ...
  it("each insight has required fields", () => {
    const allDates = new Set(generateDates("2025-01-01", 20));
    const habitDatesByType = new Map<string, Set<string>>();
    habitDatesByType.set(
      "meditation",
      new Set(generateDates("2025-01-01", 10))
    );
    const submissionDates = new Set(generateDates("2025-01-01", 10));

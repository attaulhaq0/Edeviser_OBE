...
describe("Badge Tier Schemas", () => {
  describe("badgeTierSchema", () => {
    it("accepts valid tier values", () => {
      expect(
        badgeTierSchema.safeParse({ tier: "bronze", category: "academic" })
          .success
      ).toBe(true);
      expect(
        badgeTierSchema.safeParse({ tier: "silver", category: "engagement" })
          .success
      ).toBe(true);
      expect(
        badgeTierSchema.safeParse({ tier: "gold", category: "streak" }).success
      ).toBe(true);
    });

    it("rejects invalid tier values", () => {
      expect(
        badgeTierSchema.safeParse({ tier: "platinum", category: "academic" })
          .success
      ).toBe(false);
    });
    ...
    it("rejects empty category", () => {
      expect(
        badgeTierSchema.safeParse({ tier: "bronze", category: "" }).success
      ).toBe(false);
    });
  });

...
describe('challengeTypes', () => {
  const EXPECTED_TYPES = ['academic', 'habit', 'xp_race', 'blooms_climb', 'cooperative'] as const;
  ...
  it('each type has required fields', () => {
    for (const key of EXPECTED_TYPES) {
      ...
      expect(config.validationRules.maxGoal).toBeGreaterThanOrEqual(config.validationRules.minGoal);
    }
  });
  ...
  it('non-fixed-goal types accept variable goals', () => {
    for (const key of ['academic', 'habit', 'xp_race', 'cooperative'] as const) {

...
describe('XPBalanceBadge', () => {
  it('renders balance value', () => {
    mockUseXPBalance.mockReturnValue({ data: { balance: 1500 }, isLoading: false });
    render(<XPBalanceBadge />);
    expect(screen.getByText('1,500 XP')).toBeTruthy();
  });

  it('renders zero balance', () => {
    mockUseXPBalance.mockReturnValue({ data: { balance: 0 }, isLoading: false });
    render(<XPBalanceBadge />);
  ...
  it('applies size classes for sm variant', () => {
    mockUseXPBalance.mockReturnValue({ data: { balance: 100 }, isLoading: false });

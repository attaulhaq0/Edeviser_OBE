...
vi.mock("@/pages/student/onboarding/PersonalityStep", () => ({
  PersonalityStep: ({
    ...
    <div data-testid="personality-step">
      ...
      {onSkip && (
        <button data-testid="step-skip" onClick={onSkip}>
          Skip
        </button>
      )}
    </div>
...
vi.mock("@/pages/student/onboarding/BaselineSelectStep", () => ({
  BaselineSelectStep: ({
    ...
    onCoursesSelected: (ids: string[]) => void;
    onSkip?: () => void;
  }) => (
    <div data-testid="baseline-select-step">
      ...
      <button
        ...
        onClick={() => onCoursesSelected(["c1"])}
      >
        Select Courses
      </button>
      <button data-testid="skip-baseline" onClick={() => onCoursesSelected([])}>
        Skip Baseline
      </button>
      {onSkip && (
        <button data-testid="step-skip" onClick={onSkip}>
...
describe("OnboardingWizard", () => {
  ...
  describe("Day 1 mode (isDay1 = true)", () => {
    ...
    it("does NOT include learning_style, study_strategy, or baseline steps in Day 1", async () => {
      ...
      expect(
        screen.queryByTestId("baseline-test-step")
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe("Full mode (isDay1 = false)", () => {
    beforeEach(() => {
      // For full mode tests, day1 is already completed so isFirstTimeLogin = false
    ...
    it("includes all 8 steps in full mode", async () => {
      ...
      await user.click(screen.getByRole("button", { name: /next/i }));
      await waitFor(() =>
        expect(screen.getByTestId("learning-style-step")).toBeInTheDocument()
      );

      // Step 5: Study Strategy
      await user.click(screen.getByRole("button", { name: /next/i }));
      await waitFor(() =>
        expect(screen.getByTestId("study-strategy-step")).toBeInTheDocument()
      );

      // Step 6: Baseline Select
      await user.click(screen.getByRole("button", { name: /next/i }));
      await waitFor(() =>
        expect(screen.getByTestId("baseline-select-step")).toBeInTheDocument()
      );

      // Step 7: Baseline Test
      await user.click(screen.getByRole("button", { name: /next/i }));
      await waitFor(() =>
        expect(screen.getByTestId("baseline-test-step")).toBeInTheDocument()
      );

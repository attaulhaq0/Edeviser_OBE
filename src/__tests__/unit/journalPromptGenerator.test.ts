...
describe('generateJournalPrompt', () => {
  ...
  it('includes rubric feedback summary when provided', () => {
    ...
    expect(result.promptText).toContain('Good use of examples but needs more analysis');
  });
  ...
  it('works for all Bloom\'s levels', () => {
    ...
    for (const level of levels) {
      const result = generateJournalPrompt({ ...baseContext, bloomsLevel: level });
  ...
  it('works for all attainment levels', () => {
    const levels: AttainmentLevel[] = ['Excellent', 'Satisfactory', 'Developing', 'Not_Yet'];
    for (const level of levels) {
      const result = generateJournalPrompt({ ...baseContext, attainmentLevel: level });
      expect(result.questions.length).toBeGreaterThanOrEqual(3);
      expect(result.promptText.length).toBeGreaterThan(0);
    }
  });

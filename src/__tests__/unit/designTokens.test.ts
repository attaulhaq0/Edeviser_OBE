...
describe('Design Tokens — src/index.css', () => {
  ...
  describe('Brand Gradient', () => {
    it('defines --brand-gradient with correct teal-to-blue gradient', () => {
      expect(indexCss).toContain('--brand-gradient: linear-gradient(93.65deg, #14B8A6 5.37%, #0382BD 78.89%)');
    });

    it('defines --hero-gradient for dashboard hero cards', () => {
      expect(indexCss).toContain('--hero-gradient: linear-gradient(135deg, #0f172a 0%, #1e3a8a 50%, #312e81 100%)');
    });
  });

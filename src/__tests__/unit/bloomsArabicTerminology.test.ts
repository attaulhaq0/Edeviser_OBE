...
import {
  BLOOMS_ARABIC_STANDARD,
  OBE_ARABIC_TERMS,
} from "@/lib/bloomsArabicTerminology";
...
describe("bloomsArabicTerminology", () => {
  ...
  it("each Bloom level has at least 5 action verbs", () => {
    for (const level of BLOOMS_LEVELS) {
      expect(
        BLOOMS_ARABIC_STANDARD[level]!.verbs.length
  ...
  it("attainment levels in ar/common.json match OBE_ARABIC_TERMS", () => {
    ...
    expect(arCommon.attainment.satisfactory).toBe(
      OBE_ARABIC_TERMS.satisfactory
    );

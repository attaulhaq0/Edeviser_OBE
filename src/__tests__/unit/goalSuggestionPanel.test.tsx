...
describe("GoalSuggestionPanel", () => {
  ...
  describe("goal suggestion rendering", () => {
    ...
    it("renders all 3 goal texts", () => {
      renderPanel();
      expect(
        screen.getByText("Review lecture notes for Chapter 5")
      ).toBeTruthy();
      expect(
        screen.getByText("Complete 3 practice problems this week")
      ).toBeTruthy();
    ...
    it("renders cohort completion rate for each goal", () => {
      renderPanel();
      expect(
        screen.getByText("90% of similar students completed this type of goal")
      ).toBeTruthy();
      expect(
        screen.getByText("65% of similar students completed this type of goal")
      ).toBeTruthy();
      expect(
        screen.getByText("30% of similar students completed this type of goal")
      ).toBeTruthy();
    });
  ...
  describe("loading state", () => {
    ...
    it("does not render goal texts when loading", () => {
      renderPanel({ isLoading: true });
      expect(
        screen.queryByText("Review lecture notes for Chapter 5")
      ).toBeNull();
    });
  });
  ...
  describe("filters non-suggested goals", () => {
    it('only renders goals with status "suggested"', () => {
      const mixed: GoalSuggestion[] = [
        ...
        makeSuggestion({
          id: "g2",
          goal_text: "Accepted goal",
          status: "accepted",
        }),
        makeSuggestion({
          id: "g3",
          goal_text: "Dismissed goal",
          status: "dismissed",
        }),
      ];

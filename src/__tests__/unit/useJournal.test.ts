...
describe('useJournal hooks — queryFn / mutationFn logic', () => {
  ...
  describe('useJournalEntries queryFn', () => {
    ...
    it('calls select with explicit columns and applies student_id filter', () => {
      ...
      chain.select('id, student_id, course_id, content, clo_id, is_shared, created_at');
  ...
  describe('useCreateJournalEntry mutationFn', () => {
    it('inserts into journal_entries and logs audit event', async () => {
      const input = {
        course_id: 'course-1',
        content: 'This is a long enough journal entry for testing purposes with enough words.',
        clo_id: 'clo-1',
        is_shared: false,
      };

      const createdEntry = { id: 'created-entry-id', student_id: 'student-user-id', ...input };
  ...
  describe('useUpdateJournalEntry mutationFn', () => {
    it('updates journal_entries and logs audit event', async () => {
      ...
      const changes = { content: 'Updated reflection content with enough words for the test.' };

      mockSingle.mockResolvedValue({ data: { id: entryId, ...changes }, error: null });
  ...
  describe('error handling', () => {
    ...
    it('returns error when insert fails', async () => {
      mockSingle.mockResolvedValue({ data: null, error: { message: 'Content too short' } });

...
import { computeEarnSpendRatio, classifyInflation, getStatusLabel } from '@/lib/earnSpendRatioCalculator';

describe('XP Economist Dashboard', () => {
  describe('computeEarnSpendRatio', () => {
    it('computes correct ratio for healthy economy', () => {
      const result = computeEarnSpendRatio({ totalEarned: 3000, totalSpent: 1000 });
    ...
    it('detects inflationary economy (ratio > 4)', () => {
      const result = computeEarnSpendRatio({ totalEarned: 5000, totalSpent: 1000 });
      expect(result.ratio).toBe(5);
      expect(result.status).toBe('inflationary');
    });

    it('detects deflationary economy (ratio < 2)', () => {
      const result = computeEarnSpendRatio({ totalEarned: 1000, totalSpent: 1000 });
      expect(result.ratio).toBe(1);
      expect(result.status).toBe('deflationary');
    });

    it('handles no spending', () => {
      const result = computeEarnSpendRatio({ totalEarned: 5000, totalSpent: 0 });
      expect(result.ratio).toBeNull();
      expect(result.status).toBe('no_spending');
    });

    it('clamps negative values to 0', () => {
      const result = computeEarnSpendRatio({ totalEarned: -100, totalSpent: -50 });

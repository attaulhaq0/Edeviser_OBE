...
describe("getAttainmentLevel", () => {
  it("returns Excellent for 85", () =>
    expect(getAttainmentLevel(85)).toBe("Excellent"));
  it("returns Excellent for 100", () =>
    expect(getAttainmentLevel(100)).toBe("Excellent"));
  it("returns Satisfactory for 70", () =>
    expect(getAttainmentLevel(70)).toBe("Satisfactory"));
  it("returns Satisfactory for 84", () =>
    expect(getAttainmentLevel(84)).toBe("Satisfactory"));
  it("returns Developing for 50", () =>
    expect(getAttainmentLevel(50)).toBe("Developing"));
  it("returns Developing for 69", () =>
    expect(getAttainmentLevel(69)).toBe("Developing"));
  it("returns Not_Yet for 49", () =>
    expect(getAttainmentLevel(49)).toBe("Not_Yet"));
  it("returns Not_Yet for 0", () =>
    expect(getAttainmentLevel(0)).toBe("Not_Yet"));
});
...
describe("BloomsPill", () => {
  ...
  it("applies purple color for remembering", () => {
    ...
    expect(container.firstElementChild?.className).toContain("bg-purple-700");
  });

  it("applies yellow color for analyzing", () => {
    const { container } = render(<BloomsPill level="analyzing" />);
    expect(container.firstElementChild?.className).toContain("bg-yellow-700");
  });
});
...
describe("HabitGrid", () => {
  ...
  it("handles all habits completed", () => {
    render(
      <HabitGrid completedHabits={["login", "submit", "journal", "read"]} />
    );
    const checks = screen.getAllByText("✓");

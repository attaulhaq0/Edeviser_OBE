...
export const useHabitExport = ({
  ...
  const exportCSV = useCallback(() => {
    ...
    const heatmapData = queryClient.getQueryData<HeatmapDay[]>(
      queryKeys.heatmap.data(studentId, semesterRange.start, semesterRange.end, filter),
    );
    ...
    const rows: HabitReportRow[] = heatmapData.map((day) => {
      const academicHabits = day.habits.filter(h => h.category === 'academic');
      const wellnessHabits = day.habits.filter(h => h.category === 'wellness');
      ...
      for (const wh of enabledWellnessHabits) {
        (row as unknown as Record<string, unknown>)[wh] = wellnessHabits.some(h => h.type === wh);
      }

      return row;
    });

    const csv = generateHabitCSV(rows, {
      ...summaryData,
      totalXP: 0,

  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AISuggestion[]> => {
      ...
      const { data, error } = await supabase
        .from('ai_feedback')
        .select('id, student_id, suggestion_type, suggestion_text, suggestion_data, feedback, validated_outcome, created_at')
...
export const useSubmitAIFeedback = () => {
  ...
  return useMutation({
    mutationFn: async ({ feedbackId, feedback }: SubmitFeedbackInput): Promise<void> => {

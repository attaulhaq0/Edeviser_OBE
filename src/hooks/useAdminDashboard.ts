...
export const useAdminKPIs = () => {
  return useQuery({
    ...
    queryFn: async (): Promise<AdminKPIData> => {
      // ⚡ Bolt Optimization: Batch 5 independent KPI queries into parallel Promise.all.
      // Expected impact: Reduces waterfall latency from ~5x network trips to ~1x network trip,
      // loading the dashboard KPIs ~400% faster.
      const [
        { count: totalUsers },
        { count: activeUsers },
        { count: totalPrograms },
...
export const useOnboardingAnalytics = () => {
  return useQuery({
    ...
    queryFn: async (): Promise<OnboardingAnalytics> => {
      // ⚡ Bolt Optimization: Batch onboarding count queries into parallel Promise.all.
      // Expected impact: Halves the network latency for the onboarding metrics card.
      const [
        { count: totalStudents },
...
export const useDepartmentAnalytics = () => {
  return useQuery({
    ...
    queryFn: async (): Promise<DepartmentAttainment[]> => {
      // ⚡ Bolt Optimization: Fetch departments, programs, attainments, and outcomes in parallel.
      // Expected impact: Reduces 4 sequential API calls to a single parallel batch,
      // significantly decreasing latency for department analytics rendering.
      const [

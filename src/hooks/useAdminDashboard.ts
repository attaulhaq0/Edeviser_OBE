...
export const useAdminKPIs = () => {
  return useQuery({
    ...
    queryFn: async (): Promise<AdminKPIData> => {
      // ⚡ Bolt: Execute independent Supabase queries concurrently to reduce total network latency
      const [
        { count: totalUsers },
        { count: activeUsers },
        { count: totalPrograms },
...
export const useDepartmentAnalytics = () => {
  return useQuery({
    ...
    queryFn: async (): Promise<DepartmentAttainment[]> => {
      // ⚡ Bolt: Fetch all independent resources concurrently to reduce total network latency
      const [

...
export const useAIPerformance = (options?: { enabled?: boolean }) => {
  return useQuery({
    ...
    queryFn: async (): Promise<AIPerformanceMetrics> => {
      ...
      const thumbsUp =
        suggestions?.filter((r) => r.feedback === "thumbs_up").length ?? 0;
      ...
      const correct =
        predictions?.filter((r) => r.validated_outcome === "correct").length ??
        0;
      const predictionAccuracyRate =
        predictionTotal > 0 ? Math.round((correct / predictionTotal) * 100) : 0;
      ...
      const accepted =
        drafts?.filter((r) => r.validated_outcome === "accepted").length ?? 0;
      const draftAcceptanceRate =
        draftTotal > 0 ? Math.round((accepted / draftTotal) * 100) : 0;

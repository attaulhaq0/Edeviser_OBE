      ...
      return {
        ...
        days_completed:
          typeof row.comeback_challenge_days_completed === "number"
            ...
            : 0,
        streak_to_restore:
          typeof row.comeback_challenge_streak_to_restore === "number"
            ? row.comeback_challenge_streak_to_restore
            : 0,
        start_date:
...
export const useStartComebackChallenge = () => {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: queryKeys.comebackChallenge.all }),
  });
};

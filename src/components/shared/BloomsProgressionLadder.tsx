...
export const BloomsProgressionLadder = ({
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`Bloom's progression ladder${
        cloTitle ? ` for ${cloTitle}` : ""
      }: highest level reached is ${
        clampedLevel > 0
          ? BLOOMS_LEVELS.find((l) => l.level === clampedLevel)?.label ?? "None"
          : "None"
      }`}
    >
      {cloTitle && (
      ...
      <div className={cn("flex flex-col", compact ? "gap-0.5" : "gap-1")}>
        {BLOOMS_LEVELS.map((config) => {
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                isReached
                  ...
                  : cn(config.dimBg, config.dimText),
            ...
            >
              <span
                className={cn("font-bold", compact ? "w-3 me-1" : "w-4 me-2")}
              >
                {config.level}
              </span>

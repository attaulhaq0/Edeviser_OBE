...
const QuestionPreview = ({
  ...
  const questionId = `qp-${questionType}-${questionText.slice(0, 20).replace(/\s+/g, '-')}`;

  const isCorrect = showCorrectAnswer && selectedAnswer === correctAnswer;
  const isIncorrect = showCorrectAnswer && selectedAnswer != null && selectedAnswer !== '' && selectedAnswer !== correctAnswer;

  return (
    <Card className="bg-white border-0 shadow-md rounded-xl p-6 space-y-4">
      <p className="text-sm font-medium text-gray-900 leading-relaxed" id={questionId}>
        {questionText}
      </p>
...
const MCQOptions = ({
  ...
  <div role="radiogroup" aria-labelledby={questionId} className="space-y-2">
    {options.map((option) => {
      ...
      const isOptionIncorrect = showCorrectAnswer && isSelected && option.key !== correctAnswer;
      ...
      return (
        <button
          ...
          className={cn(
            ...
            isOptionCorrect && 'bg-green-100 text-green-800 border border-green-300',
            isOptionIncorrect && 'bg-red-100 text-red-800 border border-red-300',
            showCorrectAnswer && !isOptionCorrect && !isOptionIncorrect && 'bg-slate-50 text-gray-700',
        ...
        >
          <span className={cn(
            ...
            showCorrectAnswer && !isOptionCorrect && !isOptionIncorrect && 'bg-slate-200 text-gray-600',
          )}>
            {option.key}
          </span>

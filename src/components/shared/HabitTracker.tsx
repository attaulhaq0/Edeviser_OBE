    ...
    <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden gap-0 py-0">
      ...
      <div className="p-6 overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr>
              <th className="text-start py-1 px-2 text-xs font-semibold text-gray-500 w-28">
                Habit
              </th>
              {dateColumns.map((col) => (
                <th
                  key={col.date}
                  className="py-1 px-1 text-xs font-semibold text-gray-500 text-center w-10"
                >
                  {col.label}
                </th>
              ))}

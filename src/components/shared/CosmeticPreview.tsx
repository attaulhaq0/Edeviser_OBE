...
const CosmeticPreview = ({
  ...
  if (subCategory === "profile_theme") {
    return (
      <ThemePreview metadata={metadata} name={name} className={className} />
    );
  }

  if (subCategory === "avatar_frame") {
    return (
      <FramePreview metadata={metadata} name={name} className={className} />
    );
  }

  if (subCategory === "display_title") {
    return (
      <TitlePreview metadata={metadata} name={name} className={className} />
    );
  }
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-center rounded-lg bg-slate-50 p-4",
        className
      )}
    >
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ backgroundColor: cssVars["--theme-accent-bg"] }}
      >
        <div
  ...
  return (
    <div className={cn("flex flex-col items-center gap-2 p-3", className)}>
      <div
        ...
        style={{
          border: `${cssVars["--frame-border-width"]} ${
            cssVars["--frame-border-style"] ?? "solid"
          } ${cssVars["--frame-border-color"]}`,

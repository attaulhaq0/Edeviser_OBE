...
const CorrelationInsightCard = ({ insight }: CorrelationInsightCardProps) => {
  ...
  const colorClasses =
    HABIT_COLORS[insight.habitType] ?? "bg-slate-50 text-slate-600";
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          {hasConfidence && (
            <CorrelationConfidenceBadge
              level={
                (insight as CorrelationInsightWithConfidence).confidenceLevel
              }
              dataPointCount={
                (insight as CorrelationInsightWithConfidence).dataPointCount
              }
            />
          )}
          {/* Strength indicator */}

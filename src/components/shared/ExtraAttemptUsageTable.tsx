...
const ExtraAttemptUsageTable = ({ quizId }: ExtraAttemptUsageTableProps) => {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<ExtraAttemptUsage[]> => {
      ...
      const maxAttempts = (quiz as Record<string, unknown>).max_attempts as number;
      ...
      const studentIds = [...new Set((attempts as Array<Record<string, unknown>>).map((a) => a.student_id as string))];
      ...
      const nameMap = new Map(
        ((profiles ?? []) as Array<Record<string, unknown>>).map((p) => [p.id as string, p.full_name as string]),
      );
  ...
  return (
    <Card className="bg-white border-0 shadow-md rounded-xl overflow-hidden">
      ...
      >
        ...
        <h2 className="text-lg font-bold tracking-tight text-white">Extra Attempt Token Usage</h2>
      </div>
      <div className="p-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-start py-2 pe-4 font-semibold text-gray-600">Student</th>
                <th className="text-start py-2 pe-4 font-semibold text-gray-600">Attempt #</th>
                <th className="text-start py-2 pe-4 font-semibold text-gray-600">Date</th>
                <th className="text-start py-2 font-semibold text-gray-600">Score</th>
              </tr>
            </thead>
            <tbody>
              {data.map((usage, idx) => (
                <tr key={`${usage.studentId}-${idx}`} className="border-b border-slate-100 last:border-0">
                  ...
                  <td className="py-2 pe-4 text-gray-600">#{usage.attemptNumber}</td>

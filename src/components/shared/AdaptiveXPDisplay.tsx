...
const AdaptiveXPDisplay = ({ studentId, actionType = 'submission' }: AdaptiveXPDisplayProps) => {
  const { data: multiplierData } = useStudentXPMultiplier(studentId);
  const { data: diminishingData } = useDiminishingReturnsStatus(studentId, actionType);
      ...
      {diminishingData?.is_diminished && (
        <div className="flex items-center gap-2 p-2 rounded-lg bg-amber-50 border border-amber-200">
          ...
          <p className="text-xs text-amber-700">
            Diminishing Returns — next action earns {diminishingData.next_multiplier}x XP.
            Try a different activity to earn full XP.
          </p>
        </div>
      )}
    </div>

...
function getScoreColor(score: number): { text: string; bg: string; ring: string } {
  ...
  if (score >= 40) return { text: 'text-yellow-600', bg: 'bg-yellow-500', ring: 'ring-yellow-200' };
...
const CooperationScoreDisplay = ({
  ...
  const sizeClasses = {
    sm: { wrapper: 'gap-2', icon: 'h-4 w-4', value: 'text-lg font-black', gauge: 'h-1.5' },
    md: { wrapper: 'gap-3', icon: 'h-5 w-5', value: 'text-2xl font-black', gauge: 'h-2' },
    lg: { wrapper: 'gap-4', icon: 'h-6 w-6', value: 'text-3xl font-black', gauge: 'h-2.5' },
  }[size];
  ...
  return (
    ...
    >
      ...
      {/* Gauge bar */}
      <div className={cn('w-full rounded-full bg-gray-100 overflow-hidden', sizeClasses.gauge)}>
        <div

...
const ATTAINMENT_STYLES: Record<AttainmentLevel, { bar: string; bg: string; text: string; label: string }> = {
  Excellent: { bar: 'bg-green-600', bg: 'bg-green-50', text: 'text-green-600', label: 'Excellent' },
  Satisfactory: { bar: 'bg-blue-600', bg: 'bg-blue-50', text: 'text-blue-600', label: 'Satisfactory' },
  Developing: { bar: 'bg-yellow-600', bg: 'bg-yellow-50', text: 'text-yellow-600', label: 'Developing' },
  Not_Yet: { bar: 'bg-red-600', bg: 'bg-red-50', text: 'text-red-600', label: 'Not Yet' },
};
...
const AttainmentBar = ({ percent, label, className, showLabel = true }: AttainmentBarProps) => {
  ...
  return (
    <div className={cn('space-y-1', className)}>
      ...
      <div className={cn('h-2.5 w-full rounded-full overflow-hidden', style.bg)}>
        <div

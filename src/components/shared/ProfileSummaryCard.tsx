            ...
            <div className="space-y-2">
              {Object.entries(SELF_EFFICACY_LABELS).map(([key, label]) => (
                <ScoreBar
                  ...
                  value={
                    selfEfficacy[
                      key as keyof Omit<SelfEfficacyProfile, "overall">
                    ] ?? 0
                  }
                />
              ))}
            ...
            <div className="space-y-2">
              {Object.entries(STUDY_STRATEGY_LABELS).map(([key, label]) => (
                <ScoreBar
                  ...
                  label={label}
                  value={
                    studyStrategies[key as keyof StudyStrategyProfile] ?? 0
                  }
                />
              ))}
            </div>
          ...
          <div className="rounded-lg bg-amber-50 p-4 space-y-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-lg" aria-hidden="true">
                {varkInfo.icon === "Eye"
                  ? "👁️"
                  : varkInfo.icon === "Headphones"
                  ? "👂"
                  : varkInfo.icon === "BookOpen"
                  ? "📖"
                  : varkInfo.icon === "Hand"
                  ? "🤲"
                  : "🎯"}
            ...
            <p className="text-[10px] text-amber-600 flex items-start gap-1">
              ...
              Learning style preferences are provided as a self-awareness
              exercise. They are not used for content adaptation.
            </p>
          </div>
        )}

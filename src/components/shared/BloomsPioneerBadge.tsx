...
const SIZE_CLASSES = {
  sm: { wrapper: 'gap-1 p-2', icon: 'h-4 w-4', label: 'text-[10px]', bloom: 'text-[9px]' },
  md: { wrapper: 'gap-1.5 p-3', icon: 'h-5 w-5', label: 'text-xs', bloom: 'text-[10px]' },
  lg: { wrapper: 'gap-2 p-4', icon: 'h-6 w-6', label: 'text-sm', bloom: 'text-xs' },
};
...
export const BloomsPioneerBadge = ({
  ...
  return (
    <div
      ...
      role="img"
      aria-label={`${config.label} badge${awarded ? ' — awarded' : ' — not yet earned'}`}
    >

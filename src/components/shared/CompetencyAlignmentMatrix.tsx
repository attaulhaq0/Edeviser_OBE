...
import type { CompetencyItem, CompetencyOutcomeMapping } from '@/hooks/useCompetencyFrameworks';
          ...
          <tr>
            ...
            {outcomes.map((o) => (
              <th
                ...
                title={o.title}
              >
                <span className="block truncate max-w-[80px]">{o.code ?? o.title.slice(0, 10)}</span>
                <span className="block text-[9px] text-slate-400 font-normal">{o.type}</span>
              </th>
            ))}
              ...
              <tr key={ind.id} className="hover:bg-slate-50">
                ...
                {outcomes.map((o) => {
                  ...
                  return (
                    <td
                      key={o.id}
                      className={`px-2 py-2 text-center border-slate-100 ${hasMapped ? getAttainmentColor(o.attainment) : 'bg-white'}`}
                    >
                      {hasMapped ? (

...
const buildTree = (items: CompetencyItem[]): CompetencyItem[] => {
  const map = new Map<string, CompetencyItem & { children: CompetencyItem[] }>();
...
const TreeNode = ({ item, mappedItemIds, depth = 0 }: { item: CompetencyItem & { children?: CompetencyItem[] }; mappedItemIds?: Set<string>; depth?: number }) => {
  const [expanded, setExpanded] = useState(depth < 1);
  const hasChildren = (item.children?.length ?? 0) > 0;
  const isUnmapped = item.level === 'indicator' && mappedItemIds && !mappedItemIds.has(item.id);
  ...
  return (
    <div>
      <div
        ...
        onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && hasChildren) { e.preventDefault(); setExpanded(!expanded); } }}
      >
        {hasChildren ? (
        ...
        <Badge variant="outline" className="text-[9px] px-1">{item.level}</Badge>
      ...
      {expanded && (item.children ?? []).map((child) => (
        <TreeNode key={child.id} item={child as CompetencyItem & { children?: CompetencyItem[] }} mappedItemIds={mappedItemIds} depth={depth + 1} />
      ))}
    </div>
  );
};
...
const CompetencyTree = ({ items, mappedItemIds }: CompetencyTreeProps) => {
  ...
  if (tree.length === 0) {
    return <p className="text-sm text-slate-400 text-center py-6">No competency items.</p>;
  }

  return (
    <div className="space-y-0.5">
      {tree.map((item) => (
        <TreeNode key={item.id} item={item as CompetencyItem & { children?: CompetencyItem[] }} mappedItemIds={mappedItemIds} />
      ))}
    </div>
  );
};

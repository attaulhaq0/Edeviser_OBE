...
| Dismissible default-close dialogs | `@/components/shared/LocalizedDialogContent` | Uses the generated DialogContent with `showCloseButton={false}` and one localized logical 44px close. Intentional no-close approval/consent/celebration dialogs keep their existing owner. The edge-to-edge SearchCommand palette uses a documented custom in-flow translated close instead of this absolute-close wrapper. Other callers remain unmigrated. |

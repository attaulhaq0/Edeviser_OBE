...
// These report-job tables are managed in the live Supabase project by the
// accreditation-report migration, but are not present in the committed
// generated type snapshot. The Edge Function checks the query errors and
// returns an explicit failure when the optional report backend is unavailable;
// keep the generated database.ts immutable and track the drift here until the
// type snapshot is regenerated by the project owner.

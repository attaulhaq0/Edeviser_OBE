-- Replace a material's versioned embedding chunks atomically. The Edge
-- Function generates and validates every new vector before calling this RPC;
-- any SQL error rolls the delete and insert back together so a failed re-index
-- never destroys the last usable index.
...
BEGIN
  ...
  IF p_source_material_id IS NOT NULL THEN
    ...
    WHERE source_material_id = p_source_material_id
...
COMMENT ON FUNCTION public.replace_course_material_embeddings_v2(
  uuid, uuid, uuid, text, jsonb
) IS 'Service-only atomic replacement for authorized Supabase-native material embeddings.';

...
section_attainment AS (
  SELECT oa.outcome_id AS clo_id, en.section_id,
...
clo_summary AS (
  SELECT cc.clo_id, cc.clo_title, cc.blooms_level,
...
coverage AS (
  SELECT lo.id AS clo_id,
    CASE WHEN EXISTS (SELECT 1 FROM public.assignments a WHERE a.course_id = p_course_id
    ...
    OR EXISTS (SELECT 1 FROM public.quizzes q WHERE q.course_id = p_course_id
  ...
  FROM public.learning_outcomes lo WHERE lo.course_id = p_course_id AND lo.type = ''CLO''
)
SELECT jsonb_build_object(
  ''course_id'', p_course_id,
  ''clos'', COALESCE((SELECT jsonb_agg(jsonb_build_object(
    ''clo_id'', cs.clo_id, ''clo_title'', cs.clo_title, ''blooms_level'', cs.blooms_level,
    ''course_avg'', cs.course_avg, ''total_below_target'', cs.total_below_target,
    ''sections_with_data'', cs.sections_with_data, ''has_assessment'', cv.has_assessment
  ) ORDER BY cs.course_avg ASC NULLS LAST) FROM clo_summary cs LEFT JOIN coverage cv ON cv.clo_id = cs.clo_id), ''[]''::jsonb),
  ''sections'', COALESCE((SELECT jsonb_agg(jsonb_build_object(
    ''section_id'', sec.section_id, ''section_code'', sec.section_code, ''teacher_name'', sec.teacher_name
  ) ORDER BY sec.section_code) FROM course_sections sec), ''[]''::jsonb),
  ''matrix'', COALESCE((SELECT jsonb_agg(jsonb_build_object(
    ''clo_id'', sa.clo_id, ''section_id'', sa.section_id, ''avg_attainment'', sa.avg_attainment,
    ''student_count'', sa.student_count, ''below_target'', sa.below_target
  ) ORDER BY sa.clo_id, sa.section_id) FROM section_attainment sa), ''[]''::jsonb)
);
$fn$;

-- =============================================================================
-- problem_case_ownership_routing — continuous-verification task 8.9 (Q5)
-- Adds deterministic ownership routing to classify_problem_cases_v1:
-- each problem case now carries `recommended_owner`, derived from the
-- DOMINANT cause (never from AI):
...
BEGIN
  ...
  FROM (
    SELECT en.section_id, avg(oa.attainment_percent) AS sec_avg
    FROM public.outcome_attainment oa
    JOIN public.student_courses sc ON sc.student_id = oa.student_id AND sc.course_id = oa.course_id
    WHERE oa.course_id = p_course_id AND oa.scope = 'student_course'

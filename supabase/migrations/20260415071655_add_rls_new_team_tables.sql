...
CREATE POLICY "student_select_own_invitations" ON team_invitations FOR SELECT TO authenticated
  USING (auth_user_role() = 'student' AND invited_student_id = auth.uid());

CREATE POLICY "captain_insert_invitations" ON team_invitations FOR INSERT TO authenticated
  WITH CHECK (auth_user_role() = 'student' AND invited_by = auth.uid() AND team_id IN (
...
CREATE POLICY "student_respond_invitation" ON team_invitations FOR UPDATE TO authenticated
  USING (auth_user_role() = 'student' AND invited_student_id = auth.uid() AND status = 'pending');
...
CREATE POLICY "teacher_select_invitations" ON team_invitations FOR SELECT TO authenticated
  USING (auth_user_role() = 'teacher' AND team_id IN (
    SELECT t.id FROM teams t JOIN courses c ON t.course_id = c.id WHERE c.teacher_id = auth.uid()
  ));
...
CREATE POLICY "team_select_teaching_moments" ON peer_teaching_moments FOR SELECT TO authenticated
  USING (team_id IN (SELECT tm.team_id FROM team_members tm WHERE tm.student_id = auth.uid() AND tm.left_at IS NULL));

CREATE POLICY "author_insert_teaching_moment" ON peer_teaching_moments FOR INSERT TO authenticated
  WITH CHECK (auth_user_role() = 'student' AND author_id = auth.uid() AND team_id IN (
    SELECT tm.team_id FROM team_members tm WHERE tm.student_id = auth.uid() AND tm.left_at IS NULL
  ));
...
CREATE POLICY "teacher_select_ratings" ON teaching_moment_ratings FOR SELECT TO authenticated
  USING (auth_user_role() = 'teacher' AND teaching_moment_id IN (
    SELECT ptm.id FROM peer_teaching_moments ptm JOIN teams t ON ptm.team_id = t.id JOIN courses c ON t.course_id = c.id WHERE c.teacher_id = auth.uid()
  ));
...
CREATE POLICY "captain_insert_vote" ON replacement_votes FOR INSERT TO authenticated
  WITH CHECK (initiated_by = auth.uid() AND team_id IN (SELECT id FROM teams WHERE captain_id = auth.uid() AND deleted_at IS NULL));

CREATE POLICY "member_cast_vote" ON replacement_votes FOR UPDATE TO authenticated
  USING (status = 'open' AND team_id IN (SELECT tm.team_id FROM team_members tm WHERE tm.student_id = auth.uid() AND tm.left_at IS NULL));

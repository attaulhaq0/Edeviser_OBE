-- ============================================================
-- Migration: pg_cron job for team health score computation
-- Schedule: Weekly Monday at 02:00 UTC
-- Logic:
--   1. For each active team, compute Gini coefficient of member
--      XP contributions (last 7 days)
--   2. Compute engagement trend: compare this week vs last week XP
--      - Rising (>10% increase): trend_score = 100
--      - Stable (within ±10%): trend_score = 75
--      - Declining (>10% decrease): trend_score = 25
--   3. Compute challenge participation rate (last 30 days)
--   4. Compute activity overlap rate (days with 2+ active members / 7)
--   5. health_score = 0.30×(1−Gini)×100 + 0.25×trend_score
--                   + 0.25×participation_rate + 0.20×overlap_rate
--   6. Insert team_health_snapshots record
--   7. Update teams.health_score and health_status:
--      - ≥70: 'healthy'
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    ...
    PERFORM cron.schedule(
      ...
      BEGIN
        ...
        LOOP
              ...
              FOR i IN 1..n LOOP
                FOR j IN 1..n LOOP
                  sum_abs_diff := sum_abs_diff
                    + ABS(member_xp_values[i] - member_xp_values[j]);
                END LOOP;
              END LOOP;
          ...
          -- ── 2. Engagement trend (this week vs last week) ──
          ...
          IF last_week_xp > 0 THEN
            ...
            END IF;
          ELSE
            -- No XP last week: if any XP this week → rising, else stable
          ...
          -- ── 3. Challenge participation rate (last 30 days) ──
          ...
          IF active_challenges_count > 0 THEN
            participation_rate := (participated_challenges::numeric
                                   / active_challenges_count) * 100;
          ELSE
            participation_rate := 100;
          END IF;

          -- ── 4. Activity overlap rate (days with 2+ active members / 7) ──
          overlap_days := 0;
          FOR day_offset IN 0..6 LOOP
            check_date := (CURRENT_DATE - day_offset)::DATE;

            SELECT COUNT(DISTINCT ht.student_id) INTO active_members_on_day
            FROM habit_tracking ht
            JOIN team_members tm ON tm.student_id = ht.student_id
            WHERE tm.team_id = team_rec.id
              AND tm.left_at IS NULL
              AND ht.habit_date = check_date;
          ...
          -- ── 6. Insert health snapshot ──
          INSERT INTO team_health_snapshots (
            team_id, health_score, gini_coefficient,
            engagement_trend, challenge_participation_rate,
            activity_overlap_rate, computed_at
          ) VALUES (
            team_rec.id, health, ROUND(gini::numeric, 3),
            engagement_trend_val, ROUND(participation_rate, 2),
            ROUND(overlap_rate, 2), NOW()
          );

          -- ── 7. Update team health score and status ──
          UPDATE teams
    ...
    RAISE NOTICE 'pg_cron job "compute-team-health" created (Monday 02:00 UTC)';
  ELSE
    RAISE NOTICE 'pg_cron not available — skipping compute-team-health cron job';
  END IF;
END;
$$;

-- Grade-release trigger: cascade evidence -> CLO/PLO/ILO attainment rollup,
-- award the canonical +15 grade XP (idempotent on grade id via ON CONFLICT DO
-- NOTHING), recompute level from the pure SQL calculate_level_from_xp mirror,
-- and mark the submission graded. Ordered after submissions/grades/evidence/
-- outcome_attainment/student_gamification all exist.

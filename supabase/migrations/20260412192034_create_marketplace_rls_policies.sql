...
CREATE POLICY "students_select_active_items" ON marketplace_items
  ...
  USING (is_active = true AND institution_id = (SELECT institution_id FROM profiles WHERE id = (select auth.uid())));

CREATE POLICY "admin_all_items" ON marketplace_items
  FOR ALL TO authenticated
  USING (
    (SELECT role FROM profiles WHERE id = (select auth.uid())) = 'admin'
    AND institution_id = (SELECT institution_id FROM profiles WHERE id = (select auth.uid()))
  )
  WITH CHECK (
    (SELECT role FROM profiles WHERE id = (select auth.uid())) = 'admin'
    AND institution_id = (SELECT institution_id FROM profiles WHERE id = (select auth.uid()))
...
CREATE POLICY "admin_select_institution_purchases" ON xp_purchases
  ...
  USING (
    (SELECT role FROM profiles WHERE id = (select auth.uid())) = 'admin'
    AND student_id IN (
      SELECT id FROM profiles WHERE institution_id = (SELECT institution_id FROM profiles WHERE id = (select auth.uid()))
    )
  );
...
CREATE POLICY "parent_select_linked_purchases" ON xp_purchases
  FOR SELECT TO authenticated
  USING (
    (SELECT role FROM profiles WHERE id = (select auth.uid())) = 'parent'
    AND student_id IN (
...
CREATE POLICY "authenticated_select_sale_event_items" ON sale_event_items
  ...
  USING (
    sale_event_id IN (
      SELECT id FROM sale_events WHERE institution_id = (SELECT institution_id FROM profiles WHERE id = (select auth.uid()))
    )
  );

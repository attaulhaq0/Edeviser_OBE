CREATE OR REPLACE FUNCTION public.get_admin_analytics(p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
    ...
    'departments', COALESCE((
      ...
      LEFT JOIN LATERAL (
        SELECT
          ...
          count(DISTINCT CASE WHEN l.id IS NOT NULL THEN sp.id END)::int AS active_learners,
          round(avg(oa.attainment_percent))::int AS mastery_percent,
          CASE WHEN count(DISTINCT sc.student_id) > 0 THEN
            count(DISTINCT CASE WHEN l.id IS NOT NULL THEN sp.id END)::numeric / count(DISTINCT sc.student_id) * 100 ELSE 0 END AS active_percent

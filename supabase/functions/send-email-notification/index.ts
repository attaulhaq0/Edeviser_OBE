...
async function sendWithRetry(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.error(
        `Resend API attempt ${attempt}/${MAX_RETRIES} failed: ${response.status} ${errorBody}`
      );
      ...
      console.error(
        `Resend API attempt ${attempt}/${MAX_RETRIES} network error:`,
      ...
      return {
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>You haven't logged in today and your <strong>${
              data.streak_count ?? 0
            }-day streak</strong> is at risk of breaking.</p>
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <ul>
              ...
              <li><strong>Badges Earned:</strong> ${
      ...
      return {
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>Hi ${data.student_name ?? "there"},</p>
            <p>A new assignment has been posted in <strong>${
              data.course_name ?? "your course"
            }</strong>:</p>
            <div style="background: #f8fafc; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <h3 style="margin: 0 0 8px 0;">${
                data.assignment_title ?? "Untitled"
              }</h3>
              <p style="margin: 0; color: #64748b;">Due: ${
                data.due_date ?? "TBD"
              }</p>
            </div>
            <a href="${
      ...
      return {
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>Hi ${data.student_name ?? "there"},</p>
            <p>Your grade for <strong>${
              data.assignment_title ?? "an assignment"
            }</strong> in ${
          data.course_name ?? "your course"
        } has been released.</p>
            <div style="background: #f8fafc; border-radius: 8px; padding: 16px; margin: 16px 0;">
      ...
      return {
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            }</strong> on Edeviser as a <strong>${
          data.role ?? "user"
        }</strong>.</p>
      ...
      return {
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>A grade has been released for <strong>${
              data.student_name ?? "your child"
            }</strong> in <strong>${
          data.course_name ?? "a course"
        }</strong>.</p>
            <div style="background: #f8fafc; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <p style="margin: 0;"><strong>Assignment:</strong> ${
                data.assignment_title ?? "N/A"
              }</p>
              <p style="margin: 0;"><strong>Score:</strong> ${
                data.score ?? "N/A"
              }%</p>
      ...
      return {
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p><strong>${
              data.student_name ?? "Your child"
            }</strong>'s attendance in <strong>${
          data.course_name ?? "a course"
        }</strong> has dropped below 75%.</p>
            <div style="background: #fef2f2; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <p style="margin: 0; color: #dc2626;"><strong>Current Attendance:</strong> ${
      ...
      return {
        ...
        html: `
          <div style="font-family: 'Noto Sans', sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p><strong>${
              data.student_name ?? "Your child"
            }</strong> has been flagged as at-risk in <strong>${
          data.course_name ?? "a course"
        }</strong>.</p>
            <div style="background: #fef2f2; border-radius: 8px; padding: 16px; margin: 16px 0;">
              <p style="margin: 0; color: #dc2626;"><strong>Risk Level:</strong> ${
                data.risk_level ?? "High"
              }</p>
              <p style="margin: 4px 0 0 0; color: #64748b;">${
                data.risk_reason ??
                "Multiple performance indicators suggest the student may need additional support."
              }</p>
            </div>
...
function validatePayload(
  ...
  if (
    ...
    !VALID_TEMPLATES.includes(p.template as EmailTemplate)
  ) {
    return {
      valid: false,
      error: `template is required and must be one of: ${VALID_TEMPLATES.join(
        ", "
      )}`,
    };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (!result.success) {
      console.error(
        `Email delivery failed for ${to} (template: ${template}): ${result.error}`
      );
      return new Response(
        JSON.stringify({
    ...
    return new Response(JSON.stringify({ success: true, template, to }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {

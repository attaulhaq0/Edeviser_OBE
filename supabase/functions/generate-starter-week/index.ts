...
function validatePayload(
  ...
  | { valid: true; data: GenerateStarterWeekRequest }
  ...
  if (
    typeof body.self_efficacy_score !== "number" ||
    body.self_efficacy_score < 0 ||
    body.self_efficacy_score > 100
  ) {
    return {
      valid: false,
      error: "self_efficacy_score must be a number between 0 and 100",
    };
  }

  const enrolled_course_ids = Array.isArray(body.enrolled_course_ids)
    ? body.enrolled_course_ids
    : [];
...
): StarterWeekSession["session_type"] {
...
function generateDescription(
  ...
  switch (sessionType) {
    case "review":
      return `Review key concepts for ${courseName} — deadline in ${deadlineDays} day${
        deadlineDays === 1 ? "" : "s"
      }`;
    case "practice":
...
serve(async (req) => {
  ...
  try {
    ...
    const { student_id, self_efficacy_score, enrolled_course_ids } =
      validation.data;
    ...
    if (courseIds.length === 0) {
      ...
      for (let i = 0; i < sessionCount; i++) {
        ...
        genericSessions.push({
          ...
          description: generateDescription(
            genericTypes[i % genericTypes.length],
            null,
            null
          ),
        });
      ...
      if (insertErr) {
        ...
        return new Response(
          JSON.stringify({
            error: "Failed to save starter week sessions",
    ...
    coursesWithDeadlines.sort((a, b) => {
      if (a.nearest_deadline_days === null && b.nearest_deadline_days === null)
        return 0;
    ...
    for (let i = 0; i < sessionCount; i++) {
      ...
      sessions.push({
        ...
        description: generateDescription(
          ...
          courseInfo.nearest_deadline_days
        ),
      });
    }
    ...
    if (insertErr) {
      console.error(
        "Failed to insert starter week sessions:",
        insertErr.message
      );
      return new Response(
        JSON.stringify({

...
serve(async (req) => {
  ...
  try {
    ...
    doc.setFontSize(9);
    doc.text(`Receipt #${payment.receipt_number ?? payment_id.slice(0, 8)}`, pw - 14, 12, { align: 'right' });
    doc.text(`Date: ${payment.paid_at?.slice(0, 10) ?? new Date().toISOString().slice(0, 10)}`, pw - 14, 20, { align: 'right' });
    ...
    await supabase.storage.createBucket('fee-receipts', { public: false }).catch(() => {});
    ...
    return new Response(
      JSON.stringify({ success: true, download_url: signedUrl?.signedUrl, file_name: fileName }),

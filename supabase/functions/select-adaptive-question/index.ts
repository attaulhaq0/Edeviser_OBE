...
function validatePayload(
  payload: unknown,
): { valid: true; data: SelectQuestionRequest } | { valid: false; error: string } {
  ...
  if (body.previous_question_id !== undefined && body.previous_question_id !== null) {
    if (!isValidUUID(body.previous_question_id)) {
      return { valid: false, error: 'previous_question_id must be a valid UUID' };
    }
  }

  if (body.previous_answer_correct !== undefined && body.previous_answer_correct !== null) {
    if (typeof body.previous_answer_correct !== 'boolean') {
      return { valid: false, error: 'previous_answer_correct must be a boolean' };
    }
  }

  if (body.previous_response_time_ms !== undefined && body.previous_response_time_ms !== null) {
    if (typeof body.previous_response_time_ms !== 'number' || body.previous_response_time_ms < 0) {
      return { valid: false, error: 'previous_response_time_ms must be a non-negative number' };
    }
  }
  ...
  return {
    ...
    data: {
      ...
      previous_answer_correct: (body.previous_answer_correct as boolean) ?? undefined,
      previous_response_time_ms: (body.previous_response_time_ms as number) ?? undefined,
    },
  };
}
...
function stripAndShuffleOptions(options: MCQOption[] | null): MCQOption[] | null {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!quiz.is_adaptive) {
      return new Response(
        JSON.stringify({ error: 'This quiz is not configured for adaptive mode' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }

    const totalQuestions: number = quiz.total_questions ?? 10;
    const adaptConfig = (quiz.adaptation_config ?? {}) as Record<string, unknown>;
    const stepUp = typeof adaptConfig.difficulty_step_up === 'number'
      ? adaptConfig.difficulty_step_up
      : DIFFICULTY_STEP_UP;
    const stepDown = typeof adaptConfig.difficulty_step_down === 'number'
    ...
    if (attemptError || !attempt) {
      return new Response(
        JSON.stringify({ error: 'Quiz attempt not found or does not belong to this student' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }

    const questionSequence: QuestionSequenceEntry[] = (attempt.question_sequence ?? []) as QuestionSequenceEntry[];
    ...
    if (questionSequence.length >= totalQuestions) {
      return new Response(
        JSON.stringify({
          ...
          current_target_difficulty: difficultyTrajectory.length > 0
            ? difficultyTrajectory[difficultyTrajectory.length - 1].target_difficulty
            : 2.5,
    ...
    if (isFirstQuestion) {
      ...
      if (attainments && attainments.length > 0) {
        ...
        const totalAttainment = attainments.reduce(
          (sum: number, a: { attainment_percentage: number }) => sum + (a.attainment_percentage ?? 0),
          0,
        );
      ...
      if (previous_answer_correct !== undefined && previous_answer_correct !== null) {
        targetDifficulty = adjustDifficulty(targetDifficulty, previous_answer_correct, stepUp, stepDown);
      }
    ...
    let query = supabase
      .from('question_bank')
      .select('id, question_text, question_type, options, bloom_level, clo_id, difficulty_rating')
      .eq('status', 'approved')
      .in('clo_id', cloIds)
    ...
    if (candidates.length < MIN_ELIGIBLE_QUESTIONS) {
      ...
      if (answeredQuestionIds.length > 0) {
        expandedQuery = expandedQuery.not('id', 'in', `(${answeredQuestionIds.join(',')})`);
      }
      ...
      if (candidates.length < MIN_ELIGIBLE_QUESTIONS) {
        warnings.push(
          ...
          `for target difficulty ${targetDifficulty.toFixed(1)} (expanded range ±${EXPANDED_DIFFICULTY_RANGE}). ` +
    ...
    console.error('Unhandled error in select-adaptive-question:', (error as Error).message);
    return new Response(

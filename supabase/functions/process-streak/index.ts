 ...
 */
function isStreakSabbaticalDay(sabbaticalEnabled: boolean, dateStr: string): boolean {
  ...
  for (const membership of memberships) {
    ...
    if (milestoneXP) {
      ...
      try {
        ...
        console.error(`Failed to award team streak XP:`, (xpErr as Error).message);
      }
      ...
      try {
        ...
        console.error('Failed to check team badges:', (badgeErr as Error).message);
      }
      ...
      const notifications = memberIds.map((memberId: string) => ({
        ...
        is_read: false,
        metadata: { team_id: teamId, streak_days: newStreak, xp_awarded: milestoneXP },
      }));

      await supabase.from('notifications').insert(notifications).catch((err: Error) => {
        console.error('Failed to send team streak notifications:', err.message);
      });
    }
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: current, error: fetchErr } = await supabase
      .from('student_gamification')
      .select('streak_count, last_login_date, streak_freezes_available, comeback_challenge_active, comeback_challenge_days_completed, comeback_challenge_streak_to_restore, total_active_days, habit_difficulty_level, habit_level_streak')
    ...
    const state: StreakState | null = current
      ? {
          ...
          comeback_challenge_days_completed: current.comeback_challenge_days_completed ?? 0,
          comeback_challenge_streak_to_restore: current.comeback_challenge_streak_to_restore ?? 0,
    ...
    let streakSabbaticalEnabled = false;
    {
      ...
      if (profileRow?.institution_id) {
        ...
        streakSabbaticalEnabled = (instSettings as Record<string, unknown> | null)?.streak_sabbatical_enabled === true;
      }
    }

    // ── Step 1c: Check Streak Sabbatical — skip streak check on weekends ─

    const isSabbaticalRestDay = isStreakSabbaticalDay(streakSabbaticalEnabled, todayUTC);
    ...
    let comebackStreakToRestore = state?.comeback_challenge_streak_to_restore ?? 0;
      ...
      if (habitsCompletedToday) {
        ...
        if (comebackDaysCompleted >= 3) {
          ...
          try {
            ...
            console.error('Failed to check Comeback Kid badge:', (badgeErr as Error).message);
          }
        }
      } else {

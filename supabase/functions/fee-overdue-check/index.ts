...
serve(async (req) => {
  ...
  try {
    ...
    const structureIds = [...new Set(pendingPayments.map((p: { fee_structure_id: string }) => p.fee_structure_id))];
    ...
    const overduePaymentIds = pendingPayments
      .filter((p: { fee_structure_id: string }) => overdueStructureIds.has(p.fee_structure_id))

...
serve(async (req) => {
  ...
  try {
    ...
    // ── Auth: verify caller JWT and privileged role ─────────────────────────
    ...
    if (
      !callerProfile ||
      !["admin", "coordinator"].includes(callerProfile.role)
    ) {
      return new Response(
        JSON.stringify({
    ...
    let coursesQuery = supabase
      .from("courses")
      .select("id, title, code, semester_id")
    ...
    const gradeMap = new Map(
      (grades ?? []).map(
        (g: { assignment_id: string; score: number; max_score: number }) => [
          g.assignment_id,
          g,
        ]
      )
    ...
    for (const course of (courses ?? []) as Array<{
      id: string;
      title: string;
      code: string;
    }>) {
    ...
    await supabase.storage
      .createBucket("transcripts", { public: false })
      .catch(() => {});

...
function validatePayload(
  ...
  if (!p.semester_id || typeof p.semester_id !== "string") {
    return {
      valid: false,
      error: "semester_id is required and must be a string",
    };
  }
  return {
    valid: true,
    data: {
      ...
      semester_id: p.semester_id as string,
    },
  };
}
...
function generateCourseFilePDF(
  ...
  autoTable(doc, {
    ...
    body: clos.map((c, i) => [
      ...
      c.title,
      c.blooms_level
        ? c.blooms_level.charAt(0).toUpperCase() + c.blooms_level.slice(1)
        : "N/A",
    ]),
    theme: "grid",
  ...
  try {
    ...
    if (assignmentIds.length > 0) {
      ...
      const assignmentTitleMap = new Map(
        assignmentRows.map((a) => [a.id, a.title])
      );

...
export async function authenticateRequest(req: Request): Promise<AuthResult> {
  ...
  const institutionId = user.app_metadata?.institution_id ?? user.user_metadata?.institution_id ?? '';
 ...
 */
export function authenticateCronRequest(req: Request): { authorized: boolean; error: string | null } {
  ...
  return { authorized: false, error: 'Unauthorized: missing or invalid cron secret' };
}

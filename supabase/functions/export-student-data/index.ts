...
serve(async (req) => {
  ...
  try {
    ...
    const { format: fmt = 'json' } = await req.json().catch(() => ({ format: 'json' }));
    ...
    const submissionIds = submissionsResult.data?.map((s: { id: string }) => s.id) ?? [];

    const [profile, grades, attainment, xp, journals, badges, habits] = await Promise.all([
      supabase
        .from('profiles')
    ...
    if (exportFormat === 'csv') {
      ...
      if (exportData.outcome_attainment.length > 0) {
        sections.push(toCsv(exportData.outcome_attainment, 'Outcome Attainment'));
      }

...
serve(async (req) => {
  ...
  try {
    ...
    if (!activeTeams || activeTeams.length === 0) {
      return new Response(
        JSON.stringify({ success: true, teams_checked: 0 }),
    ...
    for (const team of activeTeams) {
      ...
      const missingMembers = memberIds.filter((id: string) => !loggedInIds.has(id));
    ...
    return new Response(
      JSON.stringify({ success: true, teams_checked: activeTeams.length, notifications_sent: notificationsSent }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
    );
  } catch (error) {
    return new Response(

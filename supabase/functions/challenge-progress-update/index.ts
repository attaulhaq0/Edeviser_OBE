  ...
  try {
    ...
    if (!goalMetric) {
      return new Response(
        JSON.stringify({
          success: true,
          message: "No matching challenge metric",
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    const { data: challenges, error: chalErr } = await supabase
      ...
      .select(
        "id, challenge_type, goal_metric, goal_target, notification_sent_90, course_id"
      )
    ...
    for (const challenge of challenges) {
      ...
      if (existing) {
        ...
        await supabase.from("challenge_participants").insert({
          ...
          participant_type:
            challenge.challenge_type === "team" ? "team" : "student",
      ...
      if (
        ...
        const totalProgress = (allParticipants ?? []).reduce(
          (sum: number, p: { current_progress: number }) =>
            sum + p.current_progress,
          0
        );
        ...
        if (totalProgress >= threshold) {
          const progressPercent = Math.round(
            (totalProgress / challenge.goal_target) * 100
          );

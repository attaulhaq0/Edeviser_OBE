...
async function findPrerequisiteCLO(
  ...
  const siblingCloIds = [
    ...new Set(siblingMappings.map((m: { source_outcome_id: string }) => m.source_outcome_id)),
  ];
  ...
  for (const clo of siblingCLOs) {
    ...
    if (level >= 0 && level < weakLevel && level > bestLevel) {
      bestLevel = level;
      bestPrereq = { id: clo.id, title: clo.title, blooms_level: clo.blooms_level };
    }
  }
...
async function buildSocialProof(
  ...
  const avgWithPrereq =
    weakAttainments.reduce((s: number, a: { attainment_percent: number }) => s + a.attainment_percent, 0) /
    weakAttainments.length;

  const avgAll =
    allWeakAttainments.reduce((s: number, a: { attainment_percent: number }) => s + a.attainment_percent, 0) /
...
serve(async (req) => {
  ...
  try {
    ...
    if (attainmentErr) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch attainment data', detail: attainmentErr.message }),
    ...
    if (!weakAttainments || weakAttainments.length === 0) {
      return new Response(
        JSON.stringify({ success: true, suggestions: [], message: 'No weak CLOs found' }),
    ...
    if (cloErr) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch CLO details', detail: cloErr.message }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }

    // Build a lookup map for CLO details
    const cloMap = new Map<string, { title: string; blooms_level: string | null; course_id: string | null }>();
    for (const clo of cloDetails ?? []) {
      cloMap.set(clo.id, { title: clo.title, blooms_level: clo.blooms_level, course_id: clo.course_id });
    }
    ...
    if (suggestions.length > 0) {
      ...
      if (insertErr) {
        ...
        return new Response(
          JSON.stringify({ error: 'Failed to store suggestions', detail: insertErr.message }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
        );
      }
    }
